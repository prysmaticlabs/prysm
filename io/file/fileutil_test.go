...
func TestIsValidFilePath(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "junk path fails",
			args: args{
				fp: " aklsjdja jaj29823. . 5% / $*",
			},
			want: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := file.IsValidFilePath(tt.args.fp)
			if (err != nil) != tt.wantErr {
				t.Errorf("IsValidFilePath() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("IsValidFilePath() got = %v, want %v", got, tt.want)
			}
		})
	}
}
