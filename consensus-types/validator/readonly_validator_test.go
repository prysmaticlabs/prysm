...
func TestReadOnlyValidator_ReturnsErrorOnNil(t *testing.T) {
	if _, err := validator.NewValidator(nil); err != validator.ErrNilWrappedValidator {
		t.Errorf("Wrong error returned. Got %v, wanted %v", err, validator.ErrNilWrappedValidator)
	}
}

func TestReadOnlyValidator_EffectiveBalance(t *testing.T) {
	bal := uint64(234)
	v, err := validator.NewValidator(&ethpb.Validator{EffectiveBalance: bal})
...
func TestReadOnlyValidator_ActivationEligibilityEpoch(t *testing.T) {
	...
	v, err := validator.NewValidator(&ethpb.Validator{ActivationEligibilityEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_ActivationEpoch(t *testing.T) {
	epoch := primitives.Epoch(234)
	v, err := validator.NewValidator(&ethpb.Validator{ActivationEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_WithdrawableEpoch(t *testing.T) {
	epoch := primitives.Epoch(234)
	v, err := validator.NewValidator(&ethpb.Validator{WithdrawableEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_ExitEpoch(t *testing.T) {
	epoch := primitives.Epoch(234)
	v, err := validator.NewValidator(&ethpb.Validator{ExitEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_PublicKey(t *testing.T) {
	...
	v, err := validator.NewValidator(&ethpb.Validator{PublicKey: key[:]})
	require.NoError(t, err)
...
func TestReadOnlyValidator_WithdrawalCredentials(t *testing.T) {
	...
	v, err := validator.NewValidator(&ethpb.Validator{WithdrawalCredentials: creds})
	require.NoError(t, err)
	assert.DeepEqual(t, creds, v.WithdrawalCredentials())
}

func TestReadOnlyValidator_Slashed(t *testing.T) {
	v, err := validator.NewValidator(&ethpb.Validator{Slashed: true})
	require.NoError(t, err)
