...
func TestStateReferenceCopy_NoUnexpectedAttestationsMutation(t *testing.T) {
	...
	// Mutator should only affect calling state: a.
	applyToEveryAttestation := func(state *State) {
...
func TestValidatorReferences_RemainsConsistent_Phase0(t *testing.T) {
	...
	assert.NoError(t, a.ReadFromEveryValidator(func(idx int, val types.ReadOnlyValidator) error {
...
func assertRefCount(t *testing.T, b *State, idx types.FieldIndex, want uint) {
