...
func TestReadOnlyValidator_ReturnsErrorOnNil(t *testing.T) {
	if _, err := state.NewValidator(nil); err != state.ErrNilWrappedValidator {
		t.Errorf("Wrong error returned. Got %v, wanted %v", err, state.ErrNilWrappedValidator)
	}
}

func TestReadOnlyValidator_EffectiveBalance(t *testing.T) {
	bal := uint64(234)
	v, err := state.NewValidator(&ethpb.Validator{EffectiveBalance: bal})
...
func TestReadOnlyValidator_ActivationEligibilityEpoch(t *testing.T) {
	...
	v, err := state.NewValidator(&ethpb.Validator{ActivationEligibilityEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_ActivationEpoch(t *testing.T) {
	epoch := primitives.Epoch(234)
	v, err := state.NewValidator(&ethpb.Validator{ActivationEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_WithdrawableEpoch(t *testing.T) {
	epoch := primitives.Epoch(234)
	v, err := state.NewValidator(&ethpb.Validator{WithdrawableEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_ExitEpoch(t *testing.T) {
	epoch := primitives.Epoch(234)
	v, err := state.NewValidator(&ethpb.Validator{ExitEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_PublicKey(t *testing.T) {
	...
	v, err := state.NewValidator(&ethpb.Validator{PublicKey: key[:]})
	require.NoError(t, err)
...
func TestReadOnlyValidator_WithdrawalCredentials(t *testing.T) {
	...
	v, err := state.NewValidator(&ethpb.Validator{WithdrawalCredentials: creds})
	require.NoError(t, err)
...
func TestReadOnlyValidator_HasETH1WithdrawalCredentials(t *testing.T) {
	...
	// No Withdrawal cred
	v, err = state.NewValidator(&ethpb.Validator{})
	require.NoError(t, err)
	require.Equal(t, false, v.HasETH1WithdrawalCredential())
}
...
func TestReadOnlyValidator_Slashed(t *testing.T) {
	v, err := state.NewValidator(&ethpb.Validator{Slashed: true})
	require.NoError(t, err)
