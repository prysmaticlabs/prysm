...
func setupDB() {
	...
	if err := db1.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b)); err != nil {
		panic(err)
	}
...
func BeaconFuzzBlock(b []byte) {
	...
	if _, err := state.ProcessBlock(ctx, st, blockInterface.WrappedPhase0SignedBeaconBlock(input.Block)); err != nil {
		_ = err
	}
}
