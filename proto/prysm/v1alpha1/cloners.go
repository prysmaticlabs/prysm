...
func CopyETH1Data(data *Eth1Data) *Eth1Data {
	...
	return &Eth1Data{
		DepositRoot:  butil.SafeCopyBytes(data.DepositRoot),
		DepositCount: data.DepositCount,
		BlockHash:    butil.SafeCopyBytes(data.BlockHash),
	}
}
...
func CopyPendingAttestation(att *PendingAttestation) *PendingAttestation {
	...
	return &PendingAttestation{
		AggregationBits: butil.SafeCopyBytes(att.AggregationBits),
		Data:            data,
...
func CopyAttestation(att *Attestation) *Attestation {
	...
	return &Attestation{
		AggregationBits: butil.SafeCopyBytes(att.AggregationBits),
		Data:            CopyAttestationData(att.Data),
		Signature:       butil.SafeCopyBytes(att.Signature),
	}
}
...
func CopyAttestationData(attData *AttestationData) *AttestationData {
	...
	return &AttestationData{
		...
		BeaconBlockRoot: butil.SafeCopyBytes(attData.BeaconBlockRoot),
...
func CopyCheckpoint(cp *Checkpoint) *Checkpoint {
	...
	return &Checkpoint{
		Epoch: cp.Epoch,
		Root:  butil.SafeCopyBytes(cp.Root),
	}
}
...
func CopySignedBeaconBlock(sigBlock *SignedBeaconBlock) *SignedBeaconBlock {
	...
	return &SignedBeaconBlock{
		...
		Signature: butil.SafeCopyBytes(sigBlock.Signature),
	}
}
...
func CopyBeaconBlock(block *BeaconBlock) *BeaconBlock {
	...
	return &BeaconBlock{
		...
		ParentRoot:    butil.SafeCopyBytes(block.ParentRoot),
		StateRoot:     butil.SafeCopyBytes(block.StateRoot),
...
func CopyBeaconBlockBody(body *BeaconBlockBody) *BeaconBlockBody {
	...
	return &BeaconBlockBody{
		RandaoReveal:      butil.SafeCopyBytes(body.RandaoReveal),
		Eth1Data:          CopyETH1Data(body.Eth1Data),
		Graffiti:          butil.SafeCopyBytes(body.Graffiti),
...
func CopySignedBeaconBlockHeader(header *SignedBeaconBlockHeader) *SignedBeaconBlockHeader {
	...
	return &SignedBeaconBlockHeader{
		...
		Signature: butil.SafeCopyBytes(header.Signature),
	}
}
...
func CopyBeaconBlockHeader(header *BeaconBlockHeader) *BeaconBlockHeader {
	...
	parentRoot := butil.SafeCopyBytes(header.ParentRoot)
	stateRoot := butil.SafeCopyBytes(header.StateRoot)
	bodyRoot := butil.SafeCopyBytes(header.BodyRoot)
...
func CopyIndexedAttestation(indexedAtt *IndexedAttestation) *IndexedAttestation {
	...
	return &IndexedAttestation{
		...
		Signature:        butil.SafeCopyBytes(indexedAtt.Signature),
	}
}
...
func CopyDeposit(deposit *Deposit) *Deposit {
	...
	return &Deposit{
		Proof: butil.SafeCopy2dBytes(deposit.Proof),
...
func CopyDepositData(depData *Deposit_Data) *Deposit_Data {
	...
	return &Deposit_Data{
		PublicKey:             butil.SafeCopyBytes(depData.PublicKey),
		WithdrawalCredentials: butil.SafeCopyBytes(depData.WithdrawalCredentials),
		Amount:                depData.Amount,
		Signature:             butil.SafeCopyBytes(depData.Signature),
	}
}
...
func CopySignedVoluntaryExit(exit *SignedVoluntaryExit) *SignedVoluntaryExit {
	...
	return &SignedVoluntaryExit{
		...
		Signature: butil.SafeCopyBytes(exit.Signature),
	}
}
...
func CopySyncCommitteeMessage(s *SyncCommitteeMessage) *SyncCommitteeMessage {
	...
	return &SyncCommitteeMessage{
		Slot:           s.Slot,
		BlockRoot:      butil.SafeCopyBytes(s.BlockRoot),
		ValidatorIndex: s.ValidatorIndex,
		Signature:      butil.SafeCopyBytes(s.Signature),
	}
}
...
func CopySyncCommitteeContribution(c *SyncCommitteeContribution) *SyncCommitteeContribution {
	...
	return &SyncCommitteeContribution{
		Slot:              c.Slot,
		BlockRoot:         butil.SafeCopyBytes(c.BlockRoot),
		SubcommitteeIndex: c.SubcommitteeIndex,
		AggregationBits:   butil.SafeCopyBytes(c.AggregationBits),
		Signature:         butil.SafeCopyBytes(c.Signature),
	}
}
...
func CopySyncAggregate(a *SyncAggregate) *SyncAggregate {
	...
	return &SyncAggregate{
		SyncCommitteeBits:      butil.SafeCopyBytes(a.SyncCommitteeBits),
		SyncCommitteeSignature: butil.SafeCopyBytes(a.SyncCommitteeSignature),
	}
}
