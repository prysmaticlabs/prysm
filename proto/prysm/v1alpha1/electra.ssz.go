...
// Hash: 9ff01d27bc1bd8141e32774d06354f56f6e54cbaed880dc771a5d457f306e985
package eth
...
func (i *InclusionList) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (0) 'InclusionInfos'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(i.InclusionInfos) * 32

	// Field (0) 'InclusionInfos'
	if size := len(i.InclusionInfos); size > 32 {
		err = ssz.ErrListTooBigFn("--.InclusionInfos", size, 32)
		return
	}
	for ii := 0; ii < len(i.InclusionInfos); ii++ {
		if dst, err = i.InclusionInfos[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
func (i *InclusionList) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'InclusionInfos'
	{
		...
		i.InclusionInfos = make([]*InclusionList_InclusionInfo, num)
		for ii := 0; ii < num; ii++ {
			if i.InclusionInfos[ii] == nil {
				i.InclusionInfos[ii] = new(InclusionList_InclusionInfo)
			}
			if err = i.InclusionInfos[ii].UnmarshalSSZ(buf[ii*32 : (ii+1)*32]); err != nil {
				return err
			}
		}
	}
...
func (i *InclusionList) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	{
		subIndx := hh.Index()
		num := uint64(len(i.InclusionInfos))
		if num > 32 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for _, elem := range i.InclusionInfos {
			if err = elem.HashTreeRootWith(hh); err != nil {
				return
			}
		}
...
// MarshalSSZ ssz marshals the InclusionList_InclusionInfo object
func (i *InclusionList_InclusionInfo) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(i)
}

// MarshalSSZTo ssz marshals the InclusionList_InclusionInfo object to a target array
func (i *InclusionList_InclusionInfo) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf

	// Field (0) 'TxHash'
	if size := len(i.TxHash); size != 32 {
		err = ssz.ErrBytesLengthFn("--.TxHash", size, 32)
		return
	}
	dst = append(dst, i.TxHash...)

	return
}

// UnmarshalSSZ ssz unmarshals the InclusionList_InclusionInfo object
func (i *InclusionList_InclusionInfo) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size != 32 {
		return ssz.ErrSize
	...
	// Field (0) 'TxHash'
	if cap(i.TxHash) == 0 {
		i.TxHash = make([]byte, 0, len(buf[0:32]))
	}
	i.TxHash = append(i.TxHash, buf[0:32]...)

	return err
}

// SizeSSZ returns the ssz encoded size in bytes for the InclusionList_InclusionInfo object
func (i *InclusionList_InclusionInfo) SizeSSZ() (size int) {
	size = 32
	return
}

// HashTreeRoot ssz hashes the InclusionList_InclusionInfo object
func (i *InclusionList_InclusionInfo) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(i)
}

// HashTreeRootWith ssz hashes the InclusionList_InclusionInfo object with a hasher
func (i *InclusionList_InclusionInfo) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
