...
// Hash: 32ac97fb9f4cd4b21151fdb5b9b4440095e1fa4bcc1f081b5f695eb8726919a2
package eth
...
// MarshalSSZ ssz marshals the BlindedBlobSidecars object
func (b *BlindedBlobSidecars) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(b)
}

// MarshalSSZTo ssz marshals the BlindedBlobSidecars object to a target array
func (b *BlindedBlobSidecars) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(4)

	// Offset (0) 'Sidecars'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(b.Sidecars) * 216
...
// UnmarshalSSZ ssz unmarshals the BlindedBlobSidecars object
func (b *BlindedBlobSidecars) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 4 {
		return ssz.ErrSize
	...
	// Field (0) 'Sidecars'
	{
		...
		b.Sidecars = make([]*BlindedBlobSidecar, num)
		for ii := 0; ii < num; ii++ {
			if b.Sidecars[ii] == nil {
				b.Sidecars[ii] = new(BlindedBlobSidecar)
			}
			if err = b.Sidecars[ii].UnmarshalSSZ(buf[ii*216 : (ii+1)*216]); err != nil {
				return err
			}
		}
	}
...
// SizeSSZ returns the ssz encoded size in bytes for the BlindedBlobSidecars object
func (b *BlindedBlobSidecars) SizeSSZ() (size int) {
	size = 4

	// Field (0) 'Sidecars'
	size += len(b.Sidecars) * 216

	return
}

// HashTreeRoot ssz hashes the BlindedBlobSidecars object
func (b *BlindedBlobSidecars) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(b)
}

// HashTreeRootWith ssz hashes the BlindedBlobSidecars object with a hasher
func (b *BlindedBlobSidecars) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
