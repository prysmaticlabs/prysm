...
// Hash: 172be8f5bc7a873e7286dcc9be5784b6fa9d03fcc3f05523c7102dce2cc1f927
package eth
...
func (b *BeaconBlockAndBlobsDeneb) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'Blobs'
	{
		...
		b.Blobs = make([]*DeprecatedBlobSidecar, num)
		for ii := 0; ii < num; ii++ {
			if b.Blobs[ii] == nil {
				b.Blobs[ii] = new(DeprecatedBlobSidecar)
			}
...
// MarshalSSZ ssz marshals the BlobSidecarNew object
func (b *BlobSidecarNew) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(b)
}

// MarshalSSZTo ssz marshals the BlobSidecarNew object to a target array
func (b *BlobSidecarNew) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(131388)
	...
	// Offset (5) 'CommitmentInclusionProof'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(b.CommitmentInclusionProof) * 32

	// Field (5) 'CommitmentInclusionProof'
	if size := len(b.CommitmentInclusionProof); size > 17 {
		err = ssz.ErrListTooBigFn("--.CommitmentInclusionProof", size, 17)
		return
	}
	for ii := 0; ii < len(b.CommitmentInclusionProof); ii++ {
...
// UnmarshalSSZ ssz unmarshals the BlobSidecarNew object
func (b *BlobSidecarNew) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 131388 {
		return ssz.ErrSize
	...
	// Offset (5) 'CommitmentInclusionProof'
	if o5 = ssz.ReadOffset(buf[131384:131388]); o5 > size {
		return ssz.ErrOffset
	}
	...
	{
		buf = tail[o5:]
		num, err := ssz.DivideInt2(len(buf), 32, 17)
		if err != nil {
			return err
		}
		b.CommitmentInclusionProof = make([][]byte, num)
		for ii := 0; ii < num; ii++ {
			if cap(b.CommitmentInclusionProof[ii]) == 0 {
				b.CommitmentInclusionProof[ii] = make([]byte, 0, len(buf[ii*32:(ii+1)*32]))
			}
			b.CommitmentInclusionProof[ii] = append(b.CommitmentInclusionProof[ii], buf[ii*32:(ii+1)*32]...)
		}
	}
	return err
}

// SizeSSZ returns the ssz encoded size in bytes for the BlobSidecarNew object
func (b *BlobSidecarNew) SizeSSZ() (size int) {
	size = 131388

	// Field (5) 'CommitmentInclusionProof'
	size += len(b.CommitmentInclusionProof) * 32

	return
}

// HashTreeRoot ssz hashes the BlobSidecarNew object
func (b *BlobSidecarNew) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(b)
}

// HashTreeRootWith ssz hashes the BlobSidecarNew object with a hasher
func (b *BlobSidecarNew) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	...
	// Field (5) 'CommitmentInclusionProof'
	{
		...
		numItems := uint64(len(b.CommitmentInclusionProof))
		if ssz.EnableVectorizedHTR {
			hh.MerkleizeWithMixinVectorizedHTR(subIndx, numItems, 17)
		} else {
			hh.MerkleizeWithMixin(subIndx, numItems, 17)
		}
	}
