...
// Hash: eda26a4ff16db48e86ff3dfa66143fcc85f89f047ad3255d052d73c57b10e1e9
package eth
...
func (b *BeaconBlockBodyWithBlobKZGs) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	dst = ssz.WriteOffset(dst, offset)
	if b.ExecutionPayload == nil {
		b.ExecutionPayload = new(v1.ExecutionPayloadWithBlobTxs)
	}
	offset += b.ExecutionPayload.SizeSSZ()
...
// MarshalSSZ ssz marshals the BeaconBlockAndBlobs object
func (b *BeaconBlockAndBlobs) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(b)
}

// MarshalSSZTo ssz marshals the BeaconBlockAndBlobs object to a target array
func (b *BeaconBlockAndBlobs) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(8)
	...
	dst = ssz.WriteOffset(dst, offset)
	if b.Block == nil {
		b.Block = new(BeaconBlockWithBlobKZGs)
	}
	offset += b.Block.SizeSSZ()

	// Offset (1) 'Blobs'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(b.Blobs) * 196608
...
// UnmarshalSSZ ssz unmarshals the BeaconBlockAndBlobs object
func (b *BeaconBlockAndBlobs) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 8 {
		return ssz.ErrSize
	...
	{
		buf = tail[o1:]
		num, err := ssz.DivideInt2(len(buf), 196608, 16777216)
		if err != nil {
			return err
		}
		...
		for ii := 0; ii < num; ii++ {
			...
			if err = b.Blobs[ii].UnmarshalSSZ(buf[ii*196608 : (ii+1)*196608]); err != nil {
				return err
			}
		}
	}
...
// SizeSSZ returns the ssz encoded size in bytes for the BeaconBlockAndBlobs object
func (b *BeaconBlockAndBlobs) SizeSSZ() (size int) {
	size = 8
	...
	// Field (1) 'Blobs'
	size += len(b.Blobs) * 196608

	return
}

// HashTreeRoot ssz hashes the BeaconBlockAndBlobs object
func (b *BeaconBlockAndBlobs) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(b)
}

// HashTreeRootWith ssz hashes the BeaconBlockAndBlobs object with a hasher
func (b *BeaconBlockAndBlobs) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
...
// MarshalSSZ ssz marshals the SignedBeaconBlockAndBlobs object
func (s *SignedBeaconBlockAndBlobs) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(s)
}

// MarshalSSZTo ssz marshals the SignedBeaconBlockAndBlobs object to a target array
func (s *SignedBeaconBlockAndBlobs) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(8)
	...
	dst = ssz.WriteOffset(dst, offset)
	if s.Block == nil {
		s.Block = new(SignedBeaconBlockWithBlobKZGs)
	}
	offset += s.Block.SizeSSZ()

	// Offset (1) 'Blobs'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(s.Blobs) * 196608
...
// UnmarshalSSZ ssz unmarshals the SignedBeaconBlockAndBlobs object
func (s *SignedBeaconBlockAndBlobs) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 8 {
		return ssz.ErrSize
	...
	// Field (1) 'Blobs'
	{
		...
		s.Blobs = make([]*Blob, num)
		for ii := 0; ii < num; ii++ {
			if s.Blobs[ii] == nil {
				s.Blobs[ii] = new(Blob)
			}
			if err = s.Blobs[ii].UnmarshalSSZ(buf[ii*196608 : (ii+1)*196608]); err != nil {
				return err
			}
		}
	}
...
// SizeSSZ returns the ssz encoded size in bytes for the SignedBeaconBlockAndBlobs object
func (s *SignedBeaconBlockAndBlobs) SizeSSZ() (size int) {
	size = 8
	...
	// Field (1) 'Blobs'
	size += len(s.Blobs) * 196608

	return
}

// HashTreeRoot ssz hashes the SignedBeaconBlockAndBlobs object
func (s *SignedBeaconBlockAndBlobs) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(s)
}

// HashTreeRootWith ssz hashes the SignedBeaconBlockAndBlobs object with a hasher
func (s *SignedBeaconBlockAndBlobs) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
...
func (b *Blob) UnmarshalSSZ(buf []byte) error {
	...
	for ii := 0; ii < 4096; ii++ {
		if cap(b.Blob[ii]) == 0 {
			b.Blob[ii] = make([]byte, 0, len(buf[0:196608][ii*48:(ii+1)*48]))
		}
		b.Blob[ii] = append(b.Blob[ii], buf[0:196608][ii*48:(ii+1)*48]...)
	}
