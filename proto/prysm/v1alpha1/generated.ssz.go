...
// Hash: 32c426b37fe70de59552f3c037ff9d6f8a2f15ba5f2cc56655215b90fe9fcd5f
package eth
...
func (e *ExecutionPayload) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	dst = ssz.WriteOffset(dst, offset)
	offset += len(e.Transactions) * 32

	// Field (12) 'Transactions'
	if len(e.Transactions) > 16777216 {
		err = ssz.ErrListTooBig
		return
	}
	for ii := 0; ii < len(e.Transactions); ii++ {
		if len(e.Transactions[ii]) != 32 {
			err = ssz.ErrBytesLength
			return
		}
...
func (e *ExecutionPayload) UnmarshalSSZ(buf []byte) error {
	...
	if cap(e.BaseFeePerGas) == 0 {
		e.BaseFeePerGas = make([]byte, 0, len(buf[436:468]))
	}
	e.BaseFeePerGas = append(e.BaseFeePerGas, buf[436:468]...)

	// Field (11) 'BlockHash'
	if cap(e.BlockHash) == 0 {
		e.BlockHash = make([]byte, 0, len(buf[468:500]))
	}
	e.BlockHash = append(e.BlockHash, buf[468:500]...)
	...
	// Field (12) 'Transactions'
	{
		...
		for ii := 0; ii < num; ii++ {
			if cap(e.Transactions[ii]) == 0 {
				e.Transactions[ii] = make([]byte, 0, len(buf[ii*32:(ii+1)*32]))
			}
			e.Transactions[ii] = append(e.Transactions[ii], buf[ii*32:(ii+1)*32]...)
		}
	}
	return err
}
...
func (e *ExecutionPayload) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	// Field (12) 'Transactions'
	{
		...
		numItems := uint64(len(e.Transactions))
...
func (b *BeaconStateMerge) UnmarshalSSZ(buf []byte) error {
	...
	if err = b.LatestExecutionPayloadHeader.UnmarshalSSZ(buf[2736629:2737161]); err != nil {
		return err
	}
...
func (e *ExecutionPayloadHeader) UnmarshalSSZ(buf []byte) error {
	...
	if cap(e.TransactionsRoot) == 0 {
		e.TransactionsRoot = make([]byte, 0, len(buf[500:532]))
	}
	e.TransactionsRoot = append(e.TransactionsRoot, buf[500:532]...)
