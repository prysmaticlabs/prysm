...
func (c *LightClientBootstrapDeneb) UnmarshalSSZ(buf []byte) error {
	...
	sszSlice2 := buf[1588:1748] // c.CurrentSyncCommitteeBranch

	sszVarOffset0 := ssz.ReadOffset(buf[0:4]) // c.Header
	if sszVarOffset0 != 1748 {
		return ssz.ErrInvalidVariableOffset
	}
	if sszVarOffset0 > size {
		return ssz.ErrOffset
...
func (c *LightClientUpdateDeneb) UnmarshalSSZ(buf []byte) error {
	...
	sszSlice2 := buf[1588:1748] // c.NextSyncCommitteeBranch
	sszSlice4 := buf[1752:1944] // c.FinalityBranch
	sszSlice5 := buf[1944:2044] // c.SyncAggregate
	sszSlice6 := buf[2044:2052] // c.SignatureSlot

	sszVarOffset0 := ssz.ReadOffset(buf[0:4]) // c.AttestedHeader
	if sszVarOffset0 != 2052 {
		return ssz.ErrInvalidVariableOffset
	}
	if sszVarOffset0 > size {
		return ssz.ErrOffset
	}
	sszVarOffset3 := ssz.ReadOffset(buf[1748:1752]) // c.FinalizedHeader
...
func (c *LightClientFinalityUpdateDeneb) UnmarshalSSZ(buf []byte) error {
	...
	sszSlice3 := buf[200:300] // c.SyncAggregate
	sszSlice4 := buf[300:308] // c.SignatureSlot

	sszVarOffset0 := ssz.ReadOffset(buf[0:4]) // c.AttestedHeader
	if sszVarOffset0 != 308 {
		return ssz.ErrInvalidVariableOffset
	}
	if sszVarOffset0 > size {
		return ssz.ErrOffset
...
func (c *LightClientOptimisticUpdateDeneb) UnmarshalSSZ(buf []byte) error {
	...
	sszSlice1 := buf[4:104]   // c.SyncAggregate
	sszSlice2 := buf[104:112] // c.SignatureSlot

	sszVarOffset0 := ssz.ReadOffset(buf[0:4]) // c.AttestedHeader
	if sszVarOffset0 != 112 {
		return ssz.ErrInvalidVariableOffset
	}
	if sszVarOffset0 > size {
		return ssz.ErrOffset
