...
func (t *Transaction) HashTreeRootWith(hh *ssz.Hasher) error {
	...
	switch t.TransactionOneof.(type) {
	case *Transaction_OpaqueTransaction:
		rawTx := t.GetOpaqueTransaction()
		opaque := make([]byte, len(rawTx))
		copy(opaque, rawTx)
		byteLen := uint64(len(opaque))
		if byteLen > 1048576 {
			return ssz.ErrIncorrectListSize
		}
		chunks, err := packBytes(opaque)
		if err != nil {
			return err
		}
		bytesRoot, err := BitwiseMerkleize(customHasher(), chunks, uint64(len(chunks)), (1048576+31)/32)
		if err != nil {
			return err
		}
		bytesRootBufRoot := make([]byte, 32)
		binary.LittleEndian.PutUint64(bytesRootBufRoot[:8], byteLen)
		mixInRoot := MixInLength(bytesRoot, bytesRootBufRoot)
		hh.PutBytes(mixInRoot[:])
...
func packBytes(bytes []byte) ([][]byte, error) {
	numItems := len(bytes)
	var chunks [][]byte
...
func BitwiseMerkleize(hasher HashFn, chunks [][]byte, count, limit uint64) ([32]byte, error) {
	if count > limit {
		return [32]byte{}, fmt.Errorf("merkleizing list that is too large, over limit")
	}
...
func customHasher() func(data []byte) [32]byte {
	hasher := sha256.New()
	var h [32]byte
