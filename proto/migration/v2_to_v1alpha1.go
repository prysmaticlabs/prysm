...
func SignedBlobsToV1Alpha1SignedBlobs(sidecars []*ethpbv2.BlobSidecar) []*ethpbalpha.BlobSidecar {
	result := make([]*ethpbalpha.BlobSidecar, len(sidecars))
	for i, sc := range sidecars {
		result[i] = &ethpbalpha.BlobSidecar{
			Index:         sc.Index,
			Blob:          bytesutil.SafeCopyBytes(sc.Blob),
			KzgCommitment: bytesutil.SafeCopyBytes(sc.KzgCommitment),
			KzgProof:      bytesutil.SafeCopyBytes(sc.KzgProof),
			// TODO: add signed block header
		}
	}
	return result
}
