...
func (c *SSZNewPayloadRequest) SizeSSZ() int {
	size := 44
	if c.ExecutionPayload == nil {
		c.ExecutionPayload = new(StatelessExecutionPayload)
	}
	size += c.ExecutionPayload.SizeSSZ()
	size += len(c.VersionedHashes) * 32
	if c.ExecutionRequests == nil {
		c.ExecutionRequests = new(StatelessExecutionRequests)
	}
	size += c.ExecutionRequests.SizeSSZ()
	return size
}

func (c *SSZNewPayloadRequest) MarshalSSZ() ([]byte, error) {
	buf := make([]byte, c.SizeSSZ())
	return c.MarshalSSZTo(buf[:0])
}

func (c *SSZNewPayloadRequest) MarshalSSZTo(dst []byte) ([]byte, error) {
	var err error
	offset := 44
	...
	dst = ssz.WriteOffset(dst, offset)
	offset += len(c.VersionedHashes) * 32
	...
	// Field 1: VersionedHashes
	if len(c.VersionedHashes) > 4096 {
		return nil, ssz.ErrListTooBig
	}
	for _, o := range c.VersionedHashes {
		if len(o) != 32 {
			return nil, ssz.ErrBytesLength
		}
		dst = append(dst, o...)
...
func (c *SSZNewPayloadRequest) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 44 {
		return ssz.ErrSize
	...
	sszSlice2 := buf[8:40] // c.ParentBeaconBlockRoot

	sszVarOffset0 := ssz.ReadOffset(buf[0:4]) // c.ExecutionPayload
	if sszVarOffset0 != 44 {
		return ssz.ErrInvalidVariableOffset
	}
	if sszVarOffset0 > size {
	...
	sszVarOffset1 := ssz.ReadOffset(buf[4:8]) // c.VersionedHashes
	if sszVarOffset1 > size || sszVarOffset1 < sszVarOffset0 {
		return ssz.ErrOffset
	}
	sszVarOffset3 := ssz.ReadOffset(buf[40:44]) // c.ExecutionRequests
	if sszVarOffset3 > size || sszVarOffset3 < sszVarOffset1 {
		return ssz.ErrOffset
	}
	sszSlice0 := buf[sszVarOffset0:sszVarOffset1] // c.ExecutionPayload
	sszSlice1 := buf[sszVarOffset1:sszVarOffset3] // c.VersionedHashes
	sszSlice3 := buf[sszVarOffset3:]              // c.ExecutionRequests

	// Field 0: ExecutionPayload
	c.ExecutionPayload = new(StatelessExecutionPayload)
	if err = c.ExecutionPayload.UnmarshalSSZ(sszSlice0); err != nil {
		return fmt.Errorf("ExecutionPayload: %w", err)
	}

	// Field 1: VersionedHashes
	{
		if len(sszSlice1)%32 != 0 {
			return fmt.Errorf("misaligned bytes: c.VersionedHashes length is %d, which is not a multiple of 32: %w", len(sszSlice1), ssz.ErrIncorrectListSize)
		}
		numElem := len(sszSlice1) / 32
		if numElem > 4096 {
			return fmt.Errorf("ssz-max exceeded: c.VersionedHashes has %d elements, ssz-max is 4096: %w", numElem, ssz.ErrListTooBig)
		}
		c.VersionedHashes = make([][]byte, numElem)
		for i := 0; i < numElem; i++ {
			var tmp []byte
			...
			tmp = make([]byte, 0, 32)
			tmp = append(tmp, tmpSlice...)
			c.VersionedHashes[i] = tmp
		}
	}

	// Field 2: ParentBeaconBlockRoot
	c.ParentBeaconBlockRoot = make([]byte, 0, 32)
	c.ParentBeaconBlockRoot = append(c.ParentBeaconBlockRoot, sszSlice2...)

	// Field 3: ExecutionRequests
	c.ExecutionRequests = new(StatelessExecutionRequests)
	if err = c.ExecutionRequests.UnmarshalSSZ(sszSlice3); err != nil {
		return fmt.Errorf("ExecutionRequests: %w", err)
	}
	return err
}

func (c *SSZNewPayloadRequest) HashTreeRoot() ([32]byte, error) {
	hh := ssz.DefaultHasherPool.Get()
	if err := c.HashTreeRootWith(hh); err != nil {
		ssz.DefaultHasherPool.Put(hh)
		return [32]byte{}, err
...
func (c *SSZNewPayloadRequest) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	// Field 0: ExecutionPayload
	...
	// Field 1: VersionedHashes
	{
		...
		hh.MerkleizeWithMixin(subIndx, uint64(len(c.VersionedHashes)), 4096)
	}
	// Field 2: ParentBeaconBlockRoot
	if len(c.ParentBeaconBlockRoot) != 32 {
		return ssz.ErrBytesLength
...
func (c *StatelessExecutionPayload) SizeSSZ() int {
	size := 540
	size += len(c.ExtraData)
	for _, o := range c.Transactions {
		size += 4
...
func (c *StatelessExecutionPayload) MarshalSSZ() ([]byte, error) {
	buf := make([]byte, c.SizeSSZ())
	return c.MarshalSSZTo(buf[:0])
}

func (c *StatelessExecutionPayload) MarshalSSZTo(dst []byte) ([]byte, error) {
	var err error
	offset := 540
...
func (c *StatelessExecutionPayload) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 540 {
		return ssz.ErrSize
...
func (c *StatelessExecutionPayload) HashTreeRoot() ([32]byte, error) {
	hh := ssz.DefaultHasherPool.Get()
	if err := c.HashTreeRootWith(hh); err != nil {
		ssz.DefaultHasherPool.Put(hh)
		return [32]byte{}, err
...
func (c *StatelessExecutionPayload) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	// Field 0: ParentHash
	if len(c.ParentHash) != 32 {
		return ssz.ErrBytesLength
...
func (c *StatelessExecutionRequests) SizeSSZ() int {
	size := 20
	size += len(c.Deposits) * 192
	size += len(c.Withdrawals) * 76
	size += len(c.Consolidations) * 116
...
func (c *StatelessExecutionRequests) MarshalSSZ() ([]byte, error) {
	buf := make([]byte, c.SizeSSZ())
	return c.MarshalSSZTo(buf[:0])
}

func (c *StatelessExecutionRequests) MarshalSSZTo(dst []byte) ([]byte, error) {
	var err error
	offset := 20
...
func (c *StatelessExecutionRequests) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 20 {
		return ssz.ErrSize
...
func (c *StatelessExecutionRequests) HashTreeRoot() ([32]byte, error) {
	hh := ssz.DefaultHasherPool.Get()
	if err := c.HashTreeRootWith(hh); err != nil {
		ssz.DefaultHasherPool.Put(hh)
		return [32]byte{}, err
...
func (c *StatelessExecutionRequests) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	// Field 0: Deposits
	{
		if len(c.Deposits) > 8192 {
	...
	// Field 3: BuilderDeposits
	{
		...
		hh.MerkleizeWithMixin(subIndx, uint64(len(c.BuilderDeposits)), 64)
	}
	// Field 4: BuilderExits
	{
		if len(c.BuilderExits) > 16 {
