...
// Hash: e1c100e331c234595f640f5dc73171ae1151683fda451733feaaa51d83d12fa2
package enginev1
...
func (i *InclusionListSummary) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	for ii := 0; ii < len(i.Entries); ii++ {
		if size := len(i.Entries[ii]); size != 20 {
			err = ssz.ErrBytesLengthFn("--.Entries[ii]", size, 20)
			return
		}
		dst = append(dst, i.Entries[ii]...)
	}
...
func (i *InclusionListSummary) UnmarshalSSZ(buf []byte) error {
	...
	{
		buf = tail[o1:]
		num, err := ssz.DivideInt2(len(buf), 20, 16)
		if err != nil {
			return err
		}
		i.Entries = make([][]byte, num)
		for ii := 0; ii < num; ii++ {
			if cap(i.Entries[ii]) == 0 {
				i.Entries[ii] = make([]byte, 0, len(buf[ii*20:(ii+1)*20]))
			}
			i.Entries[ii] = append(i.Entries[ii], buf[ii*20:(ii+1)*20]...)
		}
	}
	return err
}
...
func (i *InclusionList) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'Slot'
	i.Slot = github_com_prysmaticlabs_prysm_v5_consensus_types_primitives.Slot(ssz.UnmarshallUint64(buf[4:12]))
...
func (e *ExecutionPayloadEnvelope) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (5) 'InclusionListSignature'
...
func (e *ExecutionPayloadEnvelope) UnmarshalSSZ(buf []byte) error {
	...
	if cap(e.InclusionListSignature) == 0 {
		e.InclusionListSignature = make([]byte, 0, len(buf[56:152]))
	}
	e.InclusionListSignature = append(e.InclusionListSignature, buf[56:152]...)

	// Field (6) 'StateRoot'
	if cap(e.StateRoot) == 0 {
		e.StateRoot = make([]byte, 0, len(buf[152:184]))
	}
	e.StateRoot = append(e.StateRoot, buf[152:184]...)
