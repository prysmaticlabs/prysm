...
func (c *NewPayloadRequest) SizeSSZ() int {
	size := 44
	if c.ExecutionPayload == nil {
		c.ExecutionPayload = new(ExecutionPayloadDeneb)
	}
	size += c.ExecutionPayload.SizeSSZ()
	size += len(c.VersionedHashes) * 32
	if c.ExecutionRequests == nil {
		c.ExecutionRequests = new(ExecutionRequestsGloas)
	}
	size += c.ExecutionRequests.SizeSSZ()
	return size
}

func (c *NewPayloadRequest) MarshalSSZ() ([]byte, error) {
	buf := make([]byte, c.SizeSSZ())
	return c.MarshalSSZTo(buf[:0])
}

func (c *NewPayloadRequest) MarshalSSZTo(dst []byte) ([]byte, error) {
	var err error
	offset := 44
...
func (c *NewPayloadRequest) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 44 {
		return ssz.ErrSize
	...
	sszSlice2 := buf[8:40] // c.ParentBlockRoot
	...
	c.ParentBlockRoot = append(c.ParentBlockRoot, sszSlice2...)
...
func (c *NewPayloadRequest) HashTreeRoot() ([32]byte, error) {
	hh := ssz.DefaultHasherPool.Get()
	if err := c.HashTreeRootWith(hh); err != nil {
		ssz.DefaultHasherPool.Put(hh)
		return [32]byte{}, err
...
func (c *NewPayloadRequest) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	// Field 0: ExecutionPayload
