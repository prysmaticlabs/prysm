...
func RunBLSToExecutionChangeTest(t *testing.T, config string) {
	...
	testFolders, testsFolderPath := utils.TestFolders(t, config, "electra", "operations/bls_to_execution_change/pyspec_tests")
	if len(testFolders) == 0 {
		t.Fatalf("No test folders found for %s/%s/%s", config, "electra", "operations/bls_to_execution_change/pyspec_tests")
	}
