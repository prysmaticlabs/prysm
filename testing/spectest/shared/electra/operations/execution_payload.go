...
func RunExecutionPayloadTest(t *testing.T, config string) {
	...
	if len(testFolders) == 0 {
		t.Fatalf("No test folders found for %s/%s/%s", config, "electra", "operations/execution_payload/pyspec_tests")
	}
	for _, folder := range testFolders {
		t.Run(folder.Name(), func(t *testing.T) {
			...
			b, err := blocks2.NewBeaconBlock(&ethpb.BeaconBlockElectra{Body: body})
			require.NoError(t, err)
			...
			if postSSZExists {
				...
				pbState, err := state_native.ProtobufBeaconStateElectra(preBeaconState.ToProto())
				require.NoError(t, err)
