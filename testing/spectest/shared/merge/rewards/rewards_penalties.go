...
func RunPrecomputeRewardsAndPenaltiesTests(t *testing.T, config string) {
	...
	_, testsFolderPath := utils.TestFolders(t, config, "merge", "rewards")
	testTypes, err := util.BazelListDirectories(testsFolderPath)
	require.NoError(t, err)

	for _, testType := range testTypes {
		testFolders, testsFolderPath := utils.TestFolders(t, config, "merge", fmt.Sprintf("rewards/%s/pyspec_tests", testType))
