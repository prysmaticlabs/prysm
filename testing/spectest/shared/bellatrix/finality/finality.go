...
func RunFinalityTest(t *testing.T, config string) {
	...
	for _, folder := range testFolders {
		t.Run(folder.Name(), func(t *testing.T) {
			...
			beaconState, err := state.InitializeFromProtoBellatrix(beaconStateBase)
			require.NoError(t, err)
			...
			var processedState types.BeaconState
			var ok bool
			for i := 0; i < metaYaml.BlocksCount; i++ {
				...
				require.NoError(t, err)
				beaconState, ok = processedState.(*state.State)
			...
			pbState, err := state.ProtobufBeaconStateBellatrix(beaconState.ToProtoUnsafe())
			require.NoError(t, err)
