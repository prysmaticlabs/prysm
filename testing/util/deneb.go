...
func GenerateTestDenebBlockWithColumns(t *testing.T, parent [32]byte, slot primitives.Slot, nblobs int, opts ...DenebBlockGeneratorOption) (blocks.ROBlock, []blocks.RODataColumn) {
	g := &denebBlockGenerator{
		parent: parent,
		slot:   slot,
		nblobs: nblobs,
	...
	for i, c := range block.Block.Body.BlobKzgCommitments {
		sidecars[i] = GenerateTestDenebBlobSidecar(t, root, sh, i, c, inclusion[i])
		blobs[i] = kzg.Blob(sidecars[i].BlobSidecar.Blob)
	}

	rob, err := blocks.NewROBlock(sbb)
	require.NoError(t, err)

	columns, err := peerdas.DataColumnSidecars(rob, blobs)
	require.NoError(t, err)
	roColumns := make([]blocks.RODataColumn, len(columns))
	for i, c := range columns {
		roCol, err := blocks.NewRODataColumn(c)
		require.NoError(t, err)
		roColumns[i] = roCol
	}

	return rob, roColumns
}
