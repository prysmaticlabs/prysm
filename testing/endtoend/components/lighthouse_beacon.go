...
func (node *LighthouseBeaconNode) Start(ctx context.Context) error {
	...
	args := []string{
		...
		"--staking",
		"--bootstrapEnr-address=127.0.0.1",
		fmt.Sprintf("--bootstrapEnr-udp-port=%d", e2e.TestParams.BeaconNodeRPCPort+index+e2e.LighthouseP2PPortOffset),
		fmt.Sprintf("--bootstrapEnr-tcp-port=%d", e2e.TestParams.BeaconNodeRPCPort+index+e2e.LighthouseP2PPortOffset),
