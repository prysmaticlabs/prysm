...
func orderedEvaluationOnResponses(beaconPathsAndObjects map[string]metadata, genesisData *v1.GenesisResponse) error {
	...
	prysmForkData, ok := forkPathData.prysmResps["json"].(*beacon.StateForkResponse)
	if !ok {
		return errors.New("failed to cast type")
	}
	lighthouseForkData, ok := forkPathData.lighthouseResps["json"].(*beacon.StateForkResponse)
	if !ok {
		return errors.New("failed to cast type")
	}
