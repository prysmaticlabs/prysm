...
// protection in the validator client's database.
func importStandardProtectionJSONMinimal(ctx context.Context, validatorDB db.Database, r io.Reader) error {
	// Read the JSON file
	encodedJSON, err := io.ReadAll(r)
	if err != nil {
		return errors.Wrap(err, "could not read slashing protection JSON file")
	...
	for _, item := range interchangeJSON.Data {
		...
		// Block proposals
		if err := importBlockProposals(ctx, pubkey, item, validatorDB); err != nil {
			return errors.Wrap(err, "could not import block proposals")
		}

		// Attestations
		if err := importAttestations(ctx, pubkey, item, validatorDB); err != nil {
...
func importBlockProposals(ctx context.Context, pubkey [fieldparams.BLSPubkeyLength]byte, item *format.ProtectionData, validatorDB db.Database) error {
...
func importAttestations(ctx context.Context, pubkey [fieldparams.BLSPubkeyLength]byte, item *format.ProtectionData, validatorDB db.Database) error {
...
func saveProposals(ctx context.Context, proposalHistoryByPubKey map[[fieldparams.BLSPubkeyLength]byte]common.ProposalHistoryForPubkey, validatorDB db.Database) error {
...
func saveAttestations(ctx context.Context, attestingHistoryByPubKey map[[fieldparams.BLSPubkeyLength]byte][]*common.AttestationRecord, validatorDB db.Database) error {
	bar := initializeProgressBar(
