...
func (db *ValidatorDB) SaveProposedBlock(fork *pbp2p.Fork, pubKey *bls.PublicKey, block *pbp2p.BeaconBlock) error {
	...
	return db.update(func(tx *bolt.Tx) error {
		bucket := getBucket(tx, pubKey, forkVersion, proposedBlockBucket)
...
func (db *ValidatorDB) getMaxProposedEpoch(pubKey *bls.PublicKey) (maxProposedEpoch uint64, err error) {
	err = db.lastInAllForks(pubKey, proposedBlockBucket, func(lastInForkEnc []byte) error {
		...
		epoch := lastInFork.Slot / params.BeaconConfig().SlotsPerEpoch
		if epoch > maxProposedEpoch {
			maxProposedEpoch = epoch
		}
		return nil
	})
	return
