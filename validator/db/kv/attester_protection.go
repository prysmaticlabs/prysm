	...
	err := s.view(func(tx *bolt.Tx) error {
		...
		return sourceEpochsBucket.ForEach(func(sourceBytes, targetEpochsList []byte) error {
			...
			for i := 0; i < len(targetEpochsList); i += 8 {
				epoch := butil.BytesToEpochBigEndian(targetEpochsList[i : i+8])
				targetEpochs = append(targetEpochs, epoch)
			}
			sourceEpoch := butil.BytesToEpochBigEndian(sourceBytes)
			for _, targetEpoch := range targetEpochs {
				...
				signingRoot := signingRootsBucket.Get(butil.EpochToBytesBigEndian(targetEpoch))
				if signingRoot != nil {
...
func (s *Store) CheckSlashableAttestation(
	...
	err := s.view(func(tx *bolt.Tx) error {
		...
		if signingRootsBucket != nil {
			targetEpochBytes := butil.EpochToBytesBigEndian(att.Data.Target.Epoch)
...
func (s *Store) checkSurroundedVote(
	...
	for k, v := c.Last(); k != nil; k, v = c.Prev() {
		existingTargetEpoch := butil.BytesToEpochBigEndian(k)
		...
		for i := 0; i < len(v); i += 8 {
			sourceEpoch := butil.BytesToEpochBigEndian(v[i : i+8])
...
func (s *Store) checkSurroundingVote(
	...
	for k, v := c.Last(); k != nil; k, v = c.Prev() {
		existingSourceEpoch := butil.BytesToEpochBigEndian(k)
		...
		for i := 0; i < len(v); i += 8 {
			targetEpoch := butil.BytesToEpochBigEndian(v[i : i+8])
...
func (s *Store) saveAttestationRecords(ctx context.Context, atts []*AttestationRecord) error {
	...
	return s.update(func(tx *bolt.Tx) error {
		...
		for _, att := range atts {
			...
			sourceEpochBytes := butil.EpochToBytesBigEndian(att.Source)
			targetEpochBytes := butil.EpochToBytesBigEndian(att.Target)
			...
			if len(lowestSignedSourceBytes) >= 8 {
				lowestSignedSourceEpoch = butil.BytesToEpochBigEndian(lowestSignedSourceBytes)
			}
			if len(lowestSignedSourceBytes) == 0 || att.Source < lowestSignedSourceEpoch {
				if err := lowestSourceBucket.Put(
					att.PubKey[:], butil.EpochToBytesBigEndian(att.Source),
				); err != nil {
					return err
				}
			}
			...
			if len(lowestSignedTargetBytes) >= 8 {
				lowestSignedTargetEpoch = butil.BytesToEpochBigEndian(lowestSignedTargetBytes)
			}
			if len(lowestSignedTargetBytes) == 0 || att.Target < lowestSignedTargetEpoch {
				if err := lowestTargetBucket.Put(
					att.PubKey[:], butil.EpochToBytesBigEndian(att.Target),
				); err != nil {
					return err
				}
			}
...
func (s *Store) SigningRootAtTargetEpoch(ctx context.Context, pubKey [48]byte, target types.Epoch) ([32]byte, error) {
	...
	err := s.view(func(tx *bolt.Tx) error {
		...
		sr := signingRootsBucket.Get(butil.EpochToBytesBigEndian(target))
		copy(signingRoot[:], sr)
		return nil
	})
