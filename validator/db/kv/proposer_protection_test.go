...
func TestSaveProposalHistoryForSlot_OK(t *testing.T) {
	...
	require.DeepEqual(t, butil.PadTo([]byte{1}, 32), signingRoot[:], "Expected DB to keep object the same")
}
...
func TestSaveProposalHistoryForSlot_Overwrites(t *testing.T) {
	...
	tests := []struct {
		...
		{
			signingRoot: butil.PadTo([]byte{1}, 32),
		},
		{
			signingRoot: butil.PadTo([]byte{2}, 32),
		},
		{
			signingRoot: butil.PadTo([]byte{3}, 32),
		},
	}
...
func TestPruneProposalHistoryBySlot_OK(t *testing.T) {
	...
	signedRoot := butil.PadTo([]byte{1}, 32)
