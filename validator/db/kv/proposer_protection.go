...
func (s *Store) ProposalHistoryForSlot(ctx context.Context, publicKey [48]byte, slot types.Slot) ([32]byte, bool, error) {
	...
	err = s.view(func(tx *bolt.Tx) error {
		...
		signingRootBytes := valBucket.Get(butil.SlotToBytesBigEndian(slot))
...
func (s *Store) ProposalHistoryForPubKey(ctx context.Context, publicKey [48]byte) ([]*Proposal, error) {
	...
	err := s.view(func(tx *bolt.Tx) error {
		...
		return valBucket.ForEach(func(slotKey, signingRootBytes []byte) error {
			slot := butil.BytesToSlotBigEndian(slotKey)
			sr := make([]byte, 32)
...
func (s *Store) SaveProposalHistoryForSlot(ctx context.Context, pubKey [48]byte, slot types.Slot, signingRoot []byte) error {
	...
	err := s.update(func(tx *bolt.Tx) error {
		...
		if len(lowestSignedProposalBytes) >= 8 {
			lowestSignedProposalSlot = butil.BytesToSlotBigEndian(lowestSignedProposalBytes)
		}
		if len(lowestSignedProposalBytes) == 0 || slot < lowestSignedProposalSlot {
			if err := lowestSignedBkt.Put(pubKey[:], butil.SlotToBytesBigEndian(slot)); err != nil {
				return err
			}
		}
		...
		if len(highestSignedProposalBytes) >= 8 {
			highestSignedProposalSlot = butil.BytesToSlotBigEndian(highestSignedProposalBytes)
		}
		if len(highestSignedProposalBytes) == 0 || slot > highestSignedProposalSlot {
			if err := highestSignedBkt.Put(pubKey[:], butil.SlotToBytesBigEndian(slot)); err != nil {
				return err
			}
		}

		if err := valBucket.Put(butil.SlotToBytesBigEndian(slot), signingRoot); err != nil {
			return err
		}
...
func pruneProposalHistoryBySlot(valBucket *bolt.Bucket, newestSlot types.Slot) error {
	...
	for k, _ := c.First(); k != nil; k, _ = c.First() {
		slot := butil.BytesToSlotBigEndian(k)
