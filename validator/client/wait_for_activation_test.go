...
func TestWaitActivation_StreamSetupFails_AttemptsToReconnect(t *testing.T) {
	...
	v.pubkeyHexToValidatorIndex[hexutil.Encode(pubKey[:])] = 1
	clientStream := mock.NewMockBeaconNodeValidator_WaitForActivationClient(ctrl)
	client.EXPECT().WaitForActivation(
		gomock.Any(),
		&ethpb.ValidatorActivationRequest{
...
func TestWaitForActivation_AccountsChanged(t *testing.T) {
	...
	t.Run("Imported keymanager", func(t *testing.T) {
		...
		v.pubkeyHexToValidatorIndex[hexutil.Encode(inactivePubKey[:])] = 1
		...
		go func() {
			...
			v.pubkeyHexToValidatorIndex[hexutil.Encode(activePubKey[:])] = 1
	...
	t.Run("Derived keymanager", func(t *testing.T) {
		...
		go func() {
			...
			for _, key := range keys {
				v.pubkeyHexToValidatorIndex[hexutil.Encode(key[:])] = 1
			}
...
func TestWaitForActivation_RemoteKeymanager(t *testing.T) {
	...
	t.Run("activated", func(t *testing.T) {
		...
		v.pubkeyHexToValidatorIndex[hexutil.Encode(activeKey[:])] = 1
		v.pubkeyHexToValidatorIndex[hexutil.Encode(inactiveKey[:])] = 2
		go func() {
			tickerChan <- slot
