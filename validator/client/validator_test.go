...
func genMockKeymanager(numKeys int) *mockKeymanager {
	...
	for i := 0; i < numKeys; i++ {
		...
		km[bytes.ToBytes48(k.PublicKey().Marshal())] = k
	}
...
func TestWaitForChainStart_SetsGenesisInfo_IncorrectSecondTry(t *testing.T) {
	...
	genesisValidatorsRoot = bytes.ToBytes32([]byte("badvalidators"))
...
func TestWaitForChainStart_ContextCanceled(t *testing.T) {
	...
	genesisValidatorsRoot := bytes.PadTo([]byte("validators"), 32)
	clientStream := mock.NewMockBeaconNodeValidator_WaitForChainStartClient(ctrl)
	client.EXPECT().WaitForChainStart(
		gomock.Any(),
		&emptypb.Empty{},
...
func TestRolesAt_OK(t *testing.T) {
	...
	assert.Equal(t, iface.RoleAttester, roleMap[bytes.ToBytes48(validatorKey.PublicKey().Marshal())][0])
	assert.Equal(t, iface.RoleAggregator, roleMap[bytes.ToBytes48(validatorKey.PublicKey().Marshal())][1])
	assert.Equal(t, iface.RoleSyncCommittee, roleMap[bytes.ToBytes48(validatorKey.PublicKey().Marshal())][2])
	...
	require.NoError(t, err)
	assert.Equal(t, iface.RoleSyncCommittee, roleMap[bytes.ToBytes48(validatorKey.PublicKey().Marshal())][0])
}
...
func TestCheckAndLogValidatorStatus_OK(t *testing.T) {
	...
	pubKeys := [][]byte{bytes.Uint64ToBytesLittleEndian(0)}
	tests := []statusTest{
		{
...
func TestIsSyncCommitteeAggregator_OK(t *testing.T) {
	...
	aggregator, err := v.isSyncCommitteeAggregator(context.Background(), slot, bytes.ToBytes48(pubKey))
	require.NoError(t, err)
