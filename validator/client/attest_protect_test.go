...
func Test_slashableAttestationCheck(t *testing.T) {
	...
	att := &ethpb.IndexedAttestation{
		...
		Data: &ethpb.AttestationData{
			Slot:            5,
			CommitteeIndex:  2,
			BeaconBlockRoot: bytes.PadTo([]byte("great block"), 32),
			Source: &ethpb.Checkpoint{
				Epoch: 4,
				Root:  bytes.PadTo([]byte("good source"), 32),
			},
			Target: &ethpb.Checkpoint{
				Epoch: 10,
				Root:  bytes.PadTo([]byte("good target"), 32),
			},
		},
	}
...
func Test_slashableAttestationCheck_OK(t *testing.T) {
	...
	sr := [32]byte{1}
	fakePubkey := bytes.ToBytes48([]byte("test"))
...
func Test_slashableAttestationCheck_GenesisEpoch(t *testing.T) {
	...
	att := &ethpb.IndexedAttestation{
		...
		Data: &ethpb.AttestationData{
			Slot:            5,
			CommitteeIndex:  2,
			BeaconBlockRoot: bytes.PadTo([]byte("great block root"), 32),
			Source: &ethpb.Checkpoint{
				Epoch: 0,
				Root:  bytes.PadTo([]byte("great root"), 32),
			},
			Target: &ethpb.Checkpoint{
				Epoch: 0,
				Root:  bytes.PadTo([]byte("great root"), 32),
