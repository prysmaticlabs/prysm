...
func TestPreBlockSignLocalValidation_PreventsLowerThanMinProposal(t *testing.T) {
	...
	err = validator.preBlockSignValidations(context.Background(), pubKeyBytes, interfaces.WrappedPhase0BeaconBlock(block), [32]byte{4})
	...
	err = validator.preBlockSignValidations(context.Background(), pubKeyBytes, interfaces.WrappedPhase0BeaconBlock(block), [32]byte{1})
	require.NoError(t, err)
	...
	err = validator.preBlockSignValidations(context.Background(), pubKeyBytes, interfaces.WrappedPhase0BeaconBlock(block), [32]byte{3})
	require.NoError(t, err)
}
...
func TestPreBlockSignLocalValidation(t *testing.T) {
	...
	err = validator.preBlockSignValidations(context.Background(), pubKey, interfaces.WrappedPhase0BeaconBlock(block), dummySigningRoot)
	require.NoError(t, err)

	// We expect the same block sent out with a different signing root should be slasahble.
	err = validator.preBlockSignValidations(context.Background(), pubKey, interfaces.WrappedPhase0BeaconBlock(block), [32]byte{2})
	require.ErrorContains(t, failedPreBlockSignLocalErr, err)
	...
	block.Slot = 9
	err = validator.preBlockSignValidations(context.Background(), pubKey, interfaces.WrappedPhase0BeaconBlock(block), [32]byte{3})
...
func TestPreBlockSignValidation(t *testing.T) {
	...
	err := validator.preBlockSignValidations(context.Background(), pubKey, interfaces.WrappedPhase0BeaconBlock(block.Block), [32]byte{2})
...
func TestPostBlockSignUpdate(t *testing.T) {
	...
	mockProtector := &mockSlasher.MockProtector{AllowBlock: false}
	validator.protector = mockProtector
	err := validator.postBlockSignUpdate(context.Background(), pubKey, interfaces.WrappedPhase0SignedBeaconBlock(emptyBlock), [32]byte{})
