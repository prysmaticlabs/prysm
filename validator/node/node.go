	...
	if fileConfig.ProposeConfig != nil {
		...
		for key, option := range fileConfig.ProposeConfig {
			...
			if !mixedcaseAddress.ValidChecksum() {
				log.Warnf("Fee recipient %s is not a checksum Ethereum address. The checksummed address is %s and will be used as the fee recipient. We recommend using a mixed-case address (checksum) to prevent spelling mistakes in your fee recipient Ethereum address", option.FeeRecipient, checksumAddress.Hex())
			}
