...
func processSlashingsWrapper(t *testing.T, state interfaces.BeaconState) (interfaces.BeaconState, error) {
	state, err := epoch.ProcessSlashings(state)
	require.NoError(t, err, "Could not process slashings")
	return state, nil
}

func processSlashingsPrecomputeWrapper(t *testing.T, state interfaces.BeaconState) (interfaces.BeaconState, error) {
