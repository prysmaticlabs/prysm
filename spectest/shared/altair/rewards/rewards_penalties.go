...
func RunPrecomputeRewardsAndPenaltiesTests(t *testing.T, config string) {
	...
	testTypes, err := customtesting.BazelListDirectories(testsFolderPath)
...
func runPrecomputeRewardsAndPenaltiesTest(t *testing.T, testFolderPath string) {
	...
	testfiles, err := customtesting.BazelListFiles(path.Join(testFolderPath))
	require.NoError(t, err)
	...
	for _, dFile := range deltaFiles {
		sourceFile, err := customtesting.BazelFileBytes(path.Join(testFolderPath, dFile))
		require.NoError(t, err)
