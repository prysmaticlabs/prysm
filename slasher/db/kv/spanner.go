...
// and error in case of db error. returns empty map if the span map
// for this validator index does not exist.
func (db *Store) EpochSpans(ctx context.Context, epoch uint64) ([]byte, uint64, bool, error) {
	...
	var spans []byte
	err = db.view(func(tx *bolt.Tx) error {
		b := tx.Bucket(validatorsMinMaxSpanBucketPOC)
		if b == nil {
			return nil
		}
	...
	spansLength := uint64(0)
	if spans != nil {
		spansLength = uint64(len(spans) / spannerEncodedLength)
	}
	return spans, spansLength, false, err
}
...
func (db *Store) SaveEpochSpans(ctx context.Context, epoch uint64, spans []byte) error {
	...
	return db.update(func(tx *bolt.Tx) error {
		b, err := tx.CreateBucketIfNotExists(validatorsMinMaxSpanBucketPOC)
		if err != nil {
			return err
		}
