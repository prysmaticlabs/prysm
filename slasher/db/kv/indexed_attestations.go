...
// AttKeysFromValIDForEpoch accepts an epoch and validator id and returns all attestation keys from
// that validator in the requested epoch.
func (db *Store) AttKeysFromValIDForEpoch(ctx context.Context, targetEpoch uint64, validatorID uint64) ([][]byte, error) {
	ctx, span := trace.StartSpan(ctx, "SlasherDB.AttKeysFromValIDForEpoch")
	defer span.End()
	key := encodeEpochValidatorID(targetEpoch, validatorID)
	var attKeys [][]byte
	var err error
	...
	err = db.view(func(tx *bolt.Tx) error {
		...
		attKeys, err = unmarshalIdxAttKeys(ctx, enc)
		if err != nil {
			return err
		}
		return nil
...
func (db *Store) SaveIndexedAttestation(ctx context.Context, idxAttestation *ethpb.IndexedAttestation) error {
	...
	err = db.update(func(tx *bolt.Tx) error {
		...
		if err := addAttKeyToEpochValIDList(ctx, idxAttestation, tx); err != nil {
...
func (db *Store) DeleteIndexedAttestation(ctx context.Context, idxAttestation *ethpb.IndexedAttestation) error {
	...
	return db.update(func(tx *bolt.Tx) error {
		...
		if err := removeAttKeyFromEpochValIDList(ctx, idxAttestation, tx); err != nil {
			return err
		}
