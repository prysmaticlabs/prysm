...
func Scatter(inputLen int, sFunc func(int, int) (*ScatterResults, error)) (int, chan *ScatterResults, chan error, error) {
	if inputLen <= 0 {
		return 0, nil, nil, errors.New("input length must be greater than 0")
	}
	...
	outputCh := make(chan *ScatterResults, workers)
	...
	return workers, outputCh, errCh, nil
}
