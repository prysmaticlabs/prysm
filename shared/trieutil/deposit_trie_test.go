...
func TestDepositTrie_UpdateDeposit(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if d.trie.DepositCount != 1 {
			t.Errorf("Expected deposit count to increase by 1, received %d", d.trie.DepositCount)
		}
...
func TestVerifyMerkleBranch_OK(t *testing.T) {
	...
	index := make([]byte, 8)
	binary.LittleEndian.PutUint64(index, d.trie.DepositCount-1)
	branch := d.Branch()
	root := d.Root()
