...
func TestBitlistRoot(t *testing.T) {
	...
	if expected != result {
		t.Errorf("Expected slice to be %d, received %d", expected, result)
	}
}
...
func TestPack(t *testing.T) {
	...
	if len(expected) != len(result[0]) {
		t.Errorf("Expected slice length to be %d, received %d", len(expected), len(result[0]))
	}
	for i, v := range expected {
		if v != result[0][i] {
			t.Errorf("Expected value at slice index %d was %d, received %d", i, expected[i], result[0][i])
		}
	}
}
