...
func TestFromBytes2(t *testing.T) {
	...
	for _, tt := range tests {
		b := bytes.ToBytes(tt, 2)
		c := bytes.FromBytes2(b)
...
func TestFromBytes4(t *testing.T) {
	...
	for _, tt := range tests {
		b := bytes.ToBytes(tt, 4)
		c := bytes.FromBytes4(b)
		if c != tt {
...
func TestFromBytes8(t *testing.T) {
	...
	for _, tt := range tests {
		b := bytes.ToBytes(tt, 8)
		c := bytes.FromBytes8(b)
		assert.Equal(t, tt, c)
	}
}
...
func TestUint64ToBytes_RoundTrip(t *testing.T) {
	for i := uint64(0); i < 10000; i++ {
		b := bytes.Uint64ToBytesBigEndian(i)
...
func TestSafeCopyRootAtIndex(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input: [][]byte{{0x1}, {0x2}},
			idx:   1,
			want:  bytes.PadTo([]byte{0x2}, 32),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := bytes.SafeCopyRootAtIndex(tt.input, tt.idx)
...
func TestSafeCopy2dBytes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := bytes.SafeCopy2dBytes(tt.input); !reflect.DeepEqual(got, tt.input) {
