...
func TestVotes_CanFindHead(t *testing.T) {
	...
	//             |
	//             4 <- head
	state, err = setupInsertParameters(context.Background(), 0, indexToHash(4), indexToHash(3), params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	...
	//            /
	//           5 <- justified epoch = 2
	state, err = setupInsertParameters(context.Background(), 0, indexToHash(5), indexToHash(4), params.BeaconConfig().ZeroHash, 2, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	...
	state, err = setupInsertParameters(context.Background(), 0, indexToHash(6), indexToHash(4), params.BeaconConfig().ZeroHash, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	...
	//           |
	//           9
	state, err = setupInsertParameters(context.Background(), 0, indexToHash(7), indexToHash(5), params.BeaconConfig().ZeroHash, 2, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	state, err = setupInsertParameters(context.Background(), 0, indexToHash(8), indexToHash(7), params.BeaconConfig().ZeroHash, 2, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	state, err = setupInsertParameters(context.Background(), 0, indexToHash(9), indexToHash(8), params.BeaconConfig().ZeroHash, 2, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	...
	state, err = setupInsertParameters(context.Background(), 0, indexToHash(10), indexToHash(8), params.BeaconConfig().ZeroHash, 2, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	...
	//        |
	// head-> 11
	state, err = setupInsertParameters(context.Background(), 0, indexToHash(11), indexToHash(9), params.BeaconConfig().ZeroHash, 2, 2)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
