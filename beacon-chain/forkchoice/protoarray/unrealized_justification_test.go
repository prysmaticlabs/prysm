...
func TestStore_LongFork(t *testing.T) {
	...
	hr := [32]byte{'d'}
	state, err = setupInsertParameters(ctx, 103, hr, [32]byte{'b'}, [32]byte{'D'}, 2, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
...
func TestStore_NoDeadLock(t *testing.T) {
	...
	// Epoch 1 blocks
	state, err := setupInsertParameters(ctx, 100, [32]byte{'a'}, params.BeaconConfig().ZeroHash, [32]byte{'A'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	state, err = setupInsertParameters(ctx, 101, [32]byte{'b'}, [32]byte{'a'}, [32]byte{'B'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	state, err = setupInsertParameters(ctx, 102, [32]byte{'c'}, [32]byte{'b'}, [32]byte{'C'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	state, err = setupInsertParameters(ctx, 103, [32]byte{'d'}, [32]byte{'c'}, [32]byte{'D'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))

	// Epoch 2 Blocks
	state, err = setupInsertParameters(ctx, 104, [32]byte{'e'}, [32]byte{'d'}, [32]byte{'E'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	require.NoError(t, f.store.setUnrealizedJustifiedEpoch([32]byte{'e'}, 1))
	state, err = setupInsertParameters(ctx, 105, [32]byte{'f'}, [32]byte{'e'}, [32]byte{'F'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	require.NoError(t, f.store.setUnrealizedJustifiedEpoch([32]byte{'f'}, 1))
	state, err = setupInsertParameters(ctx, 106, [32]byte{'g'}, [32]byte{'f'}, [32]byte{'G'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	...
	state, err = setupInsertParameters(ctx, 107, [32]byte{'h'}, [32]byte{'g'}, [32]byte{'H'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	...
	hr := [32]byte{'i'}
	state, err = setupInsertParameters(ctx, 108, hr, [32]byte{'f'}, [32]byte{'I'}, 1, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
...
func TestStore_ForkNextEpoch(t *testing.T) {
	...
	// Epoch 2 blocks
	state, err = setupInsertParameters(ctx, 104, [32]byte{'e'}, [32]byte{'c'}, [32]byte{'E'}, 0, 0)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	state, err = setupInsertParameters(ctx, 105, [32]byte{'f'}, [32]byte{'e'}, [32]byte{'F'}, 0, 0)
	require.NoError(t, err)
