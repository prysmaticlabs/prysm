...
func TestForkChoice_BoostProposerRoot_SlotSchedule(t *testing.T) {
	...
	ctx := t.Context()
	zeroHash := params.BeaconConfig().ZeroHash
	// Slot 33 is in epoch 1 where slots last 6s with a 1.5s attestation deadline instead of ~4s.
	...
	t.Run("not boosted past the shortened threshold", func(t *testing.T) {
		f := setup(0, 0)
		// 3s into the slot is within the 12s era threshold but past the 6s era one.
