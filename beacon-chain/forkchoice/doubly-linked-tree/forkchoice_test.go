...
func TestForkChoice_IsCanonical(t *testing.T) {
	...
	isCanonical, err := f.IsCanonical(params.BeaconConfig().ZeroHash)
	require.NoError(t, err)
	require.Equal(t, true, isCanonical)
	isCanonical, err = f.IsCanonical(indexToHash(1))
	require.NoError(t, err)
	require.Equal(t, false, isCanonical)
	isCanonical, err = f.IsCanonical(indexToHash(2))
	require.NoError(t, err)
	require.Equal(t, true, isCanonical)
	isCanonical, err = f.IsCanonical(indexToHash(3))
	require.NoError(t, err)
	require.Equal(t, false, isCanonical)
	isCanonical, err = f.IsCanonical(indexToHash(4))
	require.NoError(t, err)
	require.Equal(t, true, isCanonical)
	isCanonical, err = f.IsCanonical(indexToHash(5))
	require.NoError(t, err)
	require.Equal(t, true, isCanonical)
	isCanonical, err = f.IsCanonical(indexToHash(6))
	require.NoError(t, err)
	require.Equal(t, true, isCanonical)
}
...
func TestForkChoice_IsCanonicalReorg(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, true, isCanonical)
	isCanonical, err = f.IsCanonical([32]byte{'1'})
	require.NoError(t, err)
	require.Equal(t, true, isCanonical)
	isCanonical, err = f.IsCanonical([32]byte{'2'})
	require.NoError(t, err)
	require.Equal(t, false, isCanonical)
	isCanonical, err = f.IsCanonical([32]byte{'3'})
	require.NoError(t, err)
	require.Equal(t, true, isCanonical)
	isCanonical, err = f.IsCanonical([32]byte{'4'})
	require.NoError(t, err)
	require.Equal(t, false, isCanonical)
	isCanonical, err = f.IsCanonical([32]byte{'5'})
	require.NoError(t, err)
	require.Equal(t, false, isCanonical)
	isCanonical, err = f.IsCanonical([32]byte{'6'})
	require.NoError(t, err)
	require.Equal(t, false, isCanonical)
	_, err = f.IsCanonical([32]byte{'p'})
