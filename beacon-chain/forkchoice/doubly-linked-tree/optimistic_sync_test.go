...
func TestPruneInvalid(t *testing.T) {
	...
	for _, tc := range tests {
		...
		state, err := setupInsertParameters(ctx, 100, [32]byte{'a'}, params.BeaconConfig().ZeroHash, [32]byte{'A'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 101, [32]byte{'b'}, [32]byte{'a'}, [32]byte{'B'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 102, [32]byte{'c'}, [32]byte{'b'}, [32]byte{'C'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 102, [32]byte{'j'}, [32]byte{'b'}, [32]byte{'J'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 103, [32]byte{'d'}, [32]byte{'c'}, [32]byte{'D'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 104, [32]byte{'e'}, [32]byte{'d'}, [32]byte{'E'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 104, [32]byte{'g'}, [32]byte{'d'}, [32]byte{'G'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 105, [32]byte{'f'}, [32]byte{'e'}, [32]byte{'F'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 105, [32]byte{'h'}, [32]byte{'g'}, [32]byte{'H'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 105, [32]byte{'k'}, [32]byte{'g'}, [32]byte{'K'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 106, [32]byte{'i'}, [32]byte{'h'}, [32]byte{'I'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
		state, err = setupInsertParameters(ctx, 106, [32]byte{'l'}, [32]byte{'k'}, [32]byte{'L'}, 1, 1)
		require.NoError(t, err)
		require.NoError(t, f.InsertOptimisticBlock(ctx, state))
...
func TestSetOptimisticToInvalid_ProposerBoost(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	state, err = setupInsertParameters(ctx, 101, [32]byte{'c'}, [32]byte{'b'}, [32]byte{'C'}, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
...
func TestSetOptimisticToInvalid_CorrectChildren(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	state, err = setupInsertParameters(ctx, 102, [32]byte{'c'}, [32]byte{'a'}, [32]byte{'C'}, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
	state, err = setupInsertParameters(ctx, 103, [32]byte{'d'}, [32]byte{'a'}, [32]byte{'D'}, 1, 1)
	require.NoError(t, err)
	require.NoError(t, f.InsertOptimisticBlock(ctx, state))
