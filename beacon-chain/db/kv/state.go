...
func (s *Store) State(ctx context.Context, blockRoot [32]byte) (interfaces.BeaconState, error) {
...
func (s *Store) GenesisState(ctx context.Context) (interfaces.BeaconState, error) {
...
func (s *Store) SaveState(ctx context.Context, st interfaces.ReadOnlyBeaconState, blockRoot [32]byte) error {
	ctx, span := trace.StartSpan(ctx, "BeaconDB.SaveState")
	defer span.End()

	return s.SaveStates(ctx, []interfaces.ReadOnlyBeaconState{st}, [][32]byte{blockRoot})
}

// SaveStates stores multiple states to the db using the provided corresponding roots.
func (s *Store) SaveStates(ctx context.Context, states []interfaces.ReadOnlyBeaconState, blockRoots [][32]byte) error {
...
func (s *Store) HighestSlotStatesBelow(ctx context.Context, slot types.Slot) ([]interfaces.ReadOnlyBeaconState, error) {
	...
	var st interfaces.ReadOnlyBeaconState
	var err error
	if best != nil {
	...
	return []interfaces.ReadOnlyBeaconState{st}, nil
}
