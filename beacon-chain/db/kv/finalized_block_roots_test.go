...
var genesisBlockRoot = butil.ToBytes32([]byte{'G', 'E', 'N', 'E', 'S', 'I', 'S'})
...
func TestStore_IsFinalized_ForkEdgeCase(t *testing.T) {
	...
	blocks1 := append(
		makeBlocks(t, slotsPerEpoch*1, 1, butil.ToBytes32(sszRootOrDie(t, blocks0[len(blocks0)-1]))), // No block builds off of the first block in epoch.
		makeBlocks(t, slotsPerEpoch*1+1, slotsPerEpoch-1, butil.ToBytes32(sszRootOrDie(t, blocks0[len(blocks0)-1])))...,
	)
	blocks2 := makeBlocks(t, slotsPerEpoch*2, slotsPerEpoch, butil.ToBytes32(sszRootOrDie(t, blocks1[len(blocks1)-1])))
	...
	require.NoError(t, err)
	// A state is required to save checkpoint
	require.NoError(t, db.SaveState(ctx, st, butil.ToBytes32(checkpoint1.Root)))
	...
	for i, block := range append(blocks0, blocks1...) {
		...
		assert.Equal(t, true, db.IsFinalizedBlock(ctx, butil.ToBytes32(root)), "%d - Expected block %#x to be finalized", i, root)
	}
	...
	// A state is required to save checkpoint
	require.NoError(t, db.SaveState(ctx, st, butil.ToBytes32(checkpoint2.Root)))
	...
	for i, block := range blocks1 {
		root := sszRootOrDie(t, block)
		if db.IsFinalizedBlock(ctx, butil.ToBytes32(root)) == (i == 0) {
