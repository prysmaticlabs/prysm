	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(t *testing.T, db *bbolt.DB) {
				err := db.Update(func(tx *bbolt.Tx) error {
					...
					if err := tx.Bucket(archivedRootBucket).Put(bytes.Uint64ToBytesLittleEndian(2048), []byte("foo")); err != nil {
						return err
					}
			...
			eval: func(t *testing.T, db *bbolt.DB) {
				err := db.View(func(tx *bbolt.Tx) error {
					v := tx.Bucket(archivedRootBucket).Get(bytes.Uint64ToBytesLittleEndian(2048))
		...
		},
		{
			...
			eval: func(t *testing.T, db *bbolt.DB) {
				err := db.View(func(tx *bbolt.Tx) error {
					k := uint64(2048)
					v := tx.Bucket(stateSlotIndicesBucket).Get(bytes.Uint64ToBytesBigEndian(k))
