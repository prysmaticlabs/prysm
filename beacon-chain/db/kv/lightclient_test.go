...
func TestStore_LightClientUpdate_CanSaveRetrieveAltair(t *testing.T) {
	...
	update, err := createDefaultLightClientUpdate(primitives.Slot(config.AltairForkEpoch * primitives.Epoch(config.SlotsPerEpoch)).Add(1))
	require.NoError(t, err)
	update.SetSignatureSlot(7)
	parentRoot := make([]byte, 32)
	for i := 0; i < 32; i++ {
		parentRoot[i] = byte(i)
	}
	simpleHeader, err := light_client.NewWrappedHeader(&pb.LightClientHeaderAltair{
	...
	require.NoError(t, err)
	update.SetAttestedHeader(simpleHeader)
	update.SetFinalizedHeader(simpleHeader)
