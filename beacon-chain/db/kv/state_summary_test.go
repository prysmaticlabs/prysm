...
func TestStateSummary_CanSaveRetrieve(t *testing.T) {
	...
	r1 := butil.ToBytes32([]byte{'A'})
	r2 := butil.ToBytes32([]byte{'B'})
...
func TestStateSummary_CacheToDB(t *testing.T) {
	...
	for i := range summaries {
		summaries[i] = &ethpb.StateSummary{Slot: types.Slot(i), Root: butil.PadTo(butil.Uint64ToBytesLittleEndian(uint64(i)), 32)}
	}
	...
	for i := range summaries {
		r := butil.Uint64ToBytesLittleEndian(uint64(i))
		require.Equal(t, true, db.HasStateSummary(context.Background(), butil.ToBytes32(r)))
	}
}
