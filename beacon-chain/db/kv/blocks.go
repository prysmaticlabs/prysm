...
func (s *Store) Block(ctx context.Context, blockRoot [32]byte) (block2.SignedBeaconBlock, error) {
	ctx, span := trace.StartSpan(ctx, "BeaconDB.Block")
	defer span.End()
	// Return block from cache if it exists.
	if v, ok := s.blockCache.Get(string(blockRoot[:])); v != nil && ok {
		return v.(block2.SignedBeaconBlock), nil
	}
...
func (s *Store) HeadBlock(ctx context.Context) (block2.SignedBeaconBlock, error) {
...
func (s *Store) Blocks(ctx context.Context, f *filters.QueryFilter) ([]block2.SignedBeaconBlock, [][32]byte, error) {
	ctx, span := trace.StartSpan(ctx, "BeaconDB.Blocks")
	defer span.End()
	blocks := make([]block2.SignedBeaconBlock, 0)
...
func (s *Store) BlocksBySlot(ctx context.Context, slot types.Slot) (bool, []block2.SignedBeaconBlock, error) {
...
// SaveBlock to the db.
func (s *Store) SaveBlock(ctx context.Context, signed block2.SignedBeaconBlock) error {
	...
	return s.SaveBlocks(ctx, []block2.SignedBeaconBlock{signed})
}

// SaveBlocks via bulk updates to the db.
func (s *Store) SaveBlocks(ctx context.Context, blocks []block2.SignedBeaconBlock) error {
...
func (s *Store) GenesisBlock(ctx context.Context) (block2.SignedBeaconBlock, error) {
...
func (s *Store) HighestSlotBlocksBelow(ctx context.Context, slot types.Slot) ([]block2.SignedBeaconBlock, error) {
	...
	var blk block2.SignedBeaconBlock
	var err error
	if best != nil {
	...
	return []block2.SignedBeaconBlock{blk}, nil
}
...
func createBlockIndicesFromBlock(ctx context.Context, block block2.BeaconBlock) map[string][]byte {
