...
func TestStore_DeleteBlock(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, db.SaveBlockRoot(ctx, stRoot, root))
	blkRoot, err := db.BlockRoot(ctx, stRoot)
	require.NoError(t, err)
	require.Equal(t, blkRoot, root)
	...
	require.NoError(t, err)
	require.NotNil(t, b)
	st2, err := util.NewBeaconState(util.FillRootsNaturalOpt)
	require.NoError(t, err)
	require.NoError(t, db.SaveState(ctx, st2, root2))
	stRoot2, err := st2.HashTreeRoot(ctx)
	require.NoError(t, err)
	require.NoError(t, db.SaveBlockRoot(ctx, stRoot2, root2))
	require.NoError(t, db.DeleteBlock(ctx, root2))
	st2, err = db.State(ctx, root2)
	require.NoError(t, err)
	require.Equal(t, st2, nil)
	root2, err = db.BlockRoot(ctx, stRoot2)
	require.NoError(t, err)
