...
func TestStore_BlocksBySlot_BlockRootsBySlot(t *testing.T) {
	...
	retrievedBlocks, hasBlocks, err := db.BlocksBySlot(ctx, 1)
	require.NoError(t, err)
	...
	retrievedBlocks, hasBlocks, err = db.BlocksBySlot(ctx, 20)
	require.NoError(t, err)
	...
	retrievedBlocks, hasBlocks, err = db.BlocksBySlot(ctx, 100)
	require.NoError(t, err)
