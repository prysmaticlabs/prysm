...
func Eth1DataRootWithHasher(hasher htrutils2.HashFn, eth1Data *ethpb.Eth1Data) ([32]byte, error) {
	if eth1Data == nil {
	...
	root, err := htrutils2.BitwiseMerkleize(hasher, fieldRoots, uint64(len(fieldRoots)), uint64(len(fieldRoots)))
	if err != nil {
		return [32]byte{}, err
	}
	return root, nil
...
func Eth1DatasRoot(eth1Datas []*ethpb.Eth1Data) ([32]byte, error) {
	...
	eth1Chunks, err := htrutils2.Pack(eth1VotesRoots)
	if err != nil {
		return [32]byte{}, errors.Wrap(err, "could not chunk eth1 votes roots")
	}

	eth1VotesRootsRoot, err := htrutils2.BitwiseMerkleize(
		hasher,
		eth1Chunks,
	...
	root := htrutils2.MixInLength(eth1VotesRootsRoot, eth1VotesRootBufRoot)
