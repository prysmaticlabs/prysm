...
func TestInitializeFromProto(t *testing.T) {
	testState, _ := customtesting.DeterministicGenesisState(t, 64)
...
func TestBeaconState_AppendValidator_DoesntMutateCopy(t *testing.T) {
	st0, err := customtesting.NewBeaconState()
	require.NoError(t, err)
	st1 := st0.Copy()
...
func TestBeaconState_ToProto(t *testing.T) {
	source, err := customtesting.NewBeaconState(customtesting.FillRootsNaturalOpt, func(state *ethpb.BeaconState) error {
		state.GenesisTime = 1
