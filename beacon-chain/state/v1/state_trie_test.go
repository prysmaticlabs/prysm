...
func TestBeaconState_AppendValidator_DoesntMutateCopy(t *testing.T) {
	...
	_, ok := st1.ValidatorIndexByPubkey(bytes.ToBytes48(val.PublicKey))
...
func TestBeaconState_ToProto(t *testing.T) {
	source, err := testutil.NewBeaconState(testutil.FillRootsNaturalOpt, func(state *ethpb.BeaconState) error {
		state.GenesisTime = 1
		state.GenesisValidatorsRoot = bytes.PadTo([]byte("genesisvalidatorroot"), 32)
		state.Slot = 2
		state.Fork = &ethpb.Fork{
			PreviousVersion: bytes.PadTo([]byte("123"), 4),
			CurrentVersion:  bytes.PadTo([]byte("456"), 4),
			Epoch:           3,
		}
		state.LatestBlockHeader = &eth.BeaconBlockHeader{
			Slot:          4,
			ProposerIndex: 5,
			ParentRoot:    bytes.PadTo([]byte("lbhparentroot"), 32),
			StateRoot:     bytes.PadTo([]byte("lbhstateroot"), 32),
			BodyRoot:      bytes.PadTo([]byte("lbhbodyroot"), 32),
		}
		state.BlockRoots = [][]byte{bytes.PadTo([]byte("blockroots"), 32)}
		state.StateRoots = [][]byte{bytes.PadTo([]byte("stateroots"), 32)}
		state.HistoricalRoots = [][]byte{bytes.PadTo([]byte("historicalroots"), 32)}
		state.Eth1Data = &eth.Eth1Data{
			DepositRoot:  bytes.PadTo([]byte("e1ddepositroot"), 32),
			DepositCount: 6,
			BlockHash:    bytes.PadTo([]byte("e1dblockhash"), 32),
		}
		state.Eth1DataVotes = []*eth.Eth1Data{{
			DepositRoot:  bytes.PadTo([]byte("e1dvdepositroot"), 32),
			DepositCount: 7,
			BlockHash:    bytes.PadTo([]byte("e1dvblockhash"), 32),
		}}
		state.Eth1DepositIndex = 8
		state.Validators = []*eth.Validator{{
			PublicKey:                  bytes.PadTo([]byte("publickey"), 48),
			WithdrawalCredentials:      bytes.PadTo([]byte("withdrawalcredentials"), 32),
			EffectiveBalance:           9,
			Slashed:                    true,
		...
		state.RandaoMixes = [][]byte{bytes.PadTo([]byte("randaomixes"), 32)}
		...
		state.PreviousEpochAttestations = []*ethpb.PendingAttestation{{
			...
			Data: &eth.AttestationData{
				Slot:            17,
				CommitteeIndex:  18,
				BeaconBlockRoot: bytes.PadTo([]byte("peabeaconblockroot"), 32),
				Source: &eth.Checkpoint{
					Epoch: 19,
					Root:  bytes.PadTo([]byte("peasroot"), 32),
				},
				Target: &eth.Checkpoint{
					Epoch: 20,
					Root:  bytes.PadTo([]byte("peatroot"), 32),
				},
			},
			InclusionDelay: 21,
			ProposerIndex:  22,
		...
		state.CurrentEpochAttestations = []*ethpb.PendingAttestation{{
			...
			Data: &eth.AttestationData{
				Slot:            24,
				CommitteeIndex:  25,
				BeaconBlockRoot: bytes.PadTo([]byte("ceabeaconblockroot"), 32),
				Source: &eth.Checkpoint{
					Epoch: 26,
					Root:  bytes.PadTo([]byte("ceasroot"), 32),
				},
				Target: &eth.Checkpoint{
					Epoch: 27,
					Root:  bytes.PadTo([]byte("ceatroot"), 32),
				},
			},
			InclusionDelay: 28,
			ProposerIndex:  29,
		...
		state.PreviousJustifiedCheckpoint = &eth.Checkpoint{
			Epoch: 30,
			Root:  bytes.PadTo([]byte("pjcroot"), 32),
		}
		state.CurrentJustifiedCheckpoint = &eth.Checkpoint{
			Epoch: 31,
			Root:  bytes.PadTo([]byte("cjcroot"), 32),
		}
		state.FinalizedCheckpoint = &eth.Checkpoint{
			Epoch: 32,
			Root:  bytes.PadTo([]byte("fcroot"), 32),
		}
		return nil
	})
	require.NoError(t, err)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("genesisvalidatorroot"), 32), result.GenesisValidatorsRoot)
	assert.Equal(t, types.Slot(2), result.Slot)
	resultFork := result.Fork
	require.NotNil(t, resultFork)
	assert.DeepEqual(t, bytes.PadTo([]byte("123"), 4), resultFork.PreviousVersion)
	assert.DeepEqual(t, bytes.PadTo([]byte("456"), 4), resultFork.CurrentVersion)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("lbhparentroot"), 32), resultLatestBlockHeader.ParentRoot)
	assert.DeepEqual(t, bytes.PadTo([]byte("lbhstateroot"), 32), resultLatestBlockHeader.StateRoot)
	assert.DeepEqual(t, bytes.PadTo([]byte("lbhbodyroot"), 32), resultLatestBlockHeader.BodyRoot)
	assert.DeepEqual(t, [][]byte{bytes.PadTo([]byte("blockroots"), 32)}, result.BlockRoots)
	assert.DeepEqual(t, [][]byte{bytes.PadTo([]byte("stateroots"), 32)}, result.StateRoots)
	assert.DeepEqual(t, [][]byte{bytes.PadTo([]byte("historicalroots"), 32)}, result.HistoricalRoots)
	resultEth1Data := result.Eth1Data
	require.NotNil(t, resultEth1Data)
	assert.DeepEqual(t, bytes.PadTo([]byte("e1ddepositroot"), 32), resultEth1Data.DepositRoot)
	assert.Equal(t, uint64(6), resultEth1Data.DepositCount)
	assert.DeepEqual(t, bytes.PadTo([]byte("e1dblockhash"), 32), resultEth1Data.BlockHash)
	require.Equal(t, 1, len(result.Eth1DataVotes))
	resultEth1DataVote := result.Eth1DataVotes[0]
	require.NotNil(t, resultEth1DataVote)
	assert.DeepEqual(t, bytes.PadTo([]byte("e1dvdepositroot"), 32), resultEth1DataVote.DepositRoot)
	assert.Equal(t, uint64(7), resultEth1DataVote.DepositCount)
	assert.DeepEqual(t, bytes.PadTo([]byte("e1dvblockhash"), 32), resultEth1DataVote.BlockHash)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("publickey"), 48), resultValidator.Pubkey)
	assert.DeepEqual(t, bytes.PadTo([]byte("withdrawalcredentials"), 32), resultValidator.WithdrawalCredentials)
	...
	assert.DeepEqual(t, [][]byte{bytes.PadTo([]byte("randaomixes"), 32)}, result.RandaoMixes)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("peabeaconblockroot"), 32), resultPrevEpochAttData.BeaconBlockRoot)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("peasroot"), 32), resultPrevEpochAttSource.Root)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("peatroot"), 32), resultPrevEpochAttTarget.Root)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("ceabeaconblockroot"), 32), resultCurrEpochAttData.BeaconBlockRoot)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("ceasroot"), 32), resultCurrEpochAttSource.Root)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("ceatroot"), 32), resultCurrEpochAttTarget.Root)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("pjcroot"), 32), resultPrevJustifiedCheckpoint.Root)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("cjcroot"), 32), resultCurrJustifiedCheckpoint.Root)
	...
	assert.DeepEqual(t, bytes.PadTo([]byte("fcroot"), 32), resultFinalizedCheckpoint.Root)
}
