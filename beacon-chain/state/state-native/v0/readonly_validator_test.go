...
func TestReadOnlyValidator_ReturnsErrorOnNil(t *testing.T) {
	if _, err := v0.NewValidator(nil); err != v0.ErrNilWrappedValidator {
		t.Errorf("Wrong error returned. Got %v, wanted %v", err, v0.ErrNilWrappedValidator)
	}
}

func TestReadOnlyValidator_EffectiveBalance(t *testing.T) {
	bal := uint64(234)
	v, err := v0.NewValidator(&ethpb.Validator{EffectiveBalance: bal})
...
func TestReadOnlyValidator_ActivationEligibilityEpoch(t *testing.T) {
	...
	v, err := v0.NewValidator(&ethpb.Validator{ActivationEligibilityEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_ActivationEpoch(t *testing.T) {
	epoch := types.Epoch(234)
	v, err := v0.NewValidator(&ethpb.Validator{ActivationEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_WithdrawableEpoch(t *testing.T) {
	epoch := types.Epoch(234)
	v, err := v0.NewValidator(&ethpb.Validator{WithdrawableEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_ExitEpoch(t *testing.T) {
	epoch := types.Epoch(234)
	v, err := v0.NewValidator(&ethpb.Validator{ExitEpoch: epoch})
	require.NoError(t, err)
...
func TestReadOnlyValidator_PublicKey(t *testing.T) {
	...
	v, err := v0.NewValidator(&ethpb.Validator{PublicKey: key[:]})
	require.NoError(t, err)
...
func TestReadOnlyValidator_WithdrawalCredentials(t *testing.T) {
	...
	v, err := v0.NewValidator(&ethpb.Validator{WithdrawalCredentials: creds})
	require.NoError(t, err)
...
func TestReadOnlyValidator_Slashed(t *testing.T) {
	slashed := true
	v, err := v0.NewValidator(&ethpb.Validator{Slashed: slashed})
	require.NoError(t, err)
