...
func TestReplayBlocks_ThroughForkBoundary(t *testing.T) {
	...
	bCfg.ForkVersionSchedule[bytes.ToBytes4(bCfg.AltairForkVersion)] = 1
...
func TestLoadBlocks_FirstBranch(t *testing.T) {
	...
	roots, savedBlocks, err := tree1(t, beaconDB, bytes.PadTo([]byte{'A'}, 32))
	require.NoError(t, err)
...
func TestLoadBlocks_SameSlots(t *testing.T) {
	...
	roots, savedBlocks, err := tree2(t, beaconDB, bytes.PadTo([]byte{'A'}, 32))
	require.NoError(t, err)
...
func TestLoadBlocks_SameEndSlots(t *testing.T) {
	...
	roots, savedBlocks, err := tree3(t, beaconDB, bytes.PadTo([]byte{'A'}, 32))
	require.NoError(t, err)
...
func TestLoadBlocks_SameEndSlotsWith2blocks(t *testing.T) {
	...
	roots, savedBlocks, err := tree4(t, beaconDB, bytes.PadTo([]byte{'A'}, 32))
	require.NoError(t, err)
...
func TestLoadBlocks_BadStart(t *testing.T) {
	...
	roots, _, err := tree1(t, beaconDB, bytes.PadTo([]byte{'A'}, 32))
	require.NoError(t, err)
...
func tree2(t *testing.T, beaconDB db.Database, genesisRoot []byte) ([][32]byte, []*ethpb.SignedBeaconBlock, error) {
	...
	b21.Block.StateRoot = bytes.PadTo([]byte{'A'}, 32)
	...
	b22.Block.StateRoot = bytes.PadTo([]byte{'B'}, 32)
	...
	b23.Block.StateRoot = bytes.PadTo([]byte{'C'}, 32)
	...
	b24.Block.StateRoot = bytes.PadTo([]byte{'D'}, 32)
	...
	for _, b := range []*ethpb.SignedBeaconBlock{b0, b1, b21, b22, b23, b24, b3} {
		...
		beaconBlock.Block.Slot = b.Block.Slot
		beaconBlock.Block.ParentRoot = bytes.PadTo(b.Block.ParentRoot, 32)
		beaconBlock.Block.StateRoot = bytes.PadTo(b.Block.StateRoot, 32)
		if err := beaconDB.SaveBlock(context.Background(), wrapper.WrappedPhase0SignedBeaconBlock(beaconBlock)); err != nil {
			return nil, nil, err
		}
		if err := beaconDB.SaveState(context.Background(), st.Copy(), bytes.ToBytes32(beaconBlock.Block.ParentRoot)); err != nil {
