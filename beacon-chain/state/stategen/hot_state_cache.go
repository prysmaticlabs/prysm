...
func (c *hotStateCache) get(root [32]byte) interfaces.BeaconState {
	c.lock.RLock()
	...
	if exists && item != nil {
		...
		return item.(interfaces.BeaconState).Copy()
	}
...
func (c *hotStateCache) getWithoutCopy(root [32]byte) interfaces.BeaconState {
	c.lock.RLock()
	defer c.lock.RUnlock()
	item, exists := c.cache.Get(root)
	if exists && item != nil {
		hotStateCacheHit.Inc()
		return item.(interfaces.BeaconState)
	}
	hotStateCacheMiss.Inc()
	return nil
}
...
func (c *hotStateCache) put(root [32]byte, state interfaces.BeaconState) {
	c.lock.Lock()
	defer c.lock.Unlock()
