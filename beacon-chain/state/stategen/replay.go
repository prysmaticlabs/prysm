...
func (s *State) LoadBlocks(ctx context.Context, startSlot, endSlot types.Slot, endBlockRoot [32]byte) ([]blockInterface.SignedBeaconBlock, error) {
	...
	filteredBlocks := []blockInterface.SignedBeaconBlock{blocks[length-1]}
...
func (s *State) loadFinalizedBlocks(ctx context.Context, startSlot, endSlot types.Slot) ([]blockInterface.SignedBeaconBlock, error) {
	...
	fbs := make([]blockInterface.SignedBeaconBlock, 0, len(bs))
