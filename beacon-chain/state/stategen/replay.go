...
func (s *State) LoadBlocks(ctx context.Context, startSlot, endSlot types.Slot, endBlockRoot [32]byte) ([]block2.SignedBeaconBlock, error) {
	...
	filteredBlocks := []block2.SignedBeaconBlock{blocks[length-1]}
...
func (s *State) loadFinalizedBlocks(ctx context.Context, startSlot, endSlot types.Slot) ([]block2.SignedBeaconBlock, error) {
	...
	fbs := make([]block2.SignedBeaconBlock, 0, len(bs))
