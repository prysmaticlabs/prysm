...
func TestLoadeStateBySlot_CanReplayBlock(t *testing.T) {
	...
	genesisBlockRoot := bytes.ToBytes32(nil)
	...
	service.hotStateCache.put(bytes.ToBytes32(b1.Block.ParentRoot), genesis)
...
func TestLoadeStateBySlot_DoesntReplayBlockOnRequestedSlot(t *testing.T) {
	...
	require.NotEqual(t, params.BeaconConfig().ZeroHash, bytes.ToBytes32(loadedState.LatestBlockHeader().StateRoot))
}
...
func TestLastAncestorState_CanGetUsingDB(t *testing.T) {
	...
	b0.Block.ParentRoot = bytes.PadTo([]byte{'a'}, 32)
	r0, err := b0.Block.HashTreeRoot()
	require.NoError(t, err)
	b1 := testutil.NewBeaconBlock()
	b1.Block.Slot = 1
	b1.Block.ParentRoot = bytes.PadTo(r0[:], 32)
	r1, err := b1.Block.HashTreeRoot()
	require.NoError(t, err)
	b2 := testutil.NewBeaconBlock()
	b2.Block.Slot = 2
	b2.Block.ParentRoot = bytes.PadTo(r1[:], 32)
	r2, err := b2.Block.HashTreeRoot()
	require.NoError(t, err)
	b3 := testutil.NewBeaconBlock()
	b3.Block.Slot = 3
	b3.Block.ParentRoot = bytes.PadTo(r2[:], 32)
