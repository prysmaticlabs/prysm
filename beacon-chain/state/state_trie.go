...
type StateTrie struct {
	lock         sync.RWMutex
	merkleLayers [][][]byte
	leaves       *pbp2p.BeaconState
}

func (s *StateTrie) HashTreeRoot() [32]byte {
	s.lock.RLock()
	defer s.lock.RUnlock()
	return bytesutil.ToBytes32(s.merkleLayers[len(s.merkleLayers)-1][0])
}
...
func (s *StateTrie) recomputeRoot(idx int) {
	layers := s.merkleLayers
	// The merkle tree structure looks as follows:
	// [[r1, r2, r3, r4], [parent1, parent2], [root]]
	// Using information about the index which changed, idx, we recompute
	// only its branch up the tree.
	currentIndex := idx
	root := s.merkleLayers[0][idx]
