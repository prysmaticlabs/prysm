...
func TestFieldTrie_NewTrie(t *testing.T) {
	...
	trie, err := state_native.NewFieldTrie(types.FieldIndex(5), types.BasicArray, newState.StateRoots(), uint64(params.BeaconConfig().SlotsPerHistoricalRoot))
...
func TestFieldTrie_CopyTrieImmutable(t *testing.T) {
	...
	trie, err := state_native.NewFieldTrie(types.FieldIndex(13), types.BasicArray, newState.RandaoMixes(), uint64(params.BeaconConfig().EpochsPerHistoricalVector))
	require.NoError(t, err)
