...
func TestValidatorMap_DistinctCopy(t *testing.T) {
	...
	handler.Set(bytes.ToBytes48([]byte(wantedPubkey)), 27)
	val1, _ := handler.Get(bytes.ToBytes48([]byte(wantedPubkey)))
	val2, _ := newHandler.Get(bytes.ToBytes48([]byte(wantedPubkey)))
