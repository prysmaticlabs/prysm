...
func computeFieldRoots(state *ethpb.BeaconStateAltair) ([][32]byte, error) {
...
func (h *stateRootHasher) computeFieldRootsWithHasher(state *ethpb.BeaconStateAltair) ([][32]byte, error) {
	if state == nil {
		return nil, errors.New("nil state")
	}
	hasher := hashutil.CustomSHA256Hasher()
	fieldRoots := make([][32]byte, params.BeaconConfig().BeaconStateAltairFieldCount)
