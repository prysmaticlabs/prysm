...
func TestLatestAttestationTarget_CantGetAttestation(t *testing.T) {
	...
	if _, err := service.LatestAttestationTarget(ctx, headState, index); !strings.Contains(err.Error(), want) {
...
func TestLatestAttestationTarget_ReturnsLatestAttestedBlock(t *testing.T) {
	...
	latestAttestedTarget, err := service.LatestAttestationTarget(ctx, headState, 0)
	if err != nil {
