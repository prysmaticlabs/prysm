...
func configureExecutionSetting(cliCtx *cli.Context) error {
	if cliCtx.IsSet(flags.TerminalTotalDifficultyOverride.Name) {
		log.Warnf("%s has been deprecated post-merge", flags.TerminalTotalDifficultyOverride.Name)
	}
	if cliCtx.IsSet(flags.TerminalBlockHashOverride.Name) {
		log.Warnf("%s has been deprecated post-merge", flags.TerminalBlockHashOverride.Name)
	}
	if cliCtx.IsSet(flags.TerminalBlockHashActivationEpochOverride.Name) {
		log.Warnf("%s has been deprecated post-merge", flags.TerminalBlockHashActivationEpochOverride.Name)
	}
