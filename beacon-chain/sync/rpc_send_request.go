...
type BeaconBlockProcessor func(block prysm.SignedBeaconBlock) error
...
func SendBeaconBlocksByRangeRequest(
	...
	blocks := make([]prysm.SignedBeaconBlock, 0, req.Count)
	process := func(blk prysm.SignedBeaconBlock) error {
...
func SendBeaconBlocksByRootRequest(
	...
	// Augment block processing function, if non-nil block processor is provided.
	blocks := make([]prysm.SignedBeaconBlock, 0, len(*req))
	process := func(block prysm.SignedBeaconBlock) error {
