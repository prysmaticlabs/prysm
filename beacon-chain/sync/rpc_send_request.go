...
type BeaconBlockProcessor func(block blockInterface.SignedBeaconBlock) error
...
func SendBeaconBlocksByRangeRequest(
...
) ([]blockInterface.SignedBeaconBlock, error) {
	...
	blocks := make([]blockInterface.SignedBeaconBlock, 0, req.Count)
	process := func(blk blockInterface.SignedBeaconBlock) error {
...
func SendBeaconBlocksByRootRequest(
	...
	// Augment block processing function, if non-nil block processor is provided.
	blocks := make([]blockInterface.SignedBeaconBlock, 0, len(*req))
	process := func(block blockInterface.SignedBeaconBlock) error {
