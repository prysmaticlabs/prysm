...
func TestProcessPendingAtts_HasBlockSaveUnAggregatedAtt(t *testing.T) {
	...
	require.NoError(t, db.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(sb)))
	...
	require.NoError(t, err)
	require.NoError(t, r.cfg.DB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(sb)))
...
func TestProcessPendingAtts_NoBroadcastWithBadSignature(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, r.cfg.DB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(b)))
	require.NoError(t, r.cfg.DB.SaveState(context.Background(), s, r32))
