...
func TestValidateBeaconBlockPubSub_CanRecoverStateSummary(t *testing.T) {
	...
	msg.Block.Slot = 1
	msg.Block.ProposerIndex = proposerIdx
	msg.Signature, err = core.ComputeDomainAndSign(beaconState, 0, msg.Block, params.BeaconConfig().DomainBeaconProposer, privKeys[proposerIdx])
	require.NoError(t, err)
...
func TestValidateBeaconBlockPubSub_FilterByFinalizedEpoch(t *testing.T) {
	...
	require.NoError(t, err)
	digest, err := core.ComputeForkDigest(params.BeaconConfig().GenesisForkVersion, make([]byte, 32))
	assert.NoError(t, err)
