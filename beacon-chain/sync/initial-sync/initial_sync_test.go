...
func initializeTestServices(t *testing.T, slots []types.Slot, peers []*peerData) (*mock.ChainService, *p2pt.TestP2P, db.Database) {
	...
	err := beaconDB.SaveBlock(context.Background(), wrapper.WrappedPhase0SignedBeaconBlock(customtesting.NewBeaconBlock()))
...
func extendBlockSequence(t *testing.T, inSeq []*eth.SignedBeaconBlock, size int) []*eth.SignedBeaconBlock {
	...
	startSlot := len(inSeq)
	if len(inSeq) == 0 {
		outSeq[0] = customtesting.NewBeaconBlock()
		outSeq[0].Block.StateRoot = customtesting.Random32Bytes(t)
		startSlot++
	...
	for slot := startSlot; slot < len(outSeq); slot++ {
		outSeq[slot] = customtesting.NewBeaconBlock()
		...
		outSeq[slot].Block.StateRoot = customtesting.Random32Bytes(t)
	}
