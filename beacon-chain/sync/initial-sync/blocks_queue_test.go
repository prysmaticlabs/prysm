...
func TestBlocksQueue_Loop(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, queue.start())
			processBlock := func(block block2.SignedBeaconBlock) error {
			...
			var blocks []block2.SignedBeaconBlock
...
func TestBlocksQueue_onDataReceivedEvent(t *testing.T) {
	...
	t.Run("transition ok", func(t *testing.T) {
		...
		assert.DeepSSZEqual(t, []block2.SignedBeaconBlock(nil), fsm.blocks)
...
func TestBlocksQueue_onReadyToSendEvent(t *testing.T) {
	...
	t.Run("send from the first machine", func(t *testing.T) {
		...
		queue.smm.machines[256].blocks = []block2.SignedBeaconBlock{
			wrapper.WrappedPhase0SignedBeaconBlock(testutil.NewBeaconBlock()),
		}
	...
	t.Run("previous machines are not processed - do not send", func(t *testing.T) {
		...
		queue.smm.machines[320].blocks = []block2.SignedBeaconBlock{
			wrapper.WrappedPhase0SignedBeaconBlock(testutil.NewBeaconBlock()),
		}
...
func TestBlocksQueue_stuckWhenHeadIsSetToOrphanedBlock(t *testing.T) {
	...
	isProcessedBlock := func(ctx context.Context, blk block2.SignedBeaconBlock, blkRoot [32]byte) bool {
