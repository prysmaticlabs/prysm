...
type blockReceiverFn func(ctx context.Context, block prysm.SignedBeaconBlock, blockRoot [32]byte) error

// batchBlockReceiverFn defines batch receiving function.
type batchBlockReceiverFn func(ctx context.Context, blks []prysm.SignedBeaconBlock, roots [][32]byte) error
...
func (s *Service) logSyncStatus(genesis time.Time, blk prysm.BeaconBlock, blkRoot [32]byte) {
	s.counter.Incr(1)
...
func (s *Service) logBatchSyncStatus(genesis time.Time, blks []prysm.SignedBeaconBlock, blkRoot [32]byte) {
...
func (s *Service) processBatchedBlocks(ctx context.Context, genesis time.Time,
	blks []prysm.SignedBeaconBlock, bFunc batchBlockReceiverFn) error {
	if len(blks) == 0 {
...
func (s *Service) isProcessedBlock(ctx context.Context, blk prysm.SignedBeaconBlock, blkRoot [32]byte) bool {
