...
func (s *Service) roundRobinSync(genesis time.Time) error {
	...
	if s.cfg.Chain.HeadSlot() == coreTime.SlotsSince(genesis) {
		return nil
	}
...
func (s *Service) syncToFinalizedEpoch(ctx context.Context, genesis time.Time) error {
	highestFinalizedSlot, err := coreTime.StartSlot(s.highestFinalizedEpoch() + 1)
	if err != nil {
		return err
	}
...
func (s *Service) syncToNonFinalizedEpoch(ctx context.Context, genesis time.Time) error {
	queue := newBlocksQueue(ctx, &blocksQueueConfig{
		...
		db:                  s.cfg.DB,
		chain:               s.cfg.Chain,
		highestExpectedSlot: coreTime.SlotsSince(genesis),
...
func (s *Service) logSyncStatus(genesis time.Time, blk block.BeaconBlock, blkRoot [32]byte) {
	...
	if coreTime.IsEpochStart(blk.Slot()) {
		timeRemaining := time.Duration(float64(coreTime.SlotsSince(genesis)-blk.Slot())/rate) * time.Second
		log.WithFields(logrus.Fields{
			...
			blk.Slot(), coreTime.SlotsSince(genesis), timeRemaining,
		)
	}
}
...
func (s *Service) logBatchSyncStatus(genesis time.Time, blks []block.SignedBeaconBlock, blkRoot [32]byte) {
	...
	firstBlk := blks[0]
	timeRemaining := time.Duration(float64(coreTime.SlotsSince(genesis)-firstBlk.Block().Slot())/rate) * time.Second
	log.WithFields(logrus.Fields{
		"peers":           len(s.cfg.P2P.Peers().Connected()),
		"blocksPerSecond": fmt.Sprintf("%.1f", rate),
	}).Infof(
		...
		firstBlk.Block().Slot(), coreTime.SlotsSince(genesis), timeRemaining,
	)
}
...
func (s *Service) isProcessedBlock(ctx context.Context, blk block.SignedBeaconBlock, blkRoot [32]byte) bool {
	finalizedSlot, err := coreTime.StartSlot(s.cfg.Chain.FinalizedCheckpt().Epoch)
	if err != nil {
		return false
	}
