...
func TestBlocksFetcher_findAncestor(t *testing.T) {
	...
	t.Run("error on request", func(t *testing.T) {
		...
		_, err := fetcher.findAncestor(ctx, p2.PeerID(), blockInterface.WrappedPhase0SignedBeaconBlock(knownBlocks[4]))
	...
	t.Run("no blocks", func(t *testing.T) {
		...
		fork, err := fetcher.findAncestor(ctx, p2.PeerID(), blockInterface.WrappedPhase0SignedBeaconBlock(knownBlocks[4]))
