...
func TestBlocksFetcher_RoundRobin(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			processFetchedBlocks := func() ([]block2.SignedBeaconBlock, error) {
				defer cancel()
				var unionRespBlocks []block2.SignedBeaconBlock
...
func TestBlocksFetcher_requestBlocksFromPeerReturningInvalidBlocks(t *testing.T) {
	...
	tests := []struct {
		...
		wantedErr    string
		validate     func(req *p2ppb.BeaconBlocksByRangeRequest, blocks []block2.SignedBeaconBlock)
	}{
		{
			name: "no error",
			req: &p2ppb.BeaconBlocksByRangeRequest{
