		...
		},
		{
			...
			setupSvc: func(s *Service, msg *ethpb.SignedContributionAndProof) *Service {
				...
				for i := uint64(0); i < params.BeaconConfig().SyncCommitteeSubnetCount; i++ {
					...
					assert.NoError(t, err)
					for _, p := range coms {
						idx, ok := hState.ValidatorIndexByPubkey(bytes.ToBytes48(p))
		...
		},
		{
			...
			setupSvc: func(s *Service, msg *ethpb.SignedContributionAndProof) *Service {
				...
				s.cfg.Chain = &mockChain.ChainService{
					...
					Genesis:                     time.Now().Add(-time.Second * time.Duration(params.BeaconConfig().SecondsPerSlot) * time.Duration(msg.Message.Contribution.Slot)),
					CurrentSyncCommitteeIndices: []types.CommitteeIndex{types.CommitteeIndex(msg.Message.Contribution.SubcommitteeIndex * subCommitteeSize)},
					PublicKey:                   bytes.ToBytes48(pubkey),
		...
		},
		{
			...
			setupSvc: func(s *Service, msg *ethpb.SignedContributionAndProof) *Service {
				...
				s.cfg.Chain = &mockChain.ChainService{
					...
					Genesis:                     time.Now().Add(-time.Second * time.Duration(params.BeaconConfig().SecondsPerSlot) * time.Duration(msg.Message.Contribution.Slot)),
					CurrentSyncCommitteeIndices: []types.CommitteeIndex{types.CommitteeIndex(msg.Message.Contribution.SubcommitteeIndex * subCommitteeSize)},
					PublicKey:                   bytes.ToBytes48(keys[msg.Message.AggregatorIndex].PublicKey().Marshal()),
					SyncSelectionProofDomain:    d,
				...
				for i := uint64(0); i < params.BeaconConfig().SyncCommitteeSubnetCount; i++ {
					...
					for _, p := range coms {
						...
						if isAggregator {
							...
							assert.NoError(t, err)
							valIdx, ok := hState.ValidatorIndexByPubkey(bytes.ToBytes48(coms[0]))
							assert.Equal(t, true, ok)
				...
				for i := uint64(0); i < params.BeaconConfig().SyncCommitteeSubnetCount; i++ {
					...
					for _, p := range coms {
						...
						if isAggregator {
							...
							sigs := []bls.Signature{}
							for i, p2 := range coms {
								idx, ok := hState.ValidatorIndexByPubkey(bytes.ToBytes48(p2))
								assert.Equal(t, true, ok)
