...
func TestStatusRPCRequest_BadPeerHandshake(t *testing.T) {
	...
	assert.Equal(t, false, p1.Peers().Scorers().Status(p2.PeerID()), "Peer is marked as bad")
	p1.Connect(p2)
	...
	assert.Equal(t, true, p1.Peers().Scorers().Status(p2.PeerID()), "Peer is not marked as bad")
}
