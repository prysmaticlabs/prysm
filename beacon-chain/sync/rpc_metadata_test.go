...
func TestMetaDataRPCHandler_ReceivesMetadata(t *testing.T) {
	...
	// Setup streams
	pcl := protocol.ID(p2p.RPCMetaDataTopicV1.String())
	topic := string(pcl)
...
func TestMetadataRPCHandler_SendsMetadata(t *testing.T) {
	...
	// Setup streams
	pcl := protocol.ID(p2p.RPCMetaDataTopicV1.ConvertToStringWithSuffix(r.cfg.p2p.Encoding().ProtocolSuffix()))
	topic := string(pcl)
	r.rateLimiter.limiterMap[topic] = leakybucket.NewCollector(1, 1, time.Second, false)
...
func TestMetadataRPCHandler_SendsMetadataAltair(t *testing.T) {
	...
	// Setup streams
	pcl := protocol.ID(p2p.RPCMetaDataTopicV2.ConvertToStringWithSuffix(r.cfg.p2p.Encoding().ProtocolSuffix()))
	topic := string(pcl)
