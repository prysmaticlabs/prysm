...
func (s *Service) processAttestations(ctx context.Context, attestations []interfaces.SignedAggregateAttestationAndProof) {
	for _, signedAtt := range attestations {
		att := signedAtt.GetAggregateAttestationAndProof()
		// The pending attestations can arrive in both aggregated and unaggregated forms,
		// each from has distinct validation steps.
		if helpers.IsAggregated(att.GetAttestation()) {
			...
			if s.validateBlockInAttestation(ctx, signedAtt) && aggValid {
				if err := s.cfg.attPool.SaveAggregatedAttestation(att.GetAttestation()); err != nil {
					log.WithError(err).Debug("Could not save aggregate attestation")
					continue
				}
				s.setAggregatorIndexEpochSeen(att.GetAttestation().GetData().Target.Epoch, att.GetAggregatorIndex())
			...
			if !s.cfg.chain.InForkchoice(bytesutil.ToBytes32(att.GetAttestation().GetData().BeaconBlockRoot)) {
				log.WithError(blockchain.ErrNotDescendantOfFinalized).Debug("Could not verify finalized consistency")
				continue
			}
			if err := s.cfg.chain.VerifyLmdFfgConsistency(ctx, att.GetAttestation()); err != nil {
				log.WithError(err).Debug("Could not verify FFG consistency")
				continue
			}
			preState, err := s.cfg.chain.AttestationTargetState(ctx, att.GetAttestation().GetData().Target)
			if err != nil {
				log.WithError(err).Debug("Could not retrieve attestation prestate")
				continue
			}

			valid, err := s.validateUnaggregatedAttWithState(ctx, att.GetAttestation(), preState)
			if err != nil {
				log.WithError(err).Debug("Pending unaggregated attestation failed validation")
				continue
			}
			if valid == pubsub.ValidationAccept {
				if err := s.cfg.attPool.SaveUnaggregatedAttestation(att.GetAttestation()); err != nil {
					log.WithError(err).Debug("Could not save unaggregated attestation")
					continue
				}
				s.setSeenCommitteeIndicesSlot(att.GetAttestation().GetData().Slot, att.GetAttestation().GetData().CommitteeIndex, att.GetAttestation().GetAggregationBits())

				valCount, err := helpers.ActiveValidatorCount(ctx, preState, slots.ToEpoch(att.GetAttestation().GetData().Slot))
				if err != nil {
					log.WithError(err).Debug("Could not retrieve active validator count")
					continue
				}
				// Broadcasting the signed attestation again once a node is able to process it.
				if err := s.cfg.p2p.BroadcastAttestation(ctx, helpers.ComputeSubnetForAttestation(valCount, signedAtt.GetAggregateAttestationAndProof().GetAttestation()), signedAtt.GetAggregateAttestationAndProof().GetAttestation()); err != nil {
					log.WithError(err).Debug("Could not broadcast")
				}
			}
		}
...
func (s *Service) savePendingAtt(att interfaces.SignedAggregateAttestationAndProof) {
	root := bytesutil.ToBytes32(att.GetAggregateAttestationAndProof().GetAttestation().GetData().BeaconBlockRoot)
	...
	if !ok {
		pendingAttCount.Inc()
		s.blkRootToPendingAtts[root] = []interfaces.SignedAggregateAttestationAndProof{att}
		return
	}
...
func attsAreEqual(a, b interfaces.SignedAggregateAttestationAndProof) bool {
	aAggregate := a.GetAggregateAttestationAndProof()
	aAtt := aAggregate.GetAttestation()
	aData := aAtt.GetData()
	bAggregate := b.GetAggregateAttestationAndProof()
	bAtt := bAggregate.GetAttestation()
	bData := bAtt.GetData()
	if a.GetSignature() != nil {
		return b.GetSignature() != nil && aAggregate.GetAggregatorIndex() == bAggregate.GetAggregatorIndex()
	}
	...
	if aData.CommitteeIndex != bData.CommitteeIndex {
		return false
	}
...
func (s *Service) validatePendingAtts(ctx context.Context, slot primitives.Slot) {
	...
	for bRoot, atts := range s.blkRootToPendingAtts {
		for i := len(atts) - 1; i >= 0; i-- {
			if slot >= atts[i].GetAggregateAttestationAndProof().GetAttestation().GetData().Slot+params.BeaconConfig().SlotsPerEpoch {
