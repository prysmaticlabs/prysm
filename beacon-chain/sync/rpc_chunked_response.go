...
func extractAttestationType(digest []byte, tor blockchain.TemporalOracle) (interfaces.Attestation, error) {
	if len(digest) == 0 {
	...
	return nil, errors.Wrapf(ErrNoValidDigest, "could not extract attestation data type, saw digest=%#x, genesis=%v, vr=%#x", digest, tor.GenesisTime(), tor.GenesisValidatorsRoot())
}

func extractAggregateAndProofType(digest []byte, tor blockchain.TemporalOracle) (interfaces.SignedAggregateAttestationAndProof, error) {
	if len(digest) == 0 {
		aFunc, ok := types.AggregatedAttestationMap[bytesutil.ToBytes4(params.BeaconConfig().GenesisForkVersion)]
		if !ok {
			return nil, errors.New("no aggregate and proof type exists for the genesis fork version.")
		}
		return aFunc()
	}
	if len(digest) != forkDigestLength {
	...
	vRoot := tor.GenesisValidatorsRoot()
	for k, aFunc := range types.AggregatedAttestationMap {
		rDigest, err := signing.ComputeForkDigest(k[:], vRoot[:])
		if err != nil {
			return nil, err
		}
	...
	return nil, errors.Wrapf(ErrNoValidDigest, "could not extract aggregate and proof data type, saw digest=%#x, genesis=%v, vr=%#x", digest, tor.GenesisTime(), tor.GenesisValidatorsRoot())
}
