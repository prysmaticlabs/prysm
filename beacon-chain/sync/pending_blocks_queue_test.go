...
func TestRegularSyncBeaconBlockSubscriber_ProcessPendingBlocks1(t *testing.T) {
	...
	require.NoError(t, r.cfg.DB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(b0)))
	b0Root, err := b0.Block.HashTreeRoot()
	require.NoError(t, err)
	b3 := testutil.NewBeaconBlock()
	b3.Block.Slot = 3
	b3.Block.ParentRoot = b0Root[:]
	require.NoError(t, r.cfg.DB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(b3)))
	...
	// Add b2 to the cache
	require.NoError(t, r.insertBlockToPendingQueue(b2.Block.Slot, blockInterface.WrappedPhase0SignedBeaconBlock(b2), b2Root))
	...
	// Add b1 to the cache
	require.NoError(t, r.insertBlockToPendingQueue(b1.Block.Slot, blockInterface.WrappedPhase0SignedBeaconBlock(b1), b1Root))
	require.NoError(t, r.cfg.DB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(b1)))

	// Insert bad b1 in the cache to verify the good one doesn't get replaced.
	require.NoError(t, r.insertBlockToPendingQueue(b1.Block.Slot, blockInterface.WrappedPhase0SignedBeaconBlock(testutil.NewBeaconBlock()), [32]byte{}))
...
func TestRegularSync_InsertDuplicateBlocks(t *testing.T) {
	...
	require.NoError(t, r.insertBlockToPendingQueue(b0.Block.Slot, blockInterface.WrappedPhase0SignedBeaconBlock(b0), b0r))
	require.Equal(t, 1, len(r.pendingBlocksInCache(b0.Block.Slot)), "Block was not added to map")

	require.NoError(t, r.insertBlockToPendingQueue(b1.Block.Slot, blockInterface.WrappedPhase0SignedBeaconBlock(b1), b1r))
...
func TestRegularSyncBeaconBlockSubscriber_ProcessPendingBlocks_2Chains(t *testing.T) {
	...
	require.NoError(t, r.insertBlockToPendingQueue(b4.Block.Slot, blockInterface.WrappedPhase0SignedBeaconBlock(b4), b4Root))
	require.NoError(t, r.insertBlockToPendingQueue(b5.Block.Slot, blockInterface.WrappedPhase0SignedBeaconBlock(b5), b5Root))
	...
	// Add b3 to the cache
	require.NoError(t, r.insertBlockToPendingQueue(b3.Block.Slot, blockInterface.WrappedPhase0SignedBeaconBlock(b3), b3Root))
	require.NoError(t, r.cfg.DB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(b3)))
	...
	require.NoError(t, r.cfg.DB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(b2)))
...
func TestService_sortedPendingSlots(t *testing.T) {
	...
	require.NoError(t, r.insertBlockToPendingQueue(lastSlot, blockInterface.WrappedPhase0SignedBeaconBlock(testutil.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{Block: &ethpb.BeaconBlock{Slot: lastSlot}})), [32]byte{1}))
	require.NoError(t, r.insertBlockToPendingQueue(lastSlot-3, blockInterface.WrappedPhase0SignedBeaconBlock(testutil.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{Block: &ethpb.BeaconBlock{Slot: lastSlot - 3}})), [32]byte{2}))
	require.NoError(t, r.insertBlockToPendingQueue(lastSlot-5, blockInterface.WrappedPhase0SignedBeaconBlock(testutil.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{Block: &ethpb.BeaconBlock{Slot: lastSlot - 5}})), [32]byte{3}))
	require.NoError(t, r.insertBlockToPendingQueue(lastSlot-2, blockInterface.WrappedPhase0SignedBeaconBlock(testutil.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{Block: &ethpb.BeaconBlock{Slot: lastSlot - 2}})), [32]byte{4}))
...
func TestService_AddPeningBlockToQueueOverMax(t *testing.T) {
	...
	require.NoError(t, r.insertBlockToPendingQueue(0, blockInterface.WrappedPhase0SignedBeaconBlock(b), [32]byte{}))
	require.NoError(t, r.insertBlockToPendingQueue(0, blockInterface.WrappedPhase0SignedBeaconBlock(b1), [32]byte{1}))
	require.NoError(t, r.insertBlockToPendingQueue(0, blockInterface.WrappedPhase0SignedBeaconBlock(b2), [32]byte{2}))

	b3 := copyutil.CopySignedBeaconBlock(b)
	b3.Block.StateRoot = []byte{'c'}
	require.NoError(t, r.insertBlockToPendingQueue(0, blockInterface.WrappedPhase0SignedBeaconBlock(b2), [32]byte{3}))
...
func TestService_ProcessPendingBlockOnCorrectSlot(t *testing.T) {
	...
	require.NoError(t, db.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(parentBlock)))
