...
func TestRequestPendingBlobs(t *testing.T) {
	...
	t.Run("old block should not fail", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NoError(t, s.requestPendingBlobs(context.Background(), b, []byte{}))
	})
	...
	t.Run("no peer to request blob sidecar", func(t *testing.T) {
		p1 := p2ptest.NewTestP2P(t)
		chain := &mock.ChainService{
			FinalizedCheckPoint: &ethpb.Checkpoint{
				Epoch: 1,
		...
		require.NoError(t, err)
		require.ErrorContains(t, "no peers to request blob sidecars", s.requestPendingBlobs(context.Background(), b1, []byte{}))
	})
	t.Run("unsupported protocol", func(t *testing.T) {
		...
		b1, err := blocks.NewBeaconBlock(b.Block)
		require.NoError(t, err)
		require.ErrorContains(t, "protocols not supported", s.requestPendingBlobs(context.Background(), b1, []byte{}))
	})
}
