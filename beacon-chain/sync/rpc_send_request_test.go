...
func TestSendRequest_SendBeaconBlocksByRangeRequest(t *testing.T) {
	...
	t.Run("has block processor - no errors", func(t *testing.T) {
		...
		blocksFromProcessor := make([]block2.SignedBeaconBlock, 0)
		blocks, err := SendBeaconBlocksByRangeRequest(ctx, nil, p1, p2.PeerID(), req, func(block block2.SignedBeaconBlock) error {
	...
	t.Run("has block processor - throw error", func(t *testing.T) {
		...
		_, err := SendBeaconBlocksByRangeRequest(ctx, nil, p1, p2.PeerID(), req, func(block block2.SignedBeaconBlock) error {
	...
	t.Run("process custom error", func(t *testing.T) {
		...
		p2.SetStreamHandler(pcl, knownBlocksProvider(p2, func(block block2.SignedBeaconBlock) error {
...
func TestSendRequest_SendBeaconBlocksByRootRequest(t *testing.T) {
	...
	t.Run("has block processor - no errors", func(t *testing.T) {
		...
		req := &p2pTypes.BeaconBlockByRootsReq{knownRoots[0], knownRoots[1]}
		blocksFromProcessor := make([]block2.SignedBeaconBlock, 0)
		blocks, err := SendBeaconBlocksByRootRequest(ctx, nil, p1, p2.PeerID(), req, func(block block2.SignedBeaconBlock) error {
			blocksFromProcessor = append(blocksFromProcessor, block)
			return nil
		})
		assert.NoError(t, err)
	...
	t.Run("has block processor - throw error", func(t *testing.T) {
		...
		req := &p2pTypes.BeaconBlockByRootsReq{knownRoots[0], knownRoots[1]}
		errFromProcessor := errors.New("processor error")
		_, err := SendBeaconBlocksByRootRequest(ctx, nil, p1, p2.PeerID(), req, func(block block2.SignedBeaconBlock) error {
			return errFromProcessor
		})
		assert.ErrorContains(t, errFromProcessor.Error(), err)
	})
