...
func (s *Service) processPendingBlocks(ctx context.Context) error {
	...
	for _, slot := range slots {
		...
		for _, b := range bs {
			...
			s.pendingQueueLock.RLock()
			inPendingQueue := s.seenPendingBlocks[butil.ToBytes32(b.Block().ParentRoot())]
			s.pendingQueueLock.RUnlock()
			...
			parentIsBad := s.hasBadBlock(butil.ToBytes32(b.Block().ParentRoot()))
			...
			inDB := s.cfg.DB.HasBlock(ctx, butil.ToBytes32(b.Block().ParentRoot()))
			hasPeer := len(pids) != 0
			...
			if !inPendingQueue && !inDB && hasPeer {
				log.WithFields(logrus.Fields{
					...
					"parentRoot":  hex.EncodeToString(butil.Trunc(b.Block().ParentRoot())),
				}).Debug("Requesting parent block")
				parentRoots = append(parentRoots, butil.ToBytes32(b.Block().ParentRoot()))
			...
			log.WithFields(logrus.Fields{
				"slot":      slot,
				"blockRoot": hex.EncodeToString(butil.Trunc(blkRoot[:])),
...
func (s *Service) validatePendingSlots() error {
	...
	for k := range items {
		...
		for _, b := range blks {
			...
			if oldBlockRoots[butil.ToBytes32(b.Block().ParentRoot())] {
