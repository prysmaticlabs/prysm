...
func (s *Service) deleteBlockFromPendingQueue(slot types.Slot, b block2.SignedBeaconBlock, r [32]byte) error {
	...
	newBlks := make([]block2.SignedBeaconBlock, 0, len(blks))
...
// Note: this helper is not thread safe.
func (s *Service) insertBlockToPendingQueue(slot types.Slot, b block2.SignedBeaconBlock, r [32]byte) error {
	mutexasserts.AssertRWMutexLocked(&s.pendingQueueLock)
...
func (s *Service) pendingBlocksInCache(slot types.Slot) []block2.SignedBeaconBlock {
	k := slotToCacheKey(slot)
	value, ok := s.slotToPendingBlocks.Get(k)
	if !ok {
		return []block2.SignedBeaconBlock{}
	}
	blks, ok := value.([]block2.SignedBeaconBlock)
	if !ok {
		return []block2.SignedBeaconBlock{}
	}
	return blks
...
func (s *Service) addPendingBlockToCache(b block2.SignedBeaconBlock) error {
