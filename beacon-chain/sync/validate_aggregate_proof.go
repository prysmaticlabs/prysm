...
func validateSelectionIndex(
	...
	d, err := core.Domain(bs.Fork(), epoch, domain, bs.GenesisValidatorRoot())
	if err != nil {
		return nil, err
	}
	sszUint := types.SSZUint64(data.Slot)
	root, err := core.ComputeSigningRoot(&sszUint, d)
	if err != nil {
		return nil, err
	}
...
func aggSigSet(s state.ReadOnlyBeaconState, a *ethpb.SignedAggregateAttestationAndProof) (*bls.SignatureSet, error) {
	...
	d, err := core.Domain(s.Fork(), epoch, params.BeaconConfig().DomainAggregateAndProof, s.GenesisValidatorRoot())
	if err != nil {
		return nil, err
	}
	root, err := core.ComputeSigningRoot(a.Message, d)
	if err != nil {
		return nil, err
	}
	return &bls.SignatureSet{
