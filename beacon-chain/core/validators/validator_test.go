...
func TestValidatorShard(t *testing.T) {
	...
	for i := 0; i < 21; i++ {
		validators = append(validators,
			&pb.ValidatorRecord{Pubkey: []byte{}, ExitSlot: params.BeaconConfig().FarFutureSlot})
	}
	shardCommittees := []*ShardCommittee{
...
func TestShardCommitteesAtSlot_OK(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if result[0].Shard != tt.expectedShard {
			t.Errorf(
...
func TestBoundaryAttesterIndices(t *testing.T) {
	...
	boundaryAttestations := []*pb.PendingAttestationRecord{
		{Data: &pb.AttestationData{Slot: 0}, ParticipationBitfield: []byte{0x10}}, // returns indices 242
		{Data: &pb.AttestationData{Slot: 0}, ParticipationBitfield: []byte{0xF0}}, // returns indices 237,224,2
	}
