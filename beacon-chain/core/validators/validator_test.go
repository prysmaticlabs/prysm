...
func TestVotedBalanceInAttestation(t *testing.T) {
	...
	newAttestation := &pb.AttestationRecord{
...
func TestAddValidators(t *testing.T) {
	...
	validators := AddPendingValidator(existingValidators, []byte{'A'}, []byte{'B'}, []byte{'C'}, uint64(params.PendingActivation))
	...
	validators = AddPendingValidator(existingValidators, []byte{'D'}, []byte{'E'}, []byte{'F'}, uint64(params.PendingActivation))
...
func TestChangeValidators(t *testing.T) {
	existingValidators := []*pb.ValidatorRecord{
		{Pubkey: []byte{1}, Status: uint64(params.PendingActivation), Balance: uint64(params.BeaconConfig().DepositSize * params.BeaconConfig().Gwei), LastStatusChangeSlot: params.BeaconConfig().MinWithdrawalPeriod},
		{Pubkey: []byte{2}, Status: uint64(params.PendingExit), Balance: uint64(params.BeaconConfig().DepositSize * params.BeaconConfig().Gwei), LastStatusChangeSlot: params.BeaconConfig().MinWithdrawalPeriod},
		{Pubkey: []byte{3}, Status: uint64(params.PendingActivation), Balance: uint64(params.BeaconConfig().DepositSize * params.BeaconConfig().Gwei), LastStatusChangeSlot: params.BeaconConfig().MinWithdrawalPeriod},
		{Pubkey: []byte{4}, Status: uint64(params.PendingExit), Balance: uint64(params.BeaconConfig().DepositSize * params.BeaconConfig().Gwei), LastStatusChangeSlot: params.BeaconConfig().MinWithdrawalPeriod},
		{Pubkey: []byte{5}, Status: uint64(params.PendingActivation), Balance: uint64(params.BeaconConfig().DepositSize * params.BeaconConfig().Gwei), LastStatusChangeSlot: params.BeaconConfig().MinWithdrawalPeriod},
		{Pubkey: []byte{6}, Status: uint64(params.PendingExit), Balance: uint64(params.BeaconConfig().DepositSize * params.BeaconConfig().Gwei), LastStatusChangeSlot: params.BeaconConfig().MinWithdrawalPeriod},
	...
	if validators[1].LastStatusChangeSlot != params.BeaconConfig().MinWithdrawalPeriod+1 {
		t.Errorf("Failed to set validator last status change slot")
	}
	...
	if validators[3].LastStatusChangeSlot != params.BeaconConfig().MinWithdrawalPeriod+1 {
		t.Errorf("Failed to set validator last status change slot")
	}
...
func TestValidatorMinDeposit(t *testing.T) {
	...
	if newValidators[2].LastStatusChangeSlot != currentSlot {
		t.Errorf("Validator's last status change slot should be %d got %d", currentSlot, newValidators[2].LastStatusChangeSlot)
	}
}
...
func TestDeepCopyValidators(t *testing.T) {
	...
	defaultValidator := &pb.ValidatorRecord{
		Pubkey:                []byte{'k', 'e', 'y'},
		WithdrawalCredentials: []byte{'a', 'd', 'd', 'r', 'e', 's', 's'},
		RandaoCommitment:      []byte{'r', 'a', 'n', 'd', 'a', 'o'},
		Balance:               uint64(1e9),
		Status:                uint64(params.Active),
		LastStatusChangeSlot:  10,
	}
	for i := 0; i < 100; i++ {
	...
	defaultValidator.WithdrawalCredentials = []byte{'n', 'e', 'w', 'a', 'd', 'd', 'r', 'e', 's', 's'}
	...
	defaultValidator.LastStatusChangeSlot = 5
	...
	for i, validator := range newValidatorSet {
		...
		if bytes.Equal(validator.WithdrawalCredentials, defaultValidator.WithdrawalCredentials) {
			t.Errorf("validator with index %d was unable to have their withdrawal credentials copied correctly %v", i, validator.WithdrawalCredentials)
		}
		...
		if validator.LastStatusChangeSlot == defaultValidator.LastStatusChangeSlot {
			t.Errorf("validator with index %d was unable to have their last status change slot copied correctly %d", i, validator.LastStatusChangeSlot)
		}
	}
