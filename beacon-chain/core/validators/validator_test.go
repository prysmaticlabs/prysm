...
func TestChangeValidatorRegistry(t *testing.T) {
	existingValidatorRegistry := []*pb.ValidatorRecord{
		...
		{Pubkey: []byte{9}, Status: uint64(params.ExitedWithPenalty), Balance: uint64(params.BeaconConfig().DepositSize * params.BeaconConfig().Gwei)},
		{Pubkey: []byte{10}, Status: uint64(params.ExitedWithPenalty), Balance: uint64(params.BeaconConfig().DepositSize * params.BeaconConfig().Gwei)},
