...
func BenchmarkExecuteStateTransition_FullBlock(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := coreState.ExecuteStateTransition(context.Background(), cleanStates[i], wrapper.WrappedPhase0SignedBeaconBlock(block), false)
		require.NoError(b, err)
	}
}
...
func BenchmarkExecuteStateTransition_WithCache(b *testing.B) {
	...
	_, err = coreState.ExecuteStateTransition(context.Background(), beaconState, wrapper.WrappedPhase0SignedBeaconBlock(block), false)
