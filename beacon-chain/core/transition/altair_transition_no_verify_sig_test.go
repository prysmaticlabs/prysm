...
func TestExecuteAltairStateTransitionNoVerify_FullProcess(t *testing.T) {
	...
	c := beaconState.Copy()
	sig, err := customtesting.BlockSignatureAltair(c, block.Block, privKeys)
	require.NoError(t, err)
...
func createFullAltairBlockWithOperations(t *testing.T) (state.BeaconStateAltair,
	*ethpb.SignedBeaconBlockAltair) {
	beaconState, privKeys := customtesting.DeterministicGenesisStateAltair(t, 32)
	...
	blk, err := customtesting.GenerateFullBlockAltair(tState, privKeys,
		&customtesting.BlockGenConfig{NumAttestations: 1, NumVoluntaryExits: 0, NumDeposits: 0}, 1)
	require.NoError(t, err)
