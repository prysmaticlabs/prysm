...
func TestExecuteStateTransition_FullProcess(t *testing.T) {
	...
	randaoReveal, err := customtesting.RandaoReveal(beaconState, epoch, privKeys)
	require.NoError(t, err)
	require.NoError(t, beaconState.SetSlot(beaconState.Slot()-1))
	...
	sig, err := customtesting.BlockSignature(beaconState, block.Block, privKeys)
	require.NoError(t, err)
...
func TestProcessBlock_IncorrectProcessExits(t *testing.T) {
	...
	blockAtt := customtesting.HydrateAttestation(&ethpb.Attestation{
	...
	require.NoError(t, err)
	err = beaconState.SetLatestBlockHeader(customtesting.HydrateBeaconHeader(&ethpb.BeaconBlockHeader{
...
func createFullBlockWithOperations(t *testing.T) (state.BeaconState,
	*ethpb.SignedBeaconBlock, []*ethpb.Attestation, []*ethpb.ProposerSlashing, []*ethpb.SignedVoluntaryExit) {
	beaconState, privKeys := customtesting.DeterministicGenesisState(t, 32)
	genesisBlock := blocks.NewGenesisBlock([]byte{})
	bodyRoot, err := genesisBlock.Block.HashTreeRoot()
	require.NoError(t, err)
	...
	header1 := customtesting.HydrateSignedBeaconHeader(&ethpb.SignedBeaconBlockHeader{
		Header: &ethpb.BeaconBlockHeader{
	...
	mockRoot2 := [32]byte{'A'}
	att1 := customtesting.HydrateIndexedAttestation(&ethpb.IndexedAttestation{
		Data: &ethpb.AttestationData{
	...
	mockRoot3 := [32]byte{'B'}
	att2 := customtesting.HydrateIndexedAttestation(&ethpb.IndexedAttestation{
		Data: &ethpb.AttestationData{
	...
	randaoReveal, err := customtesting.RandaoReveal(copied, currentEpoch, privKeys)
	require.NoError(t, err)
	proposerIndex, err := helpers.BeaconProposerIndex(copied)
	require.NoError(t, err)
	block := customtesting.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{
		Block: &ethpb.BeaconBlock{
...
func TestProcessSlots_ThroughAltairEpoch(t *testing.T) {
	...
	st, _ := customtesting.DeterministicGenesisState(t, params.BeaconConfig().MaxValidatorsPerCommittee)
