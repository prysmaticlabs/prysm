...
func TestSkipSlotCache_OK(t *testing.T) {
	...
	require.NoError(t, err)
	executedState, err := transition.ExecuteStateTransition(context.Background(), originalState, wrapper.WrappedPhase0SignedBeaconBlock(blk), false)
	require.NoError(t, err, "Could not run state transition")
	originalState, ok := executedState.(*v1.BeaconState)
	require.Equal(t, true, ok)
	bState, err = transition.ExecuteStateTransition(context.Background(), bState, wrapper.WrappedPhase0SignedBeaconBlock(blk), false)
...
func TestSkipSlotCache_ConcurrentMixup(t *testing.T) {
	...
	var s1, s0 state.BeaconState
	{
		...
		s1, err = transition.ExecuteStateTransition(context.Background(), originalState.Copy(), wrapper.WrappedPhase0SignedBeaconBlock(blk), false)
		require.NoError(t, err, "Could not run state transition")
	}

	{
		...
		require.NoError(t, err)
		blk.Signature = signature.Marshal()
		s0, err = transition.ExecuteStateTransition(context.Background(), originalState.Copy(), wrapper.WrappedPhase0SignedBeaconBlock(blk), false)
		require.NoError(t, err, "Could not run state transition")
	}
	...
	expected1, err := transition.ProcessSlots(context.Background(), s1.Copy(), problemSlot, false)
	require.NoError(t, err)
	...
	tmp1, err := transition.ProcessSlots(context.Background(), expected1.Copy(), problemSlot+1, false)
	require.NoError(t, err)
	...
	expected2, err := transition.ProcessSlots(context.Background(), s0.Copy(), problemSlot, false)
	require.NoError(t, err)
	...
	tmp2, err := transition.ProcessSlots(context.Background(), expected2.Copy(), problemSlot+1, false)
	require.NoError(t, err)
	...
	step := func(i int, setup state.BeaconState) {
		...
		outState, err := transition.ProcessSlots(context.Background(), setup, problemSlot.Add(1+uint64(i)), false) // keep increasing, to hit and extend the cache
		require.NoError(t, err, "Could not process state transition")
