...
func ShardProposerIndex(beaconState *stateTrie.BeaconState, slot uint64, shard uint64) (uint64, error) {
	shardCommittee, err := ShardCommittee(beaconState, SlotToEpoch(slot), shard)
	if err != nil {
		return 0, err
	}

	seed, err := Seed(beaconState, CurrentEpoch(beaconState), params.BeaconConfig().DomainShardCommittee) // TODO: Double check domain
	if err != nil {
		return 0, err
	}
	seedWithSlot := append(seed[:], bytesutil.Bytes8(slot)...)
	seedWithSlotHash := hashutil.Hash(seedWithSlot)
	return computeProposerIndexPhase1(beaconState, shardCommittee, seedWithSlotHash, 0) // TODO: Properly calculate min effective balance
}

// computeProposerIndexPhase1 returns the proposer index sampled by effective balance.
...
//        i += 1
func computeProposerIndexPhase1(bState *stateTrie.BeaconState, activeIndices []uint64, seed [32]byte, minEffectiveBalance uint64) (uint64, error) {
	length := uint64(len(activeIndices))
	if length == 0 {
		return 0, errors.New("empty active indices list")
	}
	...
	for i := uint64(0); ; i++ {
		...
		if effectiveBal*maxRandomByte >= params.BeaconConfig().MaxEffectiveBalance*uint64(randomByte) && effectiveBal > minEffectiveBalance {
			return candidateIndex, nil
		}
	}
}
