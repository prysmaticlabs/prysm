...
func IsActiveValidatorUsingTrie(validator interfaces.ReadOnlyValidator, epoch types.Epoch) bool {
	return checkValidatorActiveStatus(validator.ActivationEpoch(), validator.ExitEpoch(), epoch)
}
...
func IsSlashableValidatorUsingTrie(val interfaces.ReadOnlyValidator, epoch types.Epoch) bool {
...
func ActiveValidatorIndices(state interfaces.ReadOnlyBeaconState, epoch types.Epoch) ([]types.ValidatorIndex, error) {
...
// at the given epoch.
func ActiveValidatorCount(state interfaces.ReadOnlyBeaconState, epoch types.Epoch) (uint64, error) {
	seed, err := Seed(state, epoch, params.BeaconConfig().DomainBeaconAttester)
	if err != nil {
...
func BeaconProposerIndex(state interfaces.ReadOnlyBeaconState) (types.ValidatorIndex, error) {
...
//        i += 1
func ComputeProposerIndex(bState interfaces.ReadOnlyValidators, activeIndices []types.ValidatorIndex, seed [32]byte) (types.ValidatorIndex, error) {
...
// be placed into the activation queue.
func IsEligibleForActivationQueueUsingTrie(validator interfaces.ReadOnlyValidator) bool {
	return isEligibileForActivationQueue(validator.ActivationEligibilityEpoch(), validator.EffectiveBalance())
}
...
//    )
func IsEligibleForActivation(state interfaces.ReadOnlyCheckpoint, validator *ethpb.Validator) bool {
...
func IsEligibleForActivationUsingTrie(state interfaces.ReadOnlyCheckpoint, validator interfaces.ReadOnlyValidator) bool {
