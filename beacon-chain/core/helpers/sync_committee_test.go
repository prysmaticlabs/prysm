...
func TestIsCurrentEpochSyncCommittee_UsingCache(t *testing.T) {
	...
	for i := 0; i < len(validators); i++ {
		...
		syncCommittee.Pubkeys = append(syncCommittee.Pubkeys, bytes.PadTo(k, 48))
	}
...
func TestCurrentEpochSyncSubcommitteeIndices_UsingCommittee(t *testing.T) {
	...
	_, err = syncCommitteeCache.CurrentPeriodIndexPosition(bytes.ToBytes32(root), 0)
	...
	index, err = syncCommitteeCache.CurrentPeriodIndexPosition(bytes.ToBytes32(root), 0)
	require.NoError(t, err)
	require.DeepEqual(t, []types.CommitteeIndex{0}, index)
}
