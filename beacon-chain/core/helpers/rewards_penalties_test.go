...
func TestTotalBalance_OK(t *testing.T) {
	state, err := state.InitializeFromProtoPhase0(&ethpb.BeaconState{Validators: []*ethpb.Validator{
...
func TestGetBalance_OK(t *testing.T) {
	...
	for _, test := range tests {
		state, err := state.InitializeFromProtoPhase0(&ethpb.BeaconState{Balances: test.b})
		require.NoError(t, err)
...
func TestTotalActiveBalance(t *testing.T) {
	...
	for _, test := range tests {
		...
		state, err := state.InitializeFromProtoPhase0(&ethpb.BeaconState{Validators: validators})
		require.NoError(t, err)
