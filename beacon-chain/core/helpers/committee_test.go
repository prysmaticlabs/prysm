...
func TestCommitteeAssignment_CanRetrieve(t *testing.T) {
	...
	tests := []struct {
		...
		{
			index:      0,
			slot:       params.BeaconConfig().GenesisSlot + 134,
		...
		},
		{
			...
			committee:  []uint64{61, 105},
			shard:      57,
			isProposer: true,
		},
		{
			index:      64,
			slot:       params.BeaconConfig().GenesisSlot + 181,
			committee:  []uint64{64, 1},
			shard:      53,
			isProposer: false,
		},
		{
			index:      11,
			slot:       params.BeaconConfig().GenesisSlot + 149,
			committee:  []uint64{11, 43},
			shard:      21,
			isProposer: false,
		},
	}
