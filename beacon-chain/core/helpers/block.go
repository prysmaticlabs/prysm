...
func BlockRootAtSlot(state types.ReadOnlyBeaconState, slot primitives.Slot) ([]byte, error) {
...
func StateRootAtSlot(state types.ReadOnlyBeaconState, slot primitives.Slot) ([]byte, error) {
	if slot >= state.Slot() || state.Slot() > slot+params.BeaconConfig().SlotsPerHistoricalRoot {
		return []byte{}, errors.Errorf("slot %d out of bounds", slot)
	}
...
func BlockRoot(state types.ReadOnlyBeaconState, epoch primitives.Epoch) ([]byte, error) {
