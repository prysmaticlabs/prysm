...
func BlockRootAtSlot(state interfaces.ReadOnlyBeaconState, slot types.Slot) ([]byte, error) {
...
func StateRootAtSlot(state interfaces.ReadOnlyBeaconState, slot types.Slot) ([]byte, error) {
	if slot >= state.Slot() || state.Slot() > slot+params.BeaconConfig().SlotsPerHistoricalRoot {
		return []byte{}, errors.Errorf("slot %d out of bounds", slot)
	}
...
func BlockRoot(state interfaces.ReadOnlyBeaconState, epoch types.Epoch) ([]byte, error) {
