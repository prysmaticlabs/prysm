...
func TestSourceEpoch(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "current source",
			args: args{epoch: 0, period: 4},
			want: 0,
		},
		{
			name: "current source again",
			args: args{epoch: 5, period: 4},
			want: 0,
		},
		{
			name: "next source",
			args: args{epoch: 8, period: 4},
			want: 4,
		},
		{
			name: "next next source",
			args: args{epoch: 13, period: 4},
			want: 8,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := SourceEpoch(tt.args.epoch, tt.args.period); got != tt.want {
				t.Errorf("SourceEpoch() = %v, want %v", got, tt.want)
			}
		})
	}
}
