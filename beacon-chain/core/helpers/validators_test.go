...
func TestIsActiveValidatorUsingTrie_OK(t *testing.T) {
	...
	beaconState, err := state.InitializeFromProtoPhase0(&ethpb.BeaconState{Validators: []*ethpb.Validator{val}})
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			t.Run("with trie", func(t *testing.T) {
				beaconState, err := state.InitializeFromProtoPhase0(&ethpb.BeaconState{Validators: []*ethpb.Validator{test.validator}})
				require.NoError(t, err)
				readOnlyVal, err := beaconState.ValidatorAtIndexReadOnly(0)
				require.NoError(t, err)
...
func TestActiveValidatorIndices(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s, err := state.InitializeFromProtoPhase0(tt.args.state)
			require.NoError(t, err)
...
func TestComputeProposerIndex(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			stTrie, err := state.InitializeFromProtoUnsafePhase0(bState)
			require.NoError(t, err)
