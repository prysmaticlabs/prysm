...
func TestUpgradeToBellatrix(t *testing.T) {
	...
	pfsJustificationBits, err := preForkState.JustificationBits()
	require.NoError(t, err)
	mJustificationBits, err := mSt.JustificationBits()
	require.NoError(t, err)
	require.DeepSSZEqual(t, pfsJustificationBits, mJustificationBits)
