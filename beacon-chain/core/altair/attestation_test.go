...
func TestProcessAttestations_InclusionDelayFailure(t *testing.T) {
	attestations := []*ethpb.Attestation{
		customtesting.HydrateAttestation(&ethpb.Attestation{
	...
	beaconState, _ := customtesting.DeterministicGenesisStateAltair(t, 100)
...
func TestProcessAttestations_OK(t *testing.T) {
	beaconState, privKeys := customtesting.DeterministicGenesisStateAltair(t, 100)
...
func TestProcessAttestationNoVerify_SourceTargetHead(t *testing.T) {
	beaconState, _ := customtesting.DeterministicGenesisStateAltair(t, 64)
	err := beaconState.SetSlot(beaconState.Slot() + params.BeaconConfig().MinAttestationInclusionDelay)
	require.NoError(t, err)
...
func TestSetParticipationAndRewardProposer(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			beaconState, _ := customtesting.DeterministicGenesisStateAltair(t, params.BeaconConfig().MaxValidatorsPerCommittee)
