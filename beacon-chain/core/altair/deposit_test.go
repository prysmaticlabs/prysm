...
func TestProcessDeposits_SameValidatorMultipleDepositsSameBlock(t *testing.T) {
	...
	dep, _, err := customtesting.DeterministicDepositsAndKeysSameValidator(3)
	require.NoError(t, err)
	eth1Data, err := customtesting.DeterministicEth1Data(len(dep))
	require.NoError(t, err)
...
func TestProcessDeposits_AddsNewValidatorDeposit(t *testing.T) {
	dep, _, err := customtesting.DeterministicDepositsAndKeys(1)
	require.NoError(t, err)
	eth1Data, err := customtesting.DeterministicEth1Data(len(dep))
	require.NoError(t, err)
...
func TestProcessDeposit_SkipsInvalidDeposit(t *testing.T) {
	...
	trie, _, err := customtesting.DepositTrieFromDeposits(dep)
	require.NoError(t, err)
