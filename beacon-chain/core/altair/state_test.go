...
func TestGenesisBeaconState_OK(t *testing.T) {
	...
	require.NoError(t, err)
	newState, err := altair.GenesisBeaconState(context.Background(), deposits, genesisTime, eth1Data)
...
func TestGenesisState_HashEquality(t *testing.T) {
	...
	require.NoError(t, err)
	state1, err := altair.GenesisBeaconState(context.Background(), deposits, 0, &ethpb.Eth1Data{BlockHash: make([]byte, 32)})
	require.NoError(t, err)
	state2, err := altair.GenesisBeaconState(context.Background(), deposits, 0, &ethpb.Eth1Data{BlockHash: make([]byte, 32)})
	require.NoError(t, err)

	pbState1, err := stateAltair.ProtobufBeaconState(state1.CloneInnerState())
	require.NoError(t, err)
	pbState2, err := stateAltair.ProtobufBeaconState(state2.CloneInnerState())
	require.NoError(t, err)
...
func TestGenesisState_InitializesLatestBlockHashes(t *testing.T) {
	s, err := altair.GenesisBeaconState(context.Background(), nil, 0, &ethpb.Eth1Data{})
	require.NoError(t, err)
...
func TestGenesisState_FailsWithoutEth1data(t *testing.T) {
	_, err := altair.GenesisBeaconState(context.Background(), nil, 0, nil)
