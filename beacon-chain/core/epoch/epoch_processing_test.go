...
func TestProcessLightClientCommitteeUpdates_CanRotate(t *testing.T) {
	getState := func(slot types.Slot) *state.BeaconState {
	...
	postState, err = epoch.ProcessLightClientCommitteeUpdates(getState(types.Slot(params.BeaconConfig().EpochsPerSyncCommitteePeriod - 1)))
	require.NoError(t, err)
