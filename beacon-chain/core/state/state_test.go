...
func TestGenesisBeaconState_OK(t *testing.T) {
	...
	assert.Equal(t, uint64(65536), uint64(params.BeaconConfig().EpochsPerHistoricalVector), "EpochsPerHistoricalVector should be 8192 for these tests to pass")

	latestRandaoMixesLength := uint64(params.BeaconConfig().EpochsPerHistoricalVector)
	...
	assert.Equal(t, uint64(8192), uint64(params.BeaconConfig().EpochsPerSlashingsVector), "EpochsPerSlashingsVector should be 8192 for these tests to pass")
