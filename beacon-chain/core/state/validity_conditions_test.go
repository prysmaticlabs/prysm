...
func TestGenesisBlock(t *testing.T) {
	stateRoot := [32]byte{0}
	b1 := types.NewGenesisBlock(stateRoot)
	b2 := types.NewGenesisBlock(stateRoot)
	...
	if b1.AncestorHashes() == nil {
		t.Fatal("genesis block missing ParentHash field")
	}

	if b1.StateRoot() != stateRoot {
		t.Fatal("genesis block state root isn't initialized correctly")
	}

	b3 := types.NewBlock(nil)
	h3, err3 := b3.Hash()
	if err3 != nil {
