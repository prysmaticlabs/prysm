...
func createFullBlockWithOperations(t *testing.T) (interfaces.BeaconState,
...
func TestProcessBlock_OverMaxProposerSlashings(t *testing.T) {
	...
	_, err := state.VerifyOperationLengths(context.Background(), &v1.BeaconState{}, interfaces.WrappedPhase0SignedBeaconBlock(b))
	assert.ErrorContains(t, want, err)
}
