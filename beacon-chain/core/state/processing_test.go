...
func TestLatestCrosslinks(t *testing.T) {
	...
	if !bytes.Equal(crossLinks[1].GetShardBlockRootHash32(), []byte{'B'}) {
		t.Errorf("shard blockhash not saved in crosslink record %v", crossLinks[1].GetShardBlockRootHash32())
	}
