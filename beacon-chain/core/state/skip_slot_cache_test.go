...
func TestSkipSlotCache_OK(t *testing.T) {
	...
	require.NoError(t, err)
	executedState, err := state.ExecuteStateTransition(context.Background(), originalState, blockInterface.WrappedPhase0SignedBeaconBlock(blk))
	require.NoError(t, err, "Could not run state transition")
	originalState, ok := executedState.(*v1.BeaconState)
	require.Equal(t, true, ok)
	bState, err = state.ExecuteStateTransition(context.Background(), bState, blockInterface.WrappedPhase0SignedBeaconBlock(blk))
...
func TestSkipSlotCache_ConcurrentMixup(t *testing.T) {
	...
	var state1, state2 iface.BeaconState
	{
		...
		state1, err = state.ExecuteStateTransition(context.Background(), originalState.Copy(), blockInterface.WrappedPhase0SignedBeaconBlock(blk))
		require.NoError(t, err, "Could not run state transition")
	}

	{
		...
		require.NoError(t, err)
		blk.Signature = signature.Marshal()
		state2, err = state.ExecuteStateTransition(context.Background(), originalState.Copy(), blockInterface.WrappedPhase0SignedBeaconBlock(blk))
		require.NoError(t, err, "Could not run state transition")
	}
