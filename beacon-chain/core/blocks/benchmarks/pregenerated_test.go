...
func BenchmarkGenerateMarshalledFullStateAndBlock(b *testing.B) {
	...
	block := testutil.GenerateFullBlock(b, beaconState, privs, conf, params.BeaconConfig().SlotsPerEpoch+2)
...
func BenchmarkGenerate2FullEpochState(b *testing.B) {
	...
	b.Logf("Atts in current epoch %d\n", len(beaconState.CurrentEpochAttestations))
	b.Logf("Atts in prev epoch %d\n", len(beaconState.PreviousEpochAttestations))

	beaconBytes, err := proto.Marshal(beaconState)
	if err != nil {
		b.Fatal(err)
	}
	if err := ioutil.WriteFile("beaconState2FullEpochs.bytes", beaconBytes, 0644); err != nil {
		b.Fatal(err)
	}
}
...
func beaconState2FullEpochs(b testing.TB) *pb.BeaconState {
	beaconBytes, err := ioutil.ReadFile("beaconState2FullEpochs.bytes")
	if err != nil {
		b.Fatal(err)
	}
	beaconState := &pb.BeaconState{}
