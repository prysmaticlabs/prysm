...
func TestVerifyAttestationNoVerifySignature_IncorrectSlotTargetEpoch(t *testing.T) {
	beaconState, _ := customtesting.DeterministicGenesisState(t, 1)
...
func TestConvertToIndexed_OK(t *testing.T) {
	...
	var sig [96]byte
	copy(sig[:], "signed")
	attestation := customtesting.HydrateAttestation(&ethpb.Attestation{
		Signature: sig[:],
	})
...
func TestVerifyIndexedAttestation_OK(t *testing.T) {
	...
	_, keys, err := customtesting.DeterministicDepositsAndKeys(numOfValidators)
	require.NoError(t, err)
	for i := 0; i < len(validators); i++ {
		validators[i] = &ethpb.Validator{
			ExitEpoch:             params.BeaconConfig().FarFutureEpoch,
...
func TestValidateIndexedAttestation_BadAttestationsSignatureSet(t *testing.T) {
	beaconState, keys := customtesting.DeterministicGenesisState(t, 128)
...
func TestVerifyAttestations_HandlesPlannedFork(t *testing.T) {
	...
	require.NoError(t, err)
	att1 := customtesting.HydrateAttestation(&ethpb.Attestation{
	...
	require.NoError(t, err)
	att2 := customtesting.HydrateAttestation(&ethpb.Attestation{
