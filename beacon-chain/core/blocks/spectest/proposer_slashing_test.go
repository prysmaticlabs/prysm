...
func runProposerSlashingTest(t *testing.T, config string) {
	testsFolderPath := path.Join("tests", config, "phase0/operations/proposer_slashing/pyspec_tests")
	...
	for _, folder := range testFolders {
		...
		t.Run(folder.Name(), func(t *testing.T) {
			...
			beaconState, err := blocks.ProcessProposerSlashings(beaconState, body)
			if postSSZExists {
				if err != nil {
