...
func runVoluntaryExitTest(t *testing.T, config string) {
	testsFolderPath := path.Join("tests", config, "phase0/operations/voluntary_exit/pyspec_tests")
	...
	for _, folder := range testFolders {
		...
		t.Run(folder.Name(), func(t *testing.T) {
			...
			body := &ethpb.BeaconBlockBody{
				VoluntaryExits: []*ethpb.VoluntaryExit{voluntaryExit},
			}
			beaconState, err := blocks.ProcessVoluntaryExits(beaconState, body)
			if postSSZExists {
				if err != nil {
