...
func TestProcessProposerSlashings_ValidatorNotSlashable(t *testing.T) {
	...
	want := fmt.Sprintf(
		...
		butil.ToBytes48(beaconState.Validators()[0].PublicKey),
	)
	_, err = blocks.ProcessProposerSlashings(context.Background(), beaconState, b.Block.Body.ProposerSlashings, v.SlashValidator)
	assert.ErrorContains(t, want, err)
}
				...
				slashing: &ethpb.ProposerSlashing{
					Header_1: &ethpb.SignedBeaconBlockHeader{
						Header: &ethpb.BeaconBlockHeader{
							ProposerIndex: 1,
							Slot:          65,
							StateRoot:     butil.PadTo([]byte{}, 32),
							BodyRoot:      butil.PadTo([]byte{}, 32),
							ParentRoot:    butil.PadTo([]byte("foo"), 32),
						},
					},
					Header_2: &ethpb.SignedBeaconBlockHeader{
						Header: &ethpb.BeaconBlockHeader{
							...
							StateRoot:     butil.PadTo([]byte{}, 32),
							BodyRoot:      butil.PadTo([]byte{}, 32),
							ParentRoot:    butil.PadTo([]byte("bar"), 32),
						},
					},
				},
				beaconState: beaconState,
