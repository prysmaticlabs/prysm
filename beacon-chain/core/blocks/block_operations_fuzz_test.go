...
func TestFuzzProcessBlockHeader_10000(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		...
		s, err := v1.InitializeFromProtoUnsafe(state)
		require.NoError(t, err)
		_, err = ProcessBlockHeader(context.Background(), s, blockInterface.WrappedPhase0SignedBeaconBlock(block))
		_ = err
	}
}
...
func TestFuzzProcessRandao_10000(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		...
		s, err := v1.InitializeFromProtoUnsafe(state)
		require.NoError(t, err)
		r, err := ProcessRandao(context.Background(), s, blockInterface.WrappedPhase0SignedBeaconBlock(b))
		if err != nil && r != nil {
...
func TestFuzzProcessAttestations_10000(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		...
		s, err := v1.InitializeFromProtoUnsafe(state)
		require.NoError(t, err)
		r, err := ProcessAttestations(ctx, s, blockInterface.WrappedPhase0SignedBeaconBlock(b))
		if err != nil && r != nil {
			t.Fatalf("return value should be nil on err. found: %v on error: %v for state: %v and block: %v", r, err, state, b)
		}
	}
...
func TestFuzzProcessAttestationsNoVerify_10000(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		...
		s, err := v1.InitializeFromProtoUnsafe(state)
		require.NoError(t, err)
		r, err := ProcessAttestationsNoVerifySignature(ctx, s, blockInterface.WrappedPhase0SignedBeaconBlock(b))
		if err != nil && r != nil {
			t.Fatalf("return value should be nil on err. found: %v on error: %v for state: %v and block: %v", r, err, state, b)
		}
	}
