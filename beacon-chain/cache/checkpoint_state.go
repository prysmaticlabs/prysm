...
func (c *CheckpointStateCache) StateByCheckpoint(cp *ethpb.Checkpoint) (interfaces.BeaconState, error) {
	c.lock.RLock()
	...
	if exists && item != nil {
		...
		return item.(interfaces.BeaconState), nil
	}
...
func (c *CheckpointStateCache) AddCheckpointState(cp *ethpb.Checkpoint, s interfaces.ReadOnlyBeaconState) error {
	c.lock.Lock()
	defer c.lock.Unlock()
	h, err := hashutil.HashProto(cp)
	if err != nil {
