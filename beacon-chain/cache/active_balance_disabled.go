...
func (c *FakeBalanceCache) AddTotalEffectiveBalance(st types.ReadOnlyBeaconState, balance uint64) error {
	return nil
}

// Get returns the current epoch's effective balance for state `st` in cache.
func (c *FakeBalanceCache) Get(st types.ReadOnlyBeaconState) (uint64, error) {
	return 0, nil
}
