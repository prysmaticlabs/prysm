...
func (c *SkipSlotCache) Get(ctx context.Context, r [32]byte) (interfaces.BeaconState, error) {
	...
	if exists && item != nil {
		...
		return item.(interfaces.BeaconState).Copy(), nil
	}
	skipSlotCacheMiss.Inc()
...
func (c *SkipSlotCache) Put(_ context.Context, r [32]byte, state interfaces.BeaconState) error {
	if c.disabled {
		return nil
	}
