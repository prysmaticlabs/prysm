...
func TestPendingAttestations_FiltersWithinInclusionDelay(t *testing.T) {
	...
	beaconState := &pbp2p.BeaconState{
		Slot:             params.BeaconConfig().MinAttestationInclusionDelay + 100,
		LatestCrosslinks: []*pbp2p.Crosslink{{Epoch: 1, CrosslinkDataRootHash32: params.BeaconConfig().ZeroHash[:]}},
	}
	proposerServer := &ProposerServer{
...
func TestPendingAttestations_FiltersExpiredAttestations(t *testing.T) {
	...
	opService := &mockOperationService{
		pendingAttestations: []*pbp2p.Attestation{
			...
			{Data: &pbp2p.AttestationData{Slot: 0, JustifiedEpoch: expectedEpoch, CrosslinkDataRoot: params.BeaconConfig().ZeroHash[:]}},
			{Data: &pbp2p.AttestationData{Slot: currentSlot - 10000, JustifiedEpoch: expectedEpoch, CrosslinkDataRoot: params.BeaconConfig().ZeroHash[:]}},
			{Data: &pbp2p.AttestationData{Slot: currentSlot - 5000, JustifiedEpoch: expectedEpoch, CrosslinkDataRoot: params.BeaconConfig().ZeroHash[:]}},
			{Data: &pbp2p.AttestationData{Slot: currentSlot - 100, JustifiedEpoch: expectedEpoch, CrosslinkDataRoot: params.BeaconConfig().ZeroHash[:]}},
			{Data: &pbp2p.AttestationData{Slot: currentSlot - params.BeaconConfig().SlotsPerEpoch, JustifiedEpoch: expectedEpoch, CrosslinkDataRoot: params.BeaconConfig().ZeroHash[:]}},
			// Non-expired attestation with incorrect justified epoch
			{Data: &pbp2p.AttestationData{Slot: currentSlot - 5, JustifiedEpoch: expectedEpoch - 1, CrosslinkDataRoot: params.BeaconConfig().ZeroHash[:]}},
			// Non-expired attestations with correct justified epoch
			{Data: &pbp2p.AttestationData{Slot: currentSlot - 5, JustifiedEpoch: expectedEpoch, CrosslinkDataRoot: params.BeaconConfig().ZeroHash[:]}},
			{Data: &pbp2p.AttestationData{Slot: currentSlot - 2, JustifiedEpoch: expectedEpoch, CrosslinkDataRoot: params.BeaconConfig().ZeroHash[:]}},
			{Data: &pbp2p.AttestationData{Slot: currentSlot, JustifiedEpoch: expectedEpoch, CrosslinkDataRoot: params.BeaconConfig().ZeroHash[:]}},
		},
	}
	...
	beaconState := &pbp2p.BeaconState{
		...
		LatestCrosslinks:       []*pbp2p.Crosslink{{Epoch: 9, CrosslinkDataRootHash32: params.BeaconConfig().ZeroHash[:]}},
	}
	if err := db.SaveState(ctx, beaconState); err != nil {
		t.Fatal(err)
	}
