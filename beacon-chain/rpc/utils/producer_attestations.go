...
type producerAtts []*ethpb.Attestation

// filter separates attestation list into two groups: valid and invalid attestations.
// The first group passes the all the required checks for attestation to be considered for proposing.
// And attestations from the second group should be deleted.
func (a producerAtts) filter(ctx context.Context, state iface.BeaconState) (producerAtts, producerAtts) {
...
func (a producerAtts) sortByProfitability() (producerAtts, error) {
	if len(a) < 2 {
		return a, nil
	}
...
func (a producerAtts) sortByProfitabilityUsingMaxCover() (producerAtts, error) {
	// Separate attestations by slot, as slot number takes higher precedence when sorting.
	var slots []types.Slot
	attsBySlot := map[types.Slot]producerAtts{}
	for _, att := range a {
	...
	selectAtts := func(atts producerAtts) (producerAtts, error) {
		if len(atts) < 2 {
			return atts, nil
		}
		...
		if err == nil {
			...
			selectedAtts := make(producerAtts, selectedKeys.Count())
			leftoverAtts := make(producerAtts, selectedKeys.Not().Count())
...
func (a producerAtts) limitToMaxAttestations() producerAtts {
...
func (a producerAtts) dedup() (producerAtts, error) {
	if len(a) < 2 {
		return a, nil
	}
