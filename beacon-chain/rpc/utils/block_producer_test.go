...
func TestProducer_ProduceBlock_OK(t *testing.T) {
	...
	for i := types.ValidatorIndex(0); uint64(i) < params.BeaconConfig().MaxProposerSlashings; i++ {
		...
		err = producer.SlashingsPool.InsertProposerSlashing(context.Background(), beaconState, proposerSlashing)
		require.NoError(t, err)
	}
	...
	for i := uint64(0); i < params.BeaconConfig().MaxAttesterSlashings; i++ {
		...
		err = producer.SlashingsPool.InsertAttesterSlashing(context.Background(), beaconState, attesterSlashing)
		require.NoError(t, err)
	}

	graffiti32 := bytesutil.ToBytes32([]byte("eth2"))
	graffiti := graffiti32[:]
	slot := types.Slot(1)
	...
	assert.DeepEqual(t, graffiti, block.Body.Graffiti, "Expected block to have correct graffiti")
...
func TestProducer_ProduceBlock_AddsUnaggregatedAtts(t *testing.T) {
	...
	require.NoError(t, producer.AttPool.SaveAggregatedAttestations(atts))
	...
	for _, a := range atts {
		if !helpers.IsAggregated(a) {
			found = true
			require.NoError(t, producer.AttPool.SaveUnaggregatedAttestation(a))
		}
	}
	...
	block, err := producer.ProduceBlock(ctx, slot, randaoReveal, graffiti)
	require.NoError(t, err)
...
func TestProducer_ComputeStateRoot_OK(t *testing.T) {
	db := dbutil.SetupDB(t)
	ctx := context.Background()
	...
	_, err = producer.computeStateRoot(context.Background(), wrapper.WrappedPhase0SignedBeaconBlock(req))
	require.NoError(t, err)
}

func TestProducer_PendingDeposits_Eth1DataVoteOK(t *testing.T) {
	ctx := context.Background()
	...
	_, eth1Height, err := producer.canonicalEth1Data(ctx, beaconState, &ethpb.Eth1Data{})
	require.NoError(t, err)
	...
	_, eth1Height, err = producer.canonicalEth1Data(ctx, beaconState, vote)
	require.NoError(t, err)
...
func TestProducer_PendingDeposits_OutsideEth1FollowWindow(t *testing.T) {
	ctx := context.Background()
	...
	deposits, err := producer.deposits(ctx, beaconState, &ethpb.Eth1Data{})
	require.NoError(t, err)
...
func TestProducer_PendingDeposits_FollowsCorrectEth1Block(t *testing.T) {
	ctx := context.Background()
	...
	deposits, err = producer.deposits(ctx, beaconState, vote)
	require.NoError(t, err)
	assert.Equal(t, len(recentDeposits), len(deposits), "Received unexpected number of pending deposits")
}

func TestProducer_PendingDeposits_CantReturnBelowStateEth1DepositIndex(t *testing.T) {
	ctx := context.Background()
	height := big.NewInt(int64(params.BeaconConfig().Eth1FollowDistance))
	p := &mockPOW.POWChain{
		LatestBlockNumber: height,
	...
	for _, dp := range append(readyDeposits, recentDeposits...) {
		...
		depositTrie.Insert(depositHash[:], int(dp.Index))
		assert.NoError(t, depositCache.InsertDeposit(ctx, dp.Deposit, uint64(dp.Index), dp.Index, depositTrie.Root()))
	}
	for _, dp := range recentDeposits {
...
func TestProducer_PendingDeposits_CantReturnMoreThanMax(t *testing.T) {
	ctx := context.Background()
	...
	for _, dp := range append(readyDeposits, recentDeposits...) {
		...
		depositTrie.Insert(depositHash[:], int(dp.Index))
		assert.NoError(t, depositCache.InsertDeposit(ctx, dp.Deposit, height.Uint64(), dp.Index, depositTrie.Root()))
	}
	for _, dp := range recentDeposits {
...
func TestProducer_PendingDeposits_CantReturnMoreThanDepositCount(t *testing.T) {
	ctx := context.Background()
...
func TestProducer_DepositTrie_UtilizesCachedFinalizedDeposits(t *testing.T) {
	ctx := context.Background()
	...
	trie, err := producer.depositTrie(ctx, &ethpb.Eth1Data{}, big.NewInt(int64(params.BeaconConfig().Eth1FollowDistance)))
	require.NoError(t, err)
...
func TestProducer_DepositTrie_RebuildTrie(t *testing.T) {
	ctx := context.Background()
...
func TestProducer_ValidateDepositTrie(t *testing.T) {
	tt := []struct {
		name            string
	...
	for _, test := range tt {
		t.Run(test.name, func(t *testing.T) {
			producer := &BlockProvider{}
			valid, err := producer.validateDepositTrie(test.trieCreator(), test.eth1dataCreator())
...
func TestProducer_Eth1Data_MajorityVote(t *testing.T) {
	slot := types.Slot(64)
	...
	depositCache, err := depositcache.New()
	require.NoError(t, err)
	assert.NoError(t, depositCache.InsertDeposit(context.Background(), dc.Deposit, dc.Eth1BlockHeight, dc.Index, depositTrie.Root()))
	...
	t.Run("choose highest count", func(t *testing.T) {
		...
		ctx := context.Background()
		majorityVoteEth1Data, err := producer.eth1DataMajorityVote(ctx, beaconState)
		require.NoError(t, err)
...
func TestProducer_FilterAttestation(t *testing.T) {
	params.SetupTestConfigCleanup(t)
	params.OverrideBeaconConfig(params.MainnetConfig())
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			atts := tt.inputAtts()
			received, err := producer.filterAttestationsForBlockInclusion(context.Background(), state, atts)
			if tt.wantedErr != "" {
...
func TestProducer_Deposits_ReturnsEmptyList_IfLatestEth1DataEqGenesisEth1Block(t *testing.T) {
	ctx := context.Background()
...
func TestProducer_DeleteAttsInPool_Aggregated(t *testing.T) {
	producer := &BlockProvider{
		AttPool: attestations.NewPool(),
	}
	...
	require.NoError(t, producer.AttPool.SaveAggregatedAttestations(aggregatedAtts))
	require.NoError(t, producer.AttPool.SaveUnaggregatedAttestations(unaggregatedAtts))

	aa, err := attaggregation.Aggregate(aggregatedAtts)
	require.NoError(t, err)
	require.NoError(t, producer.deleteAttsInPool(context.Background(), append(aa, unaggregatedAtts...)))
	assert.Equal(t, 0, len(producer.AttPool.AggregatedAttestations()), "Did not delete aggregated attestation")
	atts, err := producer.AttPool.UnaggregatedAttestations()
	require.NoError(t, err)
