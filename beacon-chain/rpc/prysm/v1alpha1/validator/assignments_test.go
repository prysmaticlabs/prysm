...
func TestGetAltairDuties_SyncCommitteeOK(t *testing.T) {
	...
	h := &ethpb.BeaconBlockHeader{
		...
		ParentRoot: butil.PadTo([]byte{'b'}, 32),
		BodyRoot:   butil.PadTo([]byte{'c'}, 32),
	}
	...
	for i, pk := range pubKeys {
		pubkeysAs48ByteType[i] = butil.ToBytes48(pk)
	}
...
func TestGetDuties_CurrentEpoch_ShouldNotFail(t *testing.T) {
	...
	indices := make([]uint64, len(deposits))
	for i := 0; i < len(deposits); i++ {
		pubKeys[i] = butil.ToBytes48(deposits[i].Data.PublicKey)
		indices[i] = uint64(i)
	}
