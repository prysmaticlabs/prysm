...
func TestGetDuties_OK(t *testing.T) {
	...
	require.NoError(t, err)
	bs, err := transition.GenesisBeaconState(context.Background(), deposits, 0, eth1Data, false)
...
func TestGetDuties_CurrentEpoch_ShouldNotFail(t *testing.T) {
	...
	eth1Data, err := util.DeterministicEth1Data(len(deposits))
	require.NoError(t, err)
	bState, err := transition.GenesisBeaconState(context.Background(), deposits, 0, eth1Data, false)
