...
func (vs *Server) getAltairBeaconBlock(ctx context.Context, req *ethpb.BlockRequest) (*ethpb.BeaconBlockAltair, error) {
	...
	syncAggregate, err := vs.getSyncAggregate(ctx, req.Slot-1, bytes.ToBytes32(blkData.ParentRoot))
	if err != nil {
		return nil, err
	}
...
func (vs *Server) buildPhase0BlockData(ctx context.Context, req *ethpb.BlockRequest) (*blockData, error) {
	...
	graffiti := bytes.ToBytes32(req.Graffiti)
...
func (vs *Server) getSyncAggregate(ctx context.Context, slot types.Slot, root [32]byte) (*ethpb.SyncAggregate, error) {
	...
	if syncSig == nil {
		...
		syncSigBytes = bytes.ToBytes96(syncSig.Marshal())
	}
...
func (vs *Server) inRangeVotes(ctx context.Context,
	...
	for _, eth1Data := range beaconState.Eth1DataVotes() {
		exists, height, err := vs.BlockFetcher.BlockExistsWithCache(ctx, bytes.ToBytes32(eth1Data.BlockHash))
		if err != nil {
...
func (vs *Server) randomETH1DataVote(ctx context.Context) (*ethpb.Eth1Data, error) {
	...
	depRoot := hashutil.Hash(bytes.Bytes32(randGen.Uint64()))
	blockHash := hashutil.Hash(bytes.Bytes32(randGen.Uint64()))
	return &ethpb.Eth1Data{
		DepositRoot:  depRoot[:],
		DepositCount: headState.Eth1DepositIndex(),
		BlockHash:    blockHash[:],
...
func (vs *Server) ComputeStateRoot(ctx context.Context, block block.SignedBeaconBlock) ([]byte, error) {
	beaconState, err := vs.StateGen.StateByRoot(ctx, bytes.ToBytes32(block.Block().ParentRoot()))
	if err != nil {
...
func (vs *Server) canonicalEth1Data(
	...
	if hasSupport {
		...
		eth1BlockHash = bytes.ToBytes32(currentVote.BlockHash)
	} else {
		canonicalEth1Data = beaconState.Eth1Data()
		eth1BlockHash = bytes.ToBytes32(beaconState.Eth1Data().BlockHash)
	}
