...
func TestActivationStatus_OK(t *testing.T) {
	...
	deposits, _, err := customtesting.DeterministicDepositsAndKeys(4)
	require.NoError(t, err)
...
func TestMultipleValidatorStatus_Pubkeys(t *testing.T) {
	...
	deposits, _, err := customtesting.DeterministicDepositsAndKeys(6)
	require.NoError(t, err)
	pubKeys := [][]byte{
...
func createStateSetup(t *testing.T, head types.Epoch, mockgen *stategen.MockStateManager) (state.BeaconState,
	...
	gs, keys := customtesting.DeterministicGenesisState(t, 64)
	hs := gs.Copy()
	// Head State
	headEpoch := head
