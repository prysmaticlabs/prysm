...
func Test_computeOnChainAggregate(t *testing.T) {
	...
	t.Run("all aggregates for one root", func(t *testing.T) {
		...
		assert.DeepEqual(t, bitfield.Bitvector64([]byte{0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000000, 0b00000011}), result[0].CommitteeBitsVal())
	})
	t.Run("aggregates for multiple roots", func(t *testing.T) {
		...
		for _, a := range result {
			if reflect.DeepEqual(expectedAggBits, a.GetAggregationBits()) && reflect.DeepEqual(expectedData, a.GetData()) && reflect.DeepEqual(expectedCommitteeBits, a.CommitteeBitsVal()) {
				found = true
				break
			}
		}
