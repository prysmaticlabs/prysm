...
func TestProposer_GetBlock_OK(t *testing.T) {
	...
	require.NoError(t, db.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(genesis)), "Could not save genesis block")
...
func TestProposer_ProposeBlock_OK(t *testing.T) {
	...
	require.NoError(t, db.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(genesis)), "Could not save genesis block")
	...
	require.NoError(t, db.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(req)))
...
func TestProposer_ComputeStateRoot_OK(t *testing.T) {
	...
	_, err = proposerServer.computeStateRoot(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(req))
	require.NoError(t, err)
}
...
func TestProposer_Eth1Data_NoBlockExists(t *testing.T) {
	...
	for _, dp := range deps {
		assert.NoError(t, depositCache.InsertDeposit(context.Background(), dp.Deposit, dp.Eth1BlockHeight, dp.Index, depositTrie.Root()))
	}
