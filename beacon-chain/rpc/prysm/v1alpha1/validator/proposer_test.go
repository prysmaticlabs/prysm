...
func TestProposer_GetBlock_AddsUnaggregatedAtts(t *testing.T) {
	...
	params.SetupTestConfigCleanup(t)
	params.OverrideBeaconConfig(params.MainnetConfig())
	beaconState, privKeys := customtesting.DeterministicGenesisState(t, params.BeaconConfig().MinGenesisActiveValidatorCount)
	...
	for i := uint64(0); len(atts) < int(params.BeaconConfig().MaxAttestations); i++ {
		a, err := customtesting.GenerateAttestations(beaconState, privKeys, 4, 1, true)
		require.NoError(t, err)
		atts = append(atts, a...)
	}
	...
	atts, err = customtesting.GenerateAttestations(beaconState, privKeys, 300, 1, true)
	require.NoError(t, err)
	found := false
	for _, a := range atts {
...
func TestProposer_ProposeBlock_Altair_OK(t *testing.T) {
	...
	numDeposits := uint64(64)
	beaconState, _ := customtesting.DeterministicGenesisStateAltair(t, numDeposits)
	bsRoot, err := beaconState.HashTreeRoot(ctx)
	require.NoError(t, err)
	genesisRoot, err := genesis.Block.HashTreeRoot()
	require.NoError(t, err)
	...
	blockToPropose := customtesting.NewBeaconBlockAltair()
...
func TestProposer_FilterAttestation(t *testing.T) {
	...
	state, privKeys := customtesting.DeterministicGenesisState(t, numValidators)
		...
		},
		{
			...
			inputAtts: func() []*ethpb.Attestation {
				...
				for i := 0; i < len(atts); i++ {
					atts[i] = customtesting.HydrateAttestation(&ethpb.Attestation{
...
func TestProposer_DeleteAttsInPool_Aggregated(t *testing.T) {
	...
	aggregatedAtts := []*ethpb.Attestation{
		customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b10101}, Signature: sig}),
		customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b11010}, Signature: sig})}
	unaggregatedAtts := []*ethpb.Attestation{
		customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b10010}, Signature: sig}),
		customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b10100}, Signature: sig})}
