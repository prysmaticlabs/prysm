...
func TestServer_ListAttestations_Genesis(t *testing.T) {
	...
	signedBlock.Block.ParentRoot = butil.PadTo(parentRoot[:], 32)
		...
		blockExample.Block.Body.Attestations = []*ethpb.Attestation{
			{
				...
				Data: &ethpb.AttestationData{
					Target:          &ethpb.Checkpoint{Root: butil.PadTo([]byte("root"), 32)},
					Source:          &ethpb.Checkpoint{Root: butil.PadTo([]byte("root"), 32)},
					BeaconBlockRoot: butil.PadTo([]byte("root"), 32),
					Slot:            i,
				},
							...
							Attestations: []*ethpb.Attestation{
								{
									...
									AggregationBits: bitfield.Bitlist{0b11},
									Signature:       butil.PadTo([]byte("sig"), 96),
								},
							},
						},
					},
		...
		blockExample.Block.Body.Attestations = []*ethpb.Attestation{
			{
				...
				Signature:       butil.PadTo([]byte("root"), 96),
				AggregationBits: bitfield.Bitlist{0b11},
			},
		}
		require.NoError(t, db.SaveBlock(ctx, wrapper.WrappedPhase0SignedBeaconBlock(blockExample)))
...
func TestServer_mapAttestationToTargetRoot(t *testing.T) {
	...
	targetRoot1 := butil.ToBytes32([]byte("root1"))
	targetRoot2 := butil.ToBytes32([]byte("root2"))
...
func TestServer_ListIndexedAttestations_GenesisEpoch(t *testing.T) {
	...
	helpers.ClearCache()
	ctx := context.Background()
	targetRoot1 := butil.ToBytes32([]byte("root"))
	targetRoot2 := butil.ToBytes32([]byte("root2"))
	...
	require.NoError(t, db.SaveState(ctx, state, butil.ToBytes32(targetRoot1[:])))
	require.NoError(t, state.SetSlot(state.Slot()+1))
	require.NoError(t, db.SaveState(ctx, state, butil.ToBytes32(targetRoot2[:])))
...
func TestServer_ListIndexedAttestations_OldEpoch(t *testing.T) {
	...
	blockRoot := butil.ToBytes32([]byte("root"))
	count := params.BeaconConfig().SlotsPerEpoch
	atts := make([]*ethpb.Attestation, 0, count)
	...
	require.NoError(t, err)
	require.NoError(t, db.SaveState(ctx, state, butil.ToBytes32([]byte("root"))))
	res, err := bs.ListIndexedAttestations(ctx, &ethpb.ListIndexedAttestationsRequest{
	...
	atts := []*ethpb.Attestation{
		{
			Data: &ethpb.AttestationData{
				Slot:            1,
				BeaconBlockRoot: butil.PadTo([]byte{1}, 32),
				Source:          &ethpb.Checkpoint{Root: butil.PadTo([]byte{1}, 32)},
				Target:          &ethpb.Checkpoint{Root: butil.PadTo([]byte{1}, 32)},
			},
			AggregationBits: bitfield.Bitlist{0b1101},
			Signature:       butil.PadTo([]byte{1}, 96),
		},
		{
			Data: &ethpb.AttestationData{
				Slot:            2,
				BeaconBlockRoot: butil.PadTo([]byte{2}, 32),
				Source:          &ethpb.Checkpoint{Root: butil.PadTo([]byte{2}, 32)},
				Target:          &ethpb.Checkpoint{Root: butil.PadTo([]byte{2}, 32)},
			},
			AggregationBits: bitfield.Bitlist{0b1101},
			Signature:       butil.PadTo([]byte{2}, 96),
		},
		{
			Data: &ethpb.AttestationData{
				Slot:            3,
				BeaconBlockRoot: butil.PadTo([]byte{3}, 32),
				Source:          &ethpb.Checkpoint{Root: butil.PadTo([]byte{3}, 32)},
				Target:          &ethpb.Checkpoint{Root: butil.PadTo([]byte{3}, 32)},
			},
			AggregationBits: bitfield.Bitlist{0b1101},
			Signature:       butil.PadTo([]byte{3}, 96),
		},
	}
...
func TestServer_StreamIndexedAttestations_OK(t *testing.T) {
	...
	for _, dataRootAtts := range atts {
		targetRoot := butil.ToBytes32(dataRootAtts[0].Data.Target.Root)
