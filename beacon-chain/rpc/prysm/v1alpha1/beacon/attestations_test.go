...
func TestServer_ListAttestations_Genesis(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, db.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(signedBlock)))
...
func TestServer_ListAttestations_NoPagination(t *testing.T) {
	...
	for i := types.Slot(0); i < count; i++ {
		...
		require.NoError(t, db.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(blockExample)))
...
func TestServer_ListAttestations_FiltersCorrectly(t *testing.T) {
	...
	blocks := []blockInterface.SignedBeaconBlock{
		blockInterface.WrappedPhase0SignedBeaconBlock(
