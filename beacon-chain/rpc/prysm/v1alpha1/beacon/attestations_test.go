...
func TestServer_ListAttestations_NoPagination(t *testing.T) {
	...
	for i := types.Slot(0); i < count; i++ {
		blockExample := customtesting.NewBeaconBlock()
...
func TestServer_ListAttestations_FiltersCorrectly(t *testing.T) {
	...
	blocks := []block.SignedBeaconBlock{
		wrapper.WrappedPhase0SignedBeaconBlock(
			customtesting.HydrateSignedBeaconBlock(
		...
		wrapper.WrappedPhase0SignedBeaconBlock(
			customtesting.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{
				Block: &ethpb.BeaconBlock{
...
func TestServer_ListAttestations_Pagination_OutOfRange(t *testing.T) {
	...
	atts := make([]*ethpb.Attestation, 0, count)
	for i := types.Slot(0); i < count; i++ {
		blockExample := customtesting.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{
			Block: &ethpb.BeaconBlock{
				Body: &ethpb.BeaconBlockBody{
					Attestations: []*ethpb.Attestation{
						{
...
func TestServer_ListIndexedAttestations_GenesisEpoch(t *testing.T) {
	...
	state, _ := customtesting.DeterministicGenesisState(t, numValidators)
...
func TestServer_AttestationPool_Pagination_DefaultPageSize(t *testing.T) {
	...
	for i := 0; i < len(atts); i++ {
		att := customtesting.NewAttestation()
...
func TestServer_StreamIndexedAttestations_OK(t *testing.T) {
	...
	numValidators := 64
	headState, privKeys := customtesting.DeterministicGenesisState(t, uint64(numValidators))
...
func TestServer_StreamAttestations_OnSlotTick(t *testing.T) {
	...
	atts := []*ethpb.Attestation{
		customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Slot: 1}, AggregationBits: bitfield.Bitlist{0b1101}}),
		customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Slot: 2}, AggregationBits: bitfield.Bitlist{0b1101}}),
		customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Slot: 3}, AggregationBits: bitfield.Bitlist{0b1101}}),
	}
