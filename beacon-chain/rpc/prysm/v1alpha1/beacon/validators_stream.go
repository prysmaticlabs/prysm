...
func (is *infostream) calculateActivationTimeForPendingValidators(res []*ethpb.ValidatorInfo, headState interfaces.ReadOnlyBeaconState, epoch types.Epoch) error {
	...
	err := headState.ReadFromEveryValidator(func(idx int, val interfaces.ReadOnlyValidator) error {
		if val.IsNil() {
...
func (is *infostream) calculateStatusAndTransition(validator interfaces.ReadOnlyValidator, currentEpoch types.Epoch) (ethpb.ValidatorStatus, uint64) {
