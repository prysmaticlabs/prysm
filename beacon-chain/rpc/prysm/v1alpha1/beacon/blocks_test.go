...
func TestServer_GetChainHead_NoFinalizedBlock(t *testing.T) {
	...
	require.NoError(t, s.SetPreviousJustifiedCheckpoint(&ethpb.Checkpoint{Epoch: 3, Root: bytes.PadTo([]byte{'A'}, 32)}))
	require.NoError(t, s.SetCurrentJustifiedCheckpoint(&ethpb.Checkpoint{Epoch: 2, Root: bytes.PadTo([]byte{'B'}, 32)}))
	require.NoError(t, s.SetFinalizedCheckpoint(&ethpb.Checkpoint{Epoch: 1, Root: bytes.PadTo([]byte{'C'}, 32)}))

	genBlock := testutil.NewBeaconBlock()
	genBlock.Block.ParentRoot = bytes.PadTo([]byte{'G'}, 32)
...
func TestServer_GetChainHead(t *testing.T) {
	...
	finalizedBlock.Block.ParentRoot = bytes.PadTo([]byte{'A'}, 32)
	...
	justifiedBlock.Block.ParentRoot = bytes.PadTo([]byte{'B'}, 32)
	...
	prevJustifiedBlock.Block.ParentRoot = bytes.PadTo([]byte{'C'}, 32)
