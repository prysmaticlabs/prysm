...
func TestServer_ListBlocks_Pagination(t *testing.T) {
	...
	orphanedBlk := customtesting.NewBeaconBlock()
				...
				BlockContainers: []*ethpb.BeaconBlockContainer{
					{
						Block: &ethpb.BeaconBlockContainer_Phase0Block{
							Phase0Block: customtesting.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{
...
func TestServer_GetChainHead_NoFinalizedBlock(t *testing.T) {
	...
	genBlock := customtesting.NewBeaconBlock()
...
func TestServer_GetChainHead(t *testing.T) {
	...
	finalizedBlock := customtesting.NewBeaconBlock()
	...
	prevJustifiedBlock := customtesting.NewBeaconBlock()
...
func TestServer_StreamBlocks_OnHeadUpdated(t *testing.T) {
	ctx := context.Background()
	beaconState, privs := customtesting.DeterministicGenesisState(t, 32)
	b, err := customtesting.GenerateFullBlock(beaconState, privs, customtesting.DefaultBlockGenConfig(), 1)
	require.NoError(t, err)
...
func TestServer_GetWeakSubjectivityCheckpoint(t *testing.T) {
	...
	// Genesis block.
	genesisBlock := customtesting.NewBeaconBlock()
...
func TestServer_ListBlocksAltair_Pagination(t *testing.T) {
	...
	tests := []struct {
		...
		{req: &ethpb.ListBlocksRequest{
			...
			PageSize:    3},
			res: &ethpb.ListBeaconBlocksResponse{
				BlockContainers: []*ethpb.BeaconBlockContainer{{Block: &ethpb.BeaconBlockContainer_Phase0Block{Phase0Block: customtesting.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{
					Block: &ethpb.BeaconBlock{
						Slot: 5}})},
					BlockRoot: blkContainers[5].BlockRoot,
					Canonical: blkContainers[5].Canonical}},
