...
func TestServer_ListBlocks_Genesis_MultiBlocks(t *testing.T) {
	...
	blks := make([]blockInterface.SignedBeaconBlock, count)
	for i := types.Slot(0); i < count; i++ {
		b := testutil.NewBeaconBlock()
		b.Block.Slot = i
		require.NoError(t, err)
		blks[i] = blockInterface.WrappedPhase0SignedBeaconBlock(b)
	}
...
func TestServer_ListBlocks_Pagination(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, db.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(orphanedBlk)))
...
func TestServer_GetChainHead_NoFinalizedBlock(t *testing.T) {
	...
	require.NoError(t, db.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(genBlock)))
	...
	bs := &Server{
		BeaconDB:    db,
		HeadFetcher: &chainMock.ChainService{Block: blockInterface.WrappedPhase0SignedBeaconBlock(genBlock), State: s},
...
func TestServer_GetChainHead(t *testing.T) {
	...
	require.NoError(t, db.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(finalizedBlock)))
	...
	require.NoError(t, db.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(justifiedBlock)))
	...
	require.NoError(t, db.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(prevJustifiedBlock)))
	...
	bs := &Server{
		BeaconDB:    db,
		HeadFetcher: &chainMock.ChainService{Block: blockInterface.WrappedPhase0SignedBeaconBlock(b), State: s},
		FinalizationFetcher: &chainMock.ChainService{
			FinalizedCheckPoint:         s.FinalizedCheckpoint(),
			CurrentJustifiedCheckPoint:  s.CurrentJustifiedCheckpoint(),
			PreviousJustifiedCheckPoint: s.PreviousJustifiedCheckpoint()},
...
func TestServer_StreamBlocks_OnHeadUpdated(t *testing.T) {
	...
	for sent := 0; sent == 0; {
		sent = server.BlockNotifier.BlockFeed().Send(&feed.Event{
			...
			Data: &blockfeed.ReceivedBlockData{SignedBlock: blockInterface.WrappedPhase0SignedBeaconBlock(b)},
		})
	}
	<-exitRoutine
}
...
func TestServer_StreamBlocksVerified_OnHeadUpdated(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, db.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b)))
	chainService := &chainMock.ChainService{State: beaconState}
	server := &Server{
		Ctx:           ctx,
		StateNotifier: chainService.StateNotifier(),
	...
	for sent := 0; sent == 0; {
		sent = server.StateNotifier.StateFeed().Send(&feed.Event{
			Type: statefeed.BlockProcessed,
			Data: &statefeed.BlockProcessedData{Slot: b.Block.Slot, BlockRoot: r, SignedBlock: blockInterface.WrappedPhase0SignedBeaconBlock(b)},
		})
	}
	<-exitRoutine
}
