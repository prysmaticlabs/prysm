...
func TestProposer_GetBeaconBlock_Phase0(t *testing.T) {
	db := dbutil.SetupDB(t)
	...
	s := getService(db, beaconState, parentRoot[:])

	randaoReveal, err := util.RandaoReveal(beaconState, 0, privKeys)
	require.NoError(t, err)

	graffiti := bytesutil.ToBytes32([]byte("eth2"))
	proposerSlashings, attSlashings := injectSlashings(t, beaconState, privKeys, s)

	block, rpcerr := s.GetBeaconBlock(ctx, 1, randaoReveal, graffiti[:])
	require.Equal(t, true, rpcerr == nil)
	phase0Blk, ok := block.GetBlock().(*ethpb.GenericBeaconBlock_Phase0)
	require.Equal(t, true, ok)
	assert.Equal(t, primitives.Slot(1), phase0Blk.Phase0.Slot)
	...
	assert.DeepEqual(t, graffiti[:], phase0Blk.Phase0.Body.Graffiti, "Expected block to have correct Graffiti")
...
func TestProposer_GetBeaconBlock_Altair(t *testing.T) {
	db := dbutil.SetupDB(t)
	ctx := context.Background()
	...
	block, rpcerr := s.GetBeaconBlock(ctx, altairSlot+1, randaoReveal, graffiti[:])
	require.Equal(t, true, rpcerr == nil)
	altairBlk, ok := block.GetBlock().(*ethpb.GenericBeaconBlock_Altair)
	require.Equal(t, true, ok)

	assert.Equal(t, altairSlot+1, altairBlk.Altair.Slot)
	...
	assert.DeepEqual(t, graffiti[:], altairBlk.Altair.Body.Graffiti, "Expected block to have correct Graffiti")
...
func TestProposer_GetBeaconBlock_Bellatrix(t *testing.T) {
	db := dbutil.SetupDB(t)
	ctx := context.Background()
	...
	s := getService(db, beaconState, parentRoot[:])
	s.Eth1BlockFetcher = c
	s.ExecutionEngineCaller = &mockExecution.EngineClient{
	...
	block, rpcerr := s.GetBeaconBlock(ctx, bellatrixSlot+1, randaoReveal, graffiti[:])
	require.Equal(t, true, rpcerr == nil)
	bellatrixBlk, ok := block.GetBlock().(*ethpb.GenericBeaconBlock_Bellatrix)
	require.Equal(t, true, ok)

	assert.Equal(t, bellatrixSlot+1, bellatrixBlk.Bellatrix.Slot)
	...
	assert.DeepEqual(t, graffiti[:], bellatrixBlk.Bellatrix.Body.Graffiti, "Expected block to have correct Graffiti")
	...
	params.OverrideBeaconConfig(cfg)
	_, rpcerr = s.GetBeaconBlock(ctx, bellatrixSlot+1, randaoReveal, graffiti[:])
	require.Equal(t, true, rpcerr == nil)
	require.LogsDoNotContain(t, newHook, "Fee recipient is currently using the burn address")
}

func TestProposer_GetBeaconBlock_Capella(t *testing.T) {
	db := dbutil.SetupDB(t)
	ctx := context.Background()
	...
	require.NoError(t, err)
	s.BLSChangesPool.InsertBLSToExecChange(change)

	got, rpcerr := s.GetBeaconBlock(ctx, capellaSlot+1, randaoReveal, graffiti[:])
	require.Equal(t, true, rpcerr == nil)
...
func TestProposer_GetBeaconBlock_Deneb(t *testing.T) {
	db := dbutil.SetupDB(t)
	ctx := context.Background()
	transition.SkipSlotCache.Disable()
	...
	got, rpcerr := s.GetBeaconBlock(ctx, denebSlot+1, randaoReveal, graffiti[:])
	require.Equal(t, true, rpcerr == nil)
...
func TestProposer_GetBeaconBlock_Optimistic(t *testing.T) {
	...
	_, rpcerr := service.GetBeaconBlock(context.Background(), bellatrixSlot+1, nil, nil)
	require.Equal(t, ErrorReason(Unavailable), rpcerr.Reason)
	require.ErrorContains(t, errOptimisticMode.Error(), rpcerr.Err)
}

func getService(db db.HeadAccessDatabase, headState state.BeaconState, headRoot []byte) *Service {
...
func injectSlashings(t *testing.T, st state.BeaconState, keys []bls.SecretKey, s *Service) ([]*ethpb.ProposerSlashing, []*ethpb.AttesterSlashing) {
	...
	for i := primitives.ValidatorIndex(0); uint64(i) < params.BeaconConfig().MaxProposerSlashings; i++ {
		...
		err = s.SlashingsPool.InsertProposerSlashing(context.Background(), st, proposerSlashing)
		require.NoError(t, err)
	}
	...
	for i := uint64(0); i < params.BeaconConfig().MaxAttesterSlashings; i++ {
		...
		err = s.SlashingsPool.InsertAttesterSlashing(context.Background(), st, attesterSlashing)
		require.NoError(t, err)
	}
...
func TestProposer_ProposeBlock_OK(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, rpcerr := s.ProposeBeaconBlock(context.Background(), blockToPropose)
			if tt.err != "" { // Expecting an error
				require.ErrorContains(t, tt.err, rpcerr.Err)
			} else {
