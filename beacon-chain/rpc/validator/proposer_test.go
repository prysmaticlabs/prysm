...
func TestEth1DataMajorityVote_InVotingPeriodHead_ChooseDefault(t *testing.T) {
	slot := uint64(64)
	...
	beaconState, err := beaconstate.InitializeFromProto(&pbp2p.BeaconState{
		Eth1DataVotes: []*ethpb.Eth1Data{{BlockHash: []byte("first")}},
	})
	require.NoError(t, err)
...
func TestEth1DataMajorityVote_OutsideVotingPeriodHead_ChooseHighestCount(t *testing.T) {
	slot := 64 + params.BeaconConfig().HeadOfVotingPeriodLength
