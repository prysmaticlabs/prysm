...
func fillDBTestBlocks(ctx context.Context, t *testing.T, beaconDB db.Database) (*ethpbalpha.SignedBeaconBlock, []*ethpbalpha.BeaconBlockContainer) {
	...
	for i := types.Slot(0); i < count; i++ {
		...
		b.Block.Slot = i
		b.Block.ParentRoot = butil.PadTo([]byte{uint8(i)}, 32)
	...
	headRoot := butil.ToBytes32(blkContainers[len(blks)-1].BlockRoot)
...
func TestServer_GetBlock(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			blockID: butil.PadTo([]byte("hi there"), 32),
			wantErr: true,
		},
		{
			name:    "no block",
