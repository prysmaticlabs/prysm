...
func TestGetAttesterDuties(t *testing.T) {
	...
	vs := &Server{
		StateFetcher: &testutil.MockStater{
	...
	t.Run("execution optimistic", func(t *testing.T) {
		...
		vs := &Server{
			StateFetcher:          &testutil.MockStater{StatesBySlot: map[primitives.Slot]state.BeaconState{0: bs}},
			TimeFetcher:           chain,
			OptimisticModeFetcher: chain,
			SyncChecker:           &mockSync.Sync{IsSyncing: false},
		}
...
func TestGetProposerDuties(t *testing.T) {
	...
	t.Run("Prune payload ID cache ok", func(t *testing.T) {
		...
		vs := &Server{
			StateFetcher:           &testutil.MockStater{StatesBySlot: map[primitives.Slot]state.BeaconState{params.BeaconConfig().SlotsPerEpoch: bs}},
			HeadFetcher:            chain,
			TimeFetcher:            chain,
			OptimisticModeFetcher:  chain,
			SyncChecker:            &mockSync.Sync{IsSyncing: false},
...
func TestGetSyncCommitteeDuties(t *testing.T) {
	...
	vs := &Server{
		StateFetcher:          &testutil.MockStater{BeaconState: st},
		SyncChecker:           &mockSync.Sync{IsSyncing: false},
	...
	t.Run("correct sync committee is fetched", func(t *testing.T) {
		...
		vs := &Server{
			StateFetcher:          &testutil.MockStater{BeaconState: stateFetchFn(newSyncPeriodStartSlot)},
			SyncChecker:           &mockSync.Sync{IsSyncing: false},
			TimeFetcher:           mockChainService,
			HeadFetcher:           mockChainService,
			OptimisticModeFetcher: mockChainService,
