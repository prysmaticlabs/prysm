...
func TestGetAttesterDuties(t *testing.T) {
	...
	genesis := customtesting.NewBeaconBlock()
	depChainStart := params.BeaconConfig().MinGenesisActiveValidatorCount
	deposits, _, err := customtesting.DeterministicDepositsAndKeys(depChainStart)
...
func TestGetSyncCommitteeDuties(t *testing.T) {
	ctx := context.Background()
	numVals := uint64(10)
	st, _ := customtesting.DeterministicGenesisStateAltair(t, numVals)
	vals := st.Validators()
...
func TestProduceBlock(t *testing.T) {
	...
	for i := types.ValidatorIndex(0); uint64(i) < params.BeaconConfig().MaxProposerSlashings; i++ {
		proposerSlashing, err := customtesting.GenerateProposerSlashingForValidator(
			beaconState,
			privKeys[i],
			i, /* validator index */
		)
	...
	for i := uint64(0); i < params.BeaconConfig().MaxAttesterSlashings; i++ {
		attesterSlashing, err := customtesting.GenerateAttesterSlashingForValidator(
			beaconState,
	...
	randaoReveal, err := customtesting.RandaoReveal(beaconState, 0, privKeys)
	require.NoError(t, err)
...
func TestProduceBlockV2(t *testing.T) {
	...
	t.Run("Altair", func(t *testing.T) {
		...
		beaconState, privKeys := customtesting.DeterministicGenesisStateAltair(t, params.BeaconConfig().SyncCommitteeSize)
		...
		stateRoot, err := beaconState.HashTreeRoot(ctx)
		require.NoError(t, err, "Could not hash genesis state")
		genesisBlock := customtesting.NewBeaconBlockAltair()
...
func TestProduceAttestationData(t *testing.T) {
	...
	targetBlock := customtesting.NewBeaconBlock()
	targetBlock.Block.Slot = 1 * params.BeaconConfig().SlotsPerEpoch
	justifiedBlock := customtesting.NewBeaconBlock()
...
func TestSubmitSyncCommitteeSubscription(t *testing.T) {
	ctx := context.Background()
	genesis := customtesting.NewBeaconBlock()
	deposits, _, err := customtesting.DeterministicDepositsAndKeys(64)
	require.NoError(t, err)
	eth1Data, err := customtesting.DeterministicEth1Data(len(deposits))
	require.NoError(t, err)
	bs, err := customtesting.GenesisBeaconState(context.Background(), deposits, 0, eth1Data)
	require.NoError(t, err, "Could not set up genesis state")
	genesisRoot, err := genesis.Block.HashTreeRoot()
	require.NoError(t, err, "Could not get signing root")
	roots := make([][]byte, params.BeaconConfig().SlotsPerHistoricalRoot)
