...
func TestSlotNotTooEarly(t *testing.T) {
	...
	closeClock := startup.NewClock(genesis, [32]byte{}, startup.WithNower(func() time.Time { return now.Add(-1 * params.BeaconConfig().MaximumGossipClockDisparity / 2) }))
	...
	disparate := now.Add(-2 * params.BeaconConfig().MaximumGossipClockDisparity)
