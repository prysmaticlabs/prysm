...
func TestPeerBadResponses(t *testing.T) {
	...
	assert.Equal(t, false, p.Status(id), "Peer marked as bad when should be good")
	...
	assert.Equal(t, true, p.Status(id), "Peer not marked as bad when it should be")
...
func TestPeerIPTracker(t *testing.T) {
	...
	for _, pr := range badPeers {
		assert.Equal(t, true, p.Status(pr), "peer with bad ip is not bad")
	}
	...
	for _, pr := range badPeers {
		assert.Equal(t, false, p.Status(pr), "peer with good ip is regarded as bad")
	}
}
