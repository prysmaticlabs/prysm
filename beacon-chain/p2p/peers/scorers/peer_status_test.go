...
func TestScorers_PeerStatus_IsBadPeer(t *testing.T) {
	...
	pid := peer.ID("peer1")
	assert.Equal(t, false, peerStatuses.Scorers().Status(pid))
	assert.Equal(t, false, peerStatuses.Scorers().PeerStatusScorer().Status(pid))

	peerStatuses.Scorers().PeerStatusScorer().SetPeerStatus(pid, &pb.Status{}, p2ptypes.ErrWrongForkDigestVersion)
	assert.Equal(t, true, peerStatuses.Scorers().Status(pid))
	assert.Equal(t, true, peerStatuses.Scorers().PeerStatusScorer().Status(pid))
}
...
func TestScorers_PeerStatus_BadPeers(t *testing.T) {
	...
	pid2 := peer.ID("peer2")
	pid3 := peer.ID("peer3")
	assert.Equal(t, false, peerStatuses.Scorers().Status(pid1))
	assert.Equal(t, false, peerStatuses.Scorers().PeerStatusScorer().Status(pid1))
	assert.Equal(t, false, peerStatuses.Scorers().Status(pid2))
	assert.Equal(t, false, peerStatuses.Scorers().PeerStatusScorer().Status(pid2))
	assert.Equal(t, false, peerStatuses.Scorers().Status(pid3))
	assert.Equal(t, false, peerStatuses.Scorers().PeerStatusScorer().Status(pid3))
	...
	assert.Equal(t, true, peerStatuses.Scorers().Status(pid1))
	assert.Equal(t, true, peerStatuses.Scorers().PeerStatusScorer().Status(pid1))
	assert.Equal(t, false, peerStatuses.Scorers().Status(pid2))
	assert.Equal(t, false, peerStatuses.Scorers().PeerStatusScorer().Status(pid2))
	assert.Equal(t, true, peerStatuses.Scorers().Status(pid3))
	assert.Equal(t, true, peerStatuses.Scorers().PeerStatusScorer().Status(pid3))
