...
func TestStore_OnAttestation_ErrorConditions(t *testing.T) {
	...
	require.NoError(t, beaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(BlkWithOutState)))
	BlkWithOutStateRoot, err := BlkWithOutState.Block.HashTreeRoot()
	require.NoError(t, err)

	BlkWithStateBadAtt := testutil.NewBeaconBlock()
	BlkWithStateBadAtt.Block.Slot = 1
	require.NoError(t, beaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(BlkWithStateBadAtt)))
	...
	require.NoError(t, beaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(BlkWithValidState)))
...
func TestVerifyBeaconBlock_futureBlock(t *testing.T) {
	...
	b.Block.Slot = 2
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b)))
