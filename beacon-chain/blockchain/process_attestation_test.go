...
func TestStore_OnAttestation_Ok(t *testing.T) {
	...
	require.NoError(t, err)
	tRoot := bytes.ToBytes32(att[0].Data.Target.Root)
...
func TestStore_SaveCheckpointState(t *testing.T) {
	...
	s, err := testutil.NewBeaconState()
	require.NoError(t, err)
	err = s.SetFinalizedCheckpoint(&ethpb.Checkpoint{Root: bytes.PadTo([]byte{'A'}, 32)})
	require.NoError(t, err)
	val := &ethpb.Validator{
		PublicKey:             bytes.PadTo([]byte("foo"), 48),
		WithdrawalCredentials: bytes.PadTo([]byte("bar"), 32),
	}
	...
	r = bytes.ToBytes32([]byte{'A'})
	cp1 := &ethpb.Checkpoint{Epoch: 1, Root: bytes.PadTo([]byte{'A'}, 32)}
	require.NoError(t, service.cfg.BeaconDB.SaveState(ctx, s, bytes.ToBytes32([]byte{'A'})))
	require.NoError(t, service.cfg.BeaconDB.SaveStateSummary(ctx, &ethpb.StateSummary{Root: bytes.PadTo([]byte{'A'}, 32)}))
	...
	require.NoError(t, service.cfg.BeaconDB.SaveState(ctx, s, bytes.ToBytes32([]byte{'B'})))
	require.NoError(t, service.cfg.BeaconDB.SaveStateSummary(ctx, &ethpb.StateSummary{Root: bytes.PadTo([]byte{'B'}, 32)}))
	...
	service.finalizedCheckpt = &ethpb.Checkpoint{Root: r[:]}
	service.prevFinalizedCheckpt = &ethpb.Checkpoint{Root: r[:]}
	cp3 := &ethpb.Checkpoint{Epoch: 1, Root: bytes.PadTo([]byte{'C'}, 32)}
	require.NoError(t, service.cfg.BeaconDB.SaveState(ctx, s, bytes.ToBytes32([]byte{'C'})))
	require.NoError(t, service.cfg.BeaconDB.SaveStateSummary(ctx, &ethpb.StateSummary{Root: bytes.PadTo([]byte{'C'}, 32)}))
...
func TestStore_UpdateCheckpointState(t *testing.T) {
	...
	checkpoint := &ethpb.Checkpoint{Epoch: epoch, Root: bytes.PadTo([]byte("hi"), 32)}
	require.NoError(t, service.cfg.BeaconDB.SaveState(ctx, baseState, bytes.ToBytes32(checkpoint.Root)))
	...
	epoch = 2
	newCheckpoint := &ethpb.Checkpoint{Epoch: epoch, Root: bytes.PadTo([]byte("bye"), 32)}
	require.NoError(t, service.cfg.BeaconDB.SaveState(ctx, baseState, bytes.ToBytes32(newCheckpoint.Root)))
