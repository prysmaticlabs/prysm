...
func TestStore_OnAttestation_ErrorConditions(t *testing.T) {
	...
	BlkWithValidState := customtesting.NewBeaconBlock()
	...
	tests := []struct {
	...
	}{
		{
			...
			a:         customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Slot: params.BeaconConfig().SlotsPerEpoch, Target: &ethpb.Checkpoint{Root: make([]byte, 32)}}}),
			wantedErr: "slot 32 does not match target epoch 0",
		},
		{
			name:      "no pre state for attestations's target block",
			a:         customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Target: &ethpb.Checkpoint{Root: BlkWithOutStateRoot[:]}}}),
			wantedErr: "could not get pre state for epoch 0",
		},
		{
			name: "process attestation doesn't match current epoch",
			a: customtesting.HydrateAttestation(&ethpb.Attestation{Data: &ethpb.AttestationData{Slot: 100 * params.BeaconConfig().SlotsPerEpoch, Target: &ethpb.Checkpoint{Epoch: 100,
...
func TestStore_OnAttestation_Ok(t *testing.T) {
	...
	att, err := customtesting.GenerateAttestations(genesisState, pks, 1, 0, false)
	require.NoError(t, err)
...
func TestStore_UpdateCheckpointState(t *testing.T) {
	...
	epoch := types.Epoch(1)
	baseState, _ := customtesting.DeterministicGenesisState(t, 1)
