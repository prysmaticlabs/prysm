...
func TestStore_OnBlock(t *testing.T) {
	...
	require.NoError(t, err)
	random := customtesting.NewBeaconBlock()
	random.Block.Slot = 1
	...
	tests := []struct {
	...
	}{
		{
			...
			blk:           customtesting.NewBeaconBlock(),
			s:             st.Copy(),
...
func TestStore_OnBlockBatch(t *testing.T) {
	...
	st, keys := customtesting.DeterministicGenesisState(t, 64)
	...
	for i := 1; i < 10; i++ {
		b, err := customtesting.GenerateFullBlock(bState, keys, customtesting.DefaultBlockGenConfig(), types.Slot(i))
		require.NoError(t, err)
...
func TestRemoveStateSinceLastFinalized_EmptyStartSlot(t *testing.T) {
	...
	lastJustifiedBlk := customtesting.NewBeaconBlock()
	lastJustifiedBlk.Block.ParentRoot = bytesutil.PadTo([]byte{'G'}, 32)
	lastJustifiedRoot, err := lastJustifiedBlk.Block.HashTreeRoot()
	require.NoError(t, err)
	newJustifiedBlk := customtesting.NewBeaconBlock()
...
func TestUpdateJustified_CouldUpdateBest(t *testing.T) {
	...
	signedBlock := customtesting.NewBeaconBlock()
...
func TestFillForkChoiceMissingBlocks_FilterFinalized(t *testing.T) {
	...
	b63 := customtesting.NewBeaconBlock()
	b63.Block.Slot = 63
	...
	require.NoError(t, err)
	b64 := customtesting.NewBeaconBlock()
	b64.Block.Slot = 64
	...
	require.NoError(t, err)
	b65 := customtesting.NewBeaconBlock()
	b65.Block.Slot = 65
...
func blockTree1(t *testing.T, beaconDB db.Database, genesisRoot []byte) ([][]byte, error) {
	...
	b0 := customtesting.NewBeaconBlock()
	b0.Block.Slot = 0
	...
	b1 := customtesting.NewBeaconBlock()
	b1.Block.Slot = 1
	...
	b3 := customtesting.NewBeaconBlock()
	b3.Block.Slot = 3
	...
	b4 := customtesting.NewBeaconBlock()
	b4.Block.Slot = 4
	...
	b5 := customtesting.NewBeaconBlock()
	b5.Block.Slot = 5
	...
	b6 := customtesting.NewBeaconBlock()
	b6.Block.Slot = 6
	...
	b7 := customtesting.NewBeaconBlock()
	b7.Block.Slot = 7
	...
	b8 := customtesting.NewBeaconBlock()
	b8.Block.Slot = 8
	...
	for _, b := range []*ethpb.SignedBeaconBlock{b0, b1, b3, b4, b5, b6, b7, b8} {
		beaconBlock := customtesting.NewBeaconBlock()
...
func TestAncestor_HandleSkipSlot(t *testing.T) {
	...
	r1, err := b1.Block.HashTreeRoot()
	require.NoError(t, err)
	b100 := customtesting.NewBeaconBlock()
	b100.Block.Slot = 100
	b100.Block.ParentRoot = r1[:]
	r100, err := b100.Block.HashTreeRoot()
	require.NoError(t, err)
	b200 := customtesting.NewBeaconBlock()
	b200.Block.Slot = 200
...
func TestFinalizedImpliesNewJustified(t *testing.T) {
	...
	for _, test := range tests {
		...
		genesisState, err := customtesting.NewBeaconState()
		require.NoError(t, err)
...
func TestUpdateJustifiedInitSync(t *testing.T) {
	...
	gBlk := customtesting.NewBeaconBlock()
...
func TestHandleEpochBoundary_UpdateFirstSlot(t *testing.T) {
	...
	s, _ := customtesting.DeterministicGenesisState(t, 1024)
...
func TestOnBlock_CanFinalize(t *testing.T) {
	...
	gs, keys := customtesting.DeterministicGenesisState(t, 32)
	...
	for i := types.Slot(1); i <= 4*params.BeaconConfig().SlotsPerEpoch; i++ {
		blk, err := customtesting.GenerateFullBlock(testState, keys, customtesting.DefaultBlockGenConfig(), i)
		require.NoError(t, err)
...
func TestInsertFinalizedDeposits(t *testing.T) {
	...
	gs, _ := customtesting.DeterministicGenesisState(t, 32)
	require.NoError(t, service.saveGenesisData(ctx, gs))
	gBlk, err := service.cfg.BeaconDB.GenesisBlock(ctx)
	require.NoError(t, err)
	gRoot, err := gBlk.Block().HashTreeRoot()
...
func TestRemoveBlockAttestationsInPool_Canonical(t *testing.T) {
	...
	genesis, keys := customtesting.DeterministicGenesisState(t, 64)
	b, err := customtesting.GenerateFullBlock(genesis, keys, customtesting.DefaultBlockGenConfig(), 1)
	assert.NoError(t, err)
	r, err := b.Block.HashTreeRoot()
	require.NoError(t, err)
