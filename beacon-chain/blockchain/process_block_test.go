...
func TestStore_OnBlock(t *testing.T) {
	...
	assert.NoError(t, beaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(genesis)))
	...
	assert.NoError(t, beaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(random)))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			err = service.onBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(tt.blk), root)
...
func TestStore_OnBlockBatch(t *testing.T) {
	...
	service.saveInitSyncBlock(gRoot, blockInterface.WrappedPhase0SignedBeaconBlock(genesis))
	...
	for i := 1; i < 10; i++ {
		...
		require.NoError(t, err)
		bState, err = state.ExecuteStateTransition(ctx, bState, blockInterface.WrappedPhase0SignedBeaconBlock(b))
		require.NoError(t, err)
		if i == 1 {
		...
		require.NoError(t, err)
		service.saveInitSyncBlock(root, blockInterface.WrappedPhase0SignedBeaconBlock(b))
		blks = append(blks, blockInterface.WrappedPhase0SignedBeaconBlock(b))
...
func TestRemoveStateSinceLastFinalized_EmptyStartSlot(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(newJustifiedBlk)))
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(lastJustifiedBlk)))
...
func TestCachedPreState_CanGetFromStateSummary(t *testing.T) {
	...
	require.NoError(t, service.verifyBlkPreState(ctx, blockInterface.WrappedPhase0BeaconBlock(b.Block)))
}
...
func TestCachedPreState_CanGetFromDB(t *testing.T) {
	...
	err = service.verifyBlkPreState(ctx, blockInterface.WrappedPhase0BeaconBlock(b.Block))
	...
	require.NoError(t, service.cfg.BeaconDB.SaveStateSummary(ctx, &pb.StateSummary{Slot: 1, Root: gRoot[:]}))
	require.NoError(t, service.cfg.StateGen.SaveState(ctx, gRoot, s))
	require.NoError(t, service.verifyBlkPreState(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b).Block()))
}
...
func TestUpdateJustified_CouldUpdateBest(t *testing.T) {
	...
	require.NoError(t, beaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(signedBlock)))
...
func TestFillForkChoiceMissingBlocks_CanSave(t *testing.T) {
	...
	genesisStateRoot := [32]byte{}
	genesis := blocks.NewGenesisBlock(genesisStateRoot[:])
	require.NoError(t, beaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(genesis)))
	validGenesisRoot, err := genesis.Block.HashTreeRoot()
	require.NoError(t, err)
	st, err := testutil.NewBeaconState()
	require.NoError(t, err)
	...
	err = service.fillInForkChoiceMissingBlocks(
		context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(block).Block(), beaconState.FinalizedCheckpoint(), beaconState.CurrentJustifiedCheckpoint())
	require.NoError(t, err)
...
func TestFillForkChoiceMissingBlocks_FilterFinalized(t *testing.T) {
	...
	b63.Block.Slot = 63
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b63)))
	r63, err := b63.Block.HashTreeRoot()
	require.NoError(t, err)
	b64 := testutil.NewBeaconBlock()
	b64.Block.Slot = 64
	b64.Block.ParentRoot = r63[:]
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b64)))
	r64, err := b64.Block.HashTreeRoot()
	require.NoError(t, err)
	b65 := testutil.NewBeaconBlock()
	b65.Block.Slot = 65
	b65.Block.ParentRoot = r64[:]
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b65)))

	beaconState, _ := testutil.DeterministicGenesisState(t, 32)
	err = service.fillInForkChoiceMissingBlocks(
		context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(b65).Block(), beaconState.FinalizedCheckpoint(), beaconState.CurrentJustifiedCheckpoint())
	require.NoError(t, err)
...
func blockTree1(t *testing.T, beaconDB db.Database, genesisRoot []byte) ([][]byte, error) {
	...
	for _, b := range []*ethpb.SignedBeaconBlock{b0, b1, b3, b4, b5, b6, b7, b8} {
		...
		if err := beaconDB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(beaconBlock)); err != nil {
			return nil, err
		}
...
func TestAncestor_HandleSkipSlot(t *testing.T) {
	...
	for _, b := range []*ethpb.SignedBeaconBlock{b1, b100, b200} {
		...
		beaconBlock.Block.Slot = b.Block.Slot
		beaconBlock.Block.ParentRoot = bytesutil.PadTo(b.Block.ParentRoot, 32)
		require.NoError(t, beaconDB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(beaconBlock)))
	}
...
func TestAncestor_CanUseDB(t *testing.T) {
	...
	for _, b := range []*ethpb.SignedBeaconBlock{b1, b100, b200} {
		...
		beaconBlock.Block.Slot = b.Block.Slot
		beaconBlock.Block.ParentRoot = bytesutil.PadTo(b.Block.ParentRoot, 32)
		require.NoError(t, beaconDB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(beaconBlock))) // Saves blocks to DB.
	}
	...
	for _, test := range tests {
		...
		if test.args.diffFinalizedCheckPoint {
			...
			for _, b := range []*ethpb.SignedBeaconBlock{b1, b100} {
				...
				beaconBlock.Block.Slot = b.Block.Slot
				beaconBlock.Block.ParentRoot = bytesutil.PadTo(b.Block.ParentRoot, 32)
				require.NoError(t, service.cfg.BeaconDB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(beaconBlock)))
			}
...
func TestVerifyBlkDescendant(t *testing.T) {
	...
	r, err := b.Block.HashTreeRoot()
	require.NoError(t, err)
	require.NoError(t, beaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b)))
	...
	r1, err := b1.Block.HashTreeRoot()
	require.NoError(t, err)
	require.NoError(t, beaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b1)))
...
func TestUpdateJustifiedInitSync(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(gBlk)))
...
func TestOnBlock_CanFinalize(t *testing.T) {
	...
	for i := types.Slot(1); i <= 4*params.BeaconConfig().SlotsPerEpoch; i++ {
		...
		require.NoError(t, err)
		require.NoError(t, service.onBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(blk), r))
