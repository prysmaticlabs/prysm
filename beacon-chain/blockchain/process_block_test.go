...
func TestStore_OnBlock_ProtoArray(t *testing.T) {
	...
	wsb, err = wrappers.WrappedSignedBeaconBlock(random)
	require.NoError(t, err)
	assert.NoError(t, beaconDB.SaveBlock(ctx, wsb))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			wsb, err = wrappers.WrappedSignedBeaconBlock(tt.blk)
			require.NoError(t, err)
...
func TestRemoveStateSinceLastFinalized_EmptyStartSlot(t *testing.T) {
	...
	require.NoError(t, err)
	wsb, err := wrappers.WrappedSignedBeaconBlock(newJustifiedBlk)
	require.NoError(t, err)
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, wsb))
	wsb, err = wrappers.WrappedSignedBeaconBlock(lastJustifiedBlk)
	require.NoError(t, err)
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, wsb))
...
func TestCachedPreState_CanGetFromStateSummary_ProtoArray(t *testing.T) {
	...
	require.NoError(t, service.verifyBlkPreState(ctx, phase0.WrappedBeaconBlock(b.Block)))
}
...
func TestCachedPreState_CanGetFromDB(t *testing.T) {
	...
	b.Block.Slot = 1
	service.store.SetFinalizedCheckpt(&ethpb.Checkpoint{Root: gRoot[:]})
	err = service.verifyBlkPreState(ctx, phase0.WrappedBeaconBlock(b.Block))
...
func TestUpdateJustified_CouldUpdateBest(t *testing.T) {
	...
	wsb, err := wrappers.WrappedSignedBeaconBlock(signedBlock)
	require.NoError(t, err)
...
func TestFillForkChoiceMissingBlocks_FilterFinalized_ProtoArray(t *testing.T) {
	...
	b63.Block.Slot = 63
	wsb, err = wrappers.WrappedSignedBeaconBlock(b63)
	require.NoError(t, err)
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, wsb))
	...
	wsb, err = wrappers.WrappedSignedBeaconBlock(b64)
	require.NoError(t, err)
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, wsb))
	...
	wsb, err = wrappers.WrappedSignedBeaconBlock(b65)
	require.NoError(t, err)
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, wsb))
...
func TestUpdateJustifiedInitSync(t *testing.T) {
	...
	require.NoError(t, err)
	wsb, err := wrappers.WrappedSignedBeaconBlock(gBlk)
	require.NoError(t, err)
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(ctx, wsb))
