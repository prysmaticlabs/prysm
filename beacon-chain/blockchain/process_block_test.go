...
func TestStore_OnBlock(t *testing.T) {
	...
	require.NoError(t, service.cfg.BeaconDB.SaveState(ctx, st.Copy(), butil.ToBytes32(randomParentRoot2)))
...
func TestRemoveStateSinceLastFinalized_EmptyStartSlot(t *testing.T) {
	...
	lastJustifiedBlk.Block.ParentRoot = butil.PadTo([]byte{'G'}, 32)
	...
	newJustifiedBlk.Block.ParentRoot = butil.PadTo(lastJustifiedRoot[:], 32)
...
func TestFillForkChoiceMissingBlocks_CanSave(t *testing.T) {
	...
	assert.Equal(t, true, service.cfg.ForkChoiceStore.HasNode(butil.ToBytes32(roots[4])), "Didn't save node")
	assert.Equal(t, true, service.cfg.ForkChoiceStore.HasNode(butil.ToBytes32(roots[6])), "Didn't save node")
	assert.Equal(t, true, service.cfg.ForkChoiceStore.HasNode(butil.ToBytes32(roots[8])), "Didn't save node")
}
...
func TestFillForkChoiceMissingBlocks_RootsMatch(t *testing.T) {
	...
	for i, rt := range wantedRoots {
		assert.Equal(t, true, service.cfg.ForkChoiceStore.HasNode(butil.ToBytes32(rt)), fmt.Sprintf("Didn't save node: %d", i))
		assert.Equal(t, true, service.cfg.BeaconDB.HasBlock(context.Background(), butil.ToBytes32(rt)))
	}
}
...
func blockTree1(t *testing.T, beaconDB db.Database, genesisRoot []byte) ([][]byte, error) {
	genesisRoot = butil.PadTo(genesisRoot, 32)
...
func TestAncestor_HandleSkipSlot(t *testing.T) {
	...
	b1 := testutil.NewBeaconBlock()
	b1.Block.Slot = 1
	b1.Block.ParentRoot = butil.PadTo([]byte{'a'}, 32)
	r1, err := b1.Block.HashTreeRoot()
	require.NoError(t, err)
...
func TestAncestor_CanUseForkchoice(t *testing.T) {
	...
	for _, b := range []*ethpb.SignedBeaconBlock{b1, b100, b200} {
		...
		require.NoError(t, err)
		require.NoError(t, service.cfg.ForkChoiceStore.ProcessBlock(context.Background(), b.Block.Slot, r, butil.ToBytes32(b.Block.ParentRoot), [32]byte{}, 0, 0)) // Saves blocks to fork choice store.
	}
...
func TestFinalizedImpliesNewJustified(t *testing.T) {
	...
	for _, test := range tests {
		...
		require.NoError(t, service.cfg.BeaconDB.SaveStateSummary(ctx, &ethpb.StateSummary{Root: butil.PadTo(test.want.Root, 32)}))
		genesisState, err := testutil.NewBeaconState()
		require.NoError(t, err)
		require.NoError(t, service.cfg.BeaconDB.SaveState(ctx, genesisState, butil.ToBytes32(test.want.Root)))
...
func TestVerifyBlkDescendant(t *testing.T) {
	...
	b1 := testutil.NewBeaconBlock()
	b1.Block.Slot = 1
	b1.Block.Body.Graffiti = butil.PadTo([]byte{'a'}, 32)
	r1, err := b1.Block.HashTreeRoot()
	require.NoError(t, err)
...
func TestInsertFinalizedDeposits(t *testing.T) {
	...
	for i := uint64(0); i < uint64(4*params.BeaconConfig().SlotsPerEpoch); i++ {
		...
		assert.NoError(t, depositCache.InsertDeposit(ctx, &ethpb.Deposit{Data: &ethpb.Deposit_Data{
			PublicKey:             butil.FromBytes48([48]byte{}),
			WithdrawalCredentials: params.BeaconConfig().ZeroHash[:],
			Amount:                0,
			Signature:             zeroSig[:],
		}, Proof: [][]byte{root}}, 100+i, int64(i), butil.ToBytes32(root)))
	}
