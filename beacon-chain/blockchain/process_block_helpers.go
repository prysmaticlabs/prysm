...
func (s *Service) getBlockPreState(ctx context.Context, b block.BeaconBlock) (state.BeaconState, error) {
	...
	preState, err := s.cfg.StateGen.StateByRoot(ctx, butil.ToBytes32(b.ParentRoot()))
	if err != nil {
...
func (s *Service) verifyBlkPreState(ctx context.Context, b block.BeaconBlock) error {
	...
	parentRoot := butil.ToBytes32(b.ParentRoot())
	...
	if err := s.VerifyBlkDescendant(ctx, butil.ToBytes32(b.ParentRoot())); err != nil {
		return err
	}
...
func (s *Service) VerifyBlkDescendant(ctx context.Context, root [32]byte) error {
	...
	defer span.End()
	fRoot := s.ensureRootNotZeros(butil.ToBytes32(s.finalizedCheckpt.Root))
	...
	if bFinalizedRoot == nil {
		return fmt.Errorf("no finalized block known for block %#x", butil.Trunc(root[:]))
	}

	if !bytes.Equal(bFinalizedRoot, fRoot[:]) {
		err := fmt.Errorf("block %#x is not a descendent of the current finalized block slot %d, %#x != %#x",
			butil.Trunc(root[:]), finalizedBlk.Slot(), butil.Trunc(bFinalizedRoot),
			butil.Trunc(fRoot[:]))
...
func (s *Service) shouldUpdateCurrentJustified(ctx context.Context, newJustifiedCheckpt *ethpb.Checkpoint) (bool, error) {
	...
	justifiedRoot := s.ensureRootNotZeros(butil.ToBytes32(newJustifiedCheckpt.Root))
	var err error
	...
	cachedJustifiedRoot := s.ensureRootNotZeros(butil.ToBytes32(s.justifiedCheckpt.Root))
...
func (s *Service) ancestorByDB(ctx context.Context, r [32]byte, slot types.Slot) ([]byte, error) {
	...
	return s.ancestorByDB(ctx, butil.ToBytes32(b.ParentRoot()), slot)
}
...
func (s *Service) finalizedImpliesNewJustified(ctx context.Context, state state.BeaconState) error {
	...
	if !attestation.CheckPointIsEqual(s.justifiedCheckpt, state.CurrentJustifiedCheckpoint()) {
		if state.CurrentJustifiedCheckpoint().Epoch > s.justifiedCheckpt.Epoch {
			...
			return s.cacheJustifiedStateBalances(ctx, butil.ToBytes32(s.justifiedCheckpt.Root))
		}
		...
		justifiedRoot := s.ensureRootNotZeros(butil.ToBytes32(s.justifiedCheckpt.Root))
...
func (s *Service) fillInForkChoiceMissingBlocks(ctx context.Context, blk block.BeaconBlock,
	...
	parentRoot := butil.ToBytes32(blk.ParentRoot())
	slot := blk.Slot()
	...
	for i := len(pendingNodes) - 1; i >= 0; i-- {
		...
		if err := s.cfg.ForkChoiceStore.ProcessBlock(ctx,
			b.Slot(), r, butil.ToBytes32(b.ParentRoot()), butil.ToBytes32(b.Body().Graffiti()),
			jCheckpoint.Epoch,
