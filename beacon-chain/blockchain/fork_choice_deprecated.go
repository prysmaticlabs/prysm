...
func (c *ChainService) updateFFGCheckPts(ctx context.Context, state *pb.BeaconState) error {
	...
	savedJustifiedBlock, err := c.deprecatedBeaconDB.JustifiedBlock()
	if err != nil {
		return err
	}
	...
	if lastJustifiedSlot > savedJustifiedBlock.Slot {
		...
		newJustifiedBlock, err := c.deprecatedBeaconDB.CanonicalBlockBySlot(ctx, lastJustifiedSlot)
		if err != nil {
			return err
		}
		...
		for newJustifiedBlock == nil {
			...
			lastAvailBlkSlot--
			newJustifiedBlock, err = c.deprecatedBeaconDB.CanonicalBlockBySlot(ctx, lastAvailBlkSlot)
			if err != nil {
				return err
			}
		}
		...
		newJustifiedState, err := c.deprecatedBeaconDB.HistoricalStateFromSlot(ctx, newJustifiedBlock.Slot, newJustifiedRoot)
		if err != nil {
			return err
		}
		if err := c.deprecatedBeaconDB.SaveJustifiedBlock(newJustifiedBlock); err != nil {
			return err
		}
		if err := c.deprecatedBeaconDB.SaveJustifiedState(newJustifiedState); err != nil {
			return err
		}
	}

	lastFinalizedSlot := helpers.StartSlot(state.FinalizedCheckpoint.Epoch)
	savedFinalizedBlock, err := c.deprecatedBeaconDB.FinalizedBlock()
	...
	if lastFinalizedSlot > savedFinalizedBlock.Slot {
		...
		newFinalizedBlock, err := c.deprecatedBeaconDB.CanonicalBlockBySlot(ctx, lastFinalizedSlot)
		if err != nil {
			return err
		}
		...
		for newFinalizedBlock == nil {
			log.WithField("slot", lastAvailBlkSlot).Debug("Missing block in DB, looking one slot back")
			lastAvailBlkSlot--
			newFinalizedBlock, err = c.deprecatedBeaconDB.CanonicalBlockBySlot(ctx, lastAvailBlkSlot)
			if err != nil {
				return err
			}
		}
		...
		// save it.
		newFinalizedState, err := c.deprecatedBeaconDB.HistoricalStateFromSlot(ctx, lastFinalizedSlot, newFinalizedRoot)
		if err != nil {
			return err
		}
		if err := c.deprecatedBeaconDB.SaveFinalizedBlock(newFinalizedBlock); err != nil {
			return err
		}
		if err := c.deprecatedBeaconDB.SaveFinalizedState(newFinalizedState); err != nil {
			return err
		}
	}
	return nil
...
func (c *ChainService) ApplyForkChoiceRuleDeprecated(
	...
	justifiedState, err := c.deprecatedBeaconDB.JustifiedState()
	if err != nil {
	...
	justifiedHead, err := c.deprecatedBeaconDB.JustifiedBlock()
	if err != nil {
	...
	currentHead, err := c.deprecatedBeaconDB.ChainHead()
	if err != nil {
	...
	if !isDescendant && !proto.Equal(currentHead, newHead) {
		...
		newState, err = c.deprecatedBeaconDB.HistoricalStateFromSlot(ctx, newHead.Slot, newHeadRoot)
		if err != nil {
	...
	if err := c.deprecatedBeaconDB.UpdateChainHead(ctx, newHead, newState); err != nil {
...
func (c *ChainService) lmdGhost(
	...
	highestSlot := c.deprecatedBeaconDB.HighestBlockSlot()
	head := startBlock
	for {
		...
		maxChildVotes, err := VoteCount(maxChild, startState, voteTargets, c.deprecatedBeaconDB)
		if err != nil {
			return nil, errors.Wrap(err, "unable to determine vote count for block")
		}
		for i := 1; i < len(children); i++ {
			candidateChildVotes, err := VoteCount(children[i], startState, voteTargets, c.deprecatedBeaconDB)
			if err != nil {
				return nil, errors.Wrap(err, "unable to determine vote count for block")
			}
...
func (c *ChainService) BlockChildren(ctx context.Context, block *ethpb.BeaconBlock, highestSlot uint64) ([]*ethpb.BeaconBlock, error) {
	...
	for i := startSlot; i <= highestSlot; i++ {
		kids, err := c.deprecatedBeaconDB.BlocksBySlot(ctx, i)
		if err != nil {
...
func (c *ChainService) isDescendant(currentHead *ethpb.BeaconBlock, newHead *ethpb.BeaconBlock) (bool, error) {
	...
	for newHead.Slot > currentHead.Slot {
		...
		newHead, err = c.deprecatedBeaconDB.Block(bytesutil.ToBytes32(newHead.ParentRoot))
		if err != nil {
			return false, err
		}
		if newHead == nil {
