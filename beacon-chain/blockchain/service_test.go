...
func setupBeaconChain(t *testing.T, beaconDB db.Database) *Service {
	...
	require.NoError(t, cfg.StateGen.SaveState(ctx, butil.ToBytes32(bState.FinalizedCheckpoint().Root), bState))
...
func TestChainService_InitializeBeaconChain(t *testing.T) {
	...
	require.NoError(t, err)
	if butil.ToBytes32(r) == params.BeaconConfig().ZeroHash {
...
func TestChainService_InitializeChainInfo(t *testing.T) {
	...
	headBlock.Block.ParentRoot = butil.PadTo(genesisRoot[:], 32)
