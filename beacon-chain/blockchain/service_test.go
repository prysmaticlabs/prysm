...
func TestChainService_InitializeBeaconChain(t *testing.T) {
	...
	count := uint64(10)
	deposits, _, err := customtesting.DeterministicDepositsAndKeys(count)
	require.NoError(t, err)
	trie, _, err := customtesting.DepositTrieFromDeposits(deposits)
	require.NoError(t, err)
...
func TestChainService_InitializeChainInfo(t *testing.T) {
	...
	headBlock := customtesting.NewBeaconBlock()
...
func TestChainService_SaveHeadNoDB(t *testing.T) {
	...
	require.NoError(t, err)
	newState, err := customtesting.NewBeaconState()
	require.NoError(t, err)
