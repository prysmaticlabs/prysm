...
func TestRunningChainService(t *testing.T) {
	...
	slotsStart := crystallized.LastStateRecalculationSlot() - params.GetBeaconConfig().CycleLength
	slotIndex := (currentSlot - slotsStart) % params.GetBeaconConfig().CycleLength
