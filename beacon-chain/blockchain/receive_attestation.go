...
func (s *Service) processAttestation() {
	...
	for {
		...
		case <-st.C():
			...
			for _, a := range atts {
				hasState := s.beaconDB.HasHotStateSummary(ctx, bytesutil.ToBytes32(a.Data.BeaconBlockRoot)) && s.beaconDB.HasHotStateSummary(ctx, bytesutil.ToBytes32(a.Data.Target.Root))
