...
func (s *Service) VerifyFinalizedConsistency(ctx context.Context, root []byte) error {
	...
	if s.cfg.ForkChoiceStore.IsCanonical(bytes.ToBytes32(root)) {
		return nil
	}
...
func (s *Service) processAttestations(ctx context.Context) {
	...
	for _, a := range atts {
		...
		hasState := s.cfg.BeaconDB.HasStateSummary(ctx, bytes.ToBytes32(a.Data.BeaconBlockRoot))
		hasBlock := s.hasBlock(ctx, bytes.ToBytes32(a.Data.BeaconBlockRoot))
		...
		if err := s.ReceiveAttestationNoPubsub(ctx, a); err != nil {
			log.WithFields(logrus.Fields{
				...
				"beaconBlockRoot":  fmt.Sprintf("%#x", bytes.Trunc(a.Data.BeaconBlockRoot)),
				"targetRoot":       fmt.Sprintf("%#x", bytes.Trunc(a.Data.Target.Root)),
