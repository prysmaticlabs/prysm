...
func TestService_ReceiveBlock(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			err = s.ReceiveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(tt.args.block), root)
			if tt.wantedErr != "" {
...
func TestService_ReceiveBlockUpdateHead(t *testing.T) {
	...
	wg.Add(1)
	go func() {
		require.NoError(t, s.ReceiveBlock(ctx, blockInterface.WrappedPhase0SignedBeaconBlock(b), root))
		wg.Done()
	}()
	wg.Wait()
	if recvd := len(s.cfg.StateNotifier.(*blockchainTesting.MockStateNotifier).ReceivedEvents()); recvd < 1 {
...
func TestService_ReceiveBlockBatch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			root, err := tt.args.block.Block.HashTreeRoot()
			require.NoError(t, err)
			blks := []blockInterface.SignedBeaconBlock{blockInterface.WrappedPhase0SignedBeaconBlock(tt.args.block)}
			roots := [][32]byte{root}
...
func TestService_HasInitSyncBlock(t *testing.T) {
	...
	s.saveInitSyncBlock(r, blockInterface.WrappedPhase0SignedBeaconBlock(testutil.NewBeaconBlock()))
	if !s.HasInitSyncBlock(r) {
