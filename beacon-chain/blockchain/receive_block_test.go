...
func TestService_ReceiveBlock(t *testing.T) {
	...
	genFullBlock := func(t *testing.T, conf *customtesting.BlockGenConfig, slot types.Slot) *ethpb.SignedBeaconBlock {
		blk, err := customtesting.GenerateFullBlock(genesis, keys, conf, slot)
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				block: genFullBlock(t, customtesting.DefaultBlockGenConfig(), 2 /*slot*/),
			},
		...
		},
		{
			...
			args: args{
				block: genFullBlock(t, &customtesting.BlockGenConfig{
					NumProposerSlashings: 0,
					NumAttesterSlashings: 0,
					NumAttestations:      0,
					NumDeposits:          0,
		...
		},
		{
			...
			args: args{
				block: genFullBlock(t, customtesting.DefaultBlockGenConfig(), 1 /*slot*/),
			},
			check: func(t *testing.T, s *Service) {
...
func TestService_ReceiveBlockBatch_UpdateFinalizedCheckpoint(t *testing.T) {
	...
	for i := types.Slot(1); i < params.BeaconConfig().SlotsPerEpoch*5; i++ {
		b, err := customtesting.GenerateFullBlock(copied, keys, customtesting.DefaultBlockGenConfig(), i)
		assert.NoError(t, err)
...
func TestService_HasInitSyncBlock(t *testing.T) {
	...
	s.saveInitSyncBlock(r, wrapper.WrappedPhase0SignedBeaconBlock(customtesting.NewBeaconBlock()))
	if !s.HasInitSyncBlock(r) {
