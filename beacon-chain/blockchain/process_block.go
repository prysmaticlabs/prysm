...
func (s *Service) onBlock(ctx context.Context, signed block.SignedBeaconBlock, blockRoot [32]byte) error {
	...
	if newFinalized {
		...
		fRoot := bytes.ToBytes32(postState.FinalizedCheckpoint().Root)
...
func (s *Service) onBlockBatch(ctx context.Context, blks []block.SignedBeaconBlock,
	...
	preState, err := s.cfg.StateGen.StateByRootInitialSync(ctx, bytes.ToBytes32(b.ParentRoot()))
	if err != nil {
		return nil, nil, err
	}
...
func (s *Service) insertBlockAndAttestationsToForkChoiceStore(ctx context.Context, blk block.BeaconBlock, root [32]byte,
	...
	for _, a := range blk.Body().Attestations() {
		...
		s.cfg.ForkChoiceStore.ProcessAttestation(ctx, indices, bytes.ToBytes32(a.Data.BeaconBlockRoot), a.Data.Target.Epoch)
	}
	return nil
}
...
func (s *Service) insertBlockToForkChoiceStore(ctx context.Context, blk block.BeaconBlock,
	...
	if err := s.cfg.ForkChoiceStore.ProcessBlock(ctx,
		blk.Slot(), root, bytes.ToBytes32(blk.ParentRoot()), bytes.ToBytes32(blk.Body().Graffiti()),
		jCheckpoint.Epoch,
