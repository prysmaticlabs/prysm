...
func (s *Service) saveInitSyncBlock(r [32]byte, b block2.SignedBeaconBlock) {
...
// the block.
func (s *Service) getInitSyncBlock(r [32]byte) block2.SignedBeaconBlock {
	s.initSyncBlocksLock.RLock()
	defer s.initSyncBlocksLock.RUnlock()
...
func (s *Service) getInitSyncBlocks() []block2.SignedBeaconBlock {
	s.initSyncBlocksLock.RLock()
	defer s.initSyncBlocksLock.RUnlock()

	blks := make([]block2.SignedBeaconBlock, 0, len(s.initSyncBlocks))
...
func (s *Service) clearInitSyncBlocks() {
	s.initSyncBlocksLock.Lock()
	defer s.initSyncBlocksLock.Unlock()
	s.initSyncBlocks = make(map[[32]byte]block2.SignedBeaconBlock)
}
