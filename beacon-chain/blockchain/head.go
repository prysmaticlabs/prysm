...
type head struct {
	...
	block blockInterface.SignedBeaconBlock // current head block.
...
func (s *Service) saveHeadNoDB(ctx context.Context, b blockInterface.SignedBeaconBlock, r [32]byte, hs iface.BeaconState) error {
...
func (s *Service) setHead(root [32]byte, block blockInterface.SignedBeaconBlock, state iface.BeaconState) {
	s.headLock.Lock()
...
func (s *Service) setHeadInitialSync(root [32]byte, block blockInterface.SignedBeaconBlock, state iface.BeaconState) {
	s.headLock.Lock()
	defer s.headLock.Unlock()
...
// This is a lock free version.
func (s *Service) headBlock() blockInterface.SignedBeaconBlock {
