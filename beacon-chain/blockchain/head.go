...
type head struct {
	...
	block prysm.SignedBeaconBlock // current head block.
...
func (s *Service) saveHeadNoDB(ctx context.Context, b prysm.SignedBeaconBlock, r [32]byte, hs state.BeaconState) error {
...
func (s *Service) setHead(root [32]byte, block prysm.SignedBeaconBlock, state state.BeaconState) {
	s.headLock.Lock()
...
func (s *Service) setHeadInitialSync(root [32]byte, block prysm.SignedBeaconBlock, state state.BeaconState) {
	s.headLock.Lock()
	defer s.headLock.Unlock()
...
// This is a lock free version.
func (s *Service) headBlock() prysm.SignedBeaconBlock {
