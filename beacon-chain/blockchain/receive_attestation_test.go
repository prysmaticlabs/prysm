...
func TestVerifyLMDFFGConsistent_NotOK(t *testing.T) {
	...
	a := customtesting.NewAttestation()
...
func TestProcessAttestations_Ok(t *testing.T) {
	...
	require.NoError(t, err)
	genesisState, pks := customtesting.DeterministicGenesisState(t, 64)
	require.NoError(t, genesisState.SetGenesisTime(uint64(timeutils.Now().Unix())-params.BeaconConfig().SecondsPerSlot))
	require.NoError(t, service.saveGenesisData(ctx, genesisState))
	atts, err := customtesting.GenerateAttestations(genesisState, pks, 1, 0, false)
	require.NoError(t, err)
