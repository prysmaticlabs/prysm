...
func TestFinalizedCheckpt_CanRetrieve(t *testing.T) {
	...
	cp := &ethpb.Checkpoint{Epoch: 5, Root: butil.PadTo([]byte("foo"), 32)}
	c := setupBeaconChain(t, beaconDB)
...
func TestCurrentJustifiedCheckpt_CanRetrieve(t *testing.T) {
	...
	c := setupBeaconChain(t, beaconDB)
	assert.Equal(t, params.BeaconConfig().ZeroHash, butil.ToBytes32(c.CurrentJustifiedCheckpt().Root), "Unexpected justified epoch")
	cp := &ethpb.Checkpoint{Epoch: 6, Root: butil.PadTo([]byte("foo"), 32)}
...
func TestPreviousJustifiedCheckpt_CanRetrieve(t *testing.T) {
	...
	cp := &ethpb.Checkpoint{Epoch: 7, Root: butil.PadTo([]byte("foo"), 32)}
	c := setupBeaconChain(t, beaconDB)
	assert.Equal(t, params.BeaconConfig().ZeroHash, butil.ToBytes32(c.CurrentJustifiedCheckpt().Root), "Unexpected justified epoch")
...
func TestHeadRoot_CanRetrieve(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, [32]byte{'A'}, butil.ToBytes32(r))
}
...
func TestHeadRoot_UseDB(t *testing.T) {
	...
	r, err := c.HeadRoot(context.Background())
	require.NoError(t, err)
	assert.Equal(t, br, butil.ToBytes32(r))
}
...
func TestService_HeadPublicKeyToValidatorIndex(t *testing.T) {
	...
	i, e := c.HeadPublicKeyToValidatorIndex(context.Background(), butil.ToBytes48(v.PublicKey))
...
func TestService_HeadValidatorIndexToPublicKey(t *testing.T) {
	...
	require.Equal(t, butil.ToBytes48(v.PublicKey), p)
}
