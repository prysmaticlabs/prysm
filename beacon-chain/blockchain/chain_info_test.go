...
func TestHeadRoot_UseDB(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, beaconDB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(b)))
...
func TestHeadBlock_CanRetrieve(t *testing.T) {
	...
	require.NoError(t, err)
	c := &Service{}
	c.head = &head{block: blockInterface.WrappedPhase0SignedBeaconBlock(b), state: s}
