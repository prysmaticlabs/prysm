...
func (s *Service) Start() {
	...
	if beaconState != nil && !beaconState.IsNil() {
		...
		if err := s.cacheJustifiedStateBalances(s.ctx, s.ensureRootNotZeros(butil.ToBytes32(s.justifiedCheckpt.Root))); err != nil {
...
func (s *Service) initializeChainInfo(ctx context.Context) error {
	...
	finalizedRoot := s.ensureRootNotZeros(butil.ToBytes32(finalized.Root))
...
func (s *Service) resumeForkChoice(justifiedCheckpoint, finalizedCheckpoint *ethpb.Checkpoint) {
	store := protoarray.New(justifiedCheckpoint.Epoch, finalizedCheckpoint.Epoch, butil.ToBytes32(finalizedCheckpoint.Root))
