...
func (c *ChainService) generateStateFromSlot(slot uint64) (*pb.BeaconState, error) {
	fState, err := c.beaconDB.FinalizedState(slot)
	if err != nil {
		return nil, err
	}
	...
	pBlock, err := c.beaconDB.BlockBySlot(fState.Slot)
	...
	for i := fState.Slot + 1; i <= slot; i++ {
		exists, blk, err := c.beaconDB.HasBlockBySlot(i)
		if !exists {
