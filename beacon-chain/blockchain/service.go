...
func (s *Service) Start() {
	...
	if beaconState != nil {
		...
		if err := s.forkchoice.ProcessBlock(ctx, headBlock.Block.Slot, headBlockRoot, bytesutil.ToBytes32(headBlock.Block.ParentRoot), finalizedCheckpoint.Epoch, justifiedCheckpoint.Epoch); err != nil {
...
func (s *Service) saveGenesisData(ctx context.Context, genesisState *pb.BeaconState) error {
	...
	s.forkchoice = f.New(
		genesisState.CurrentJustifiedCheckpoint.Epoch,
		genesisState.FinalizedCheckpoint.Epoch,
		bytesutil.ToBytes32(genesisState.FinalizedCheckpoint.Root))

	if err := s.forkchoice.ProcessBlock(ctx, genesisBlk.Block.Slot, genesisBlkRoot, params.BeaconConfig().ZeroHash, genesisCheckpoint.Epoch, genesisCheckpoint.Epoch); err != nil {
