...
func TestSaveHead_Different(t *testing.T) {
	...
	service.head = &head{
		slot: 0,
		root: oldRoot,
		block: blockInterface.WrappedPhase0SignedBeaconBlock(
	...
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(newHeadSignedBlock)))
...
func TestUpdateHead_MissingJustifiedRoot(t *testing.T) {
	...
	require.NoError(t, service.cfg.BeaconDB.SaveBlock(context.Background(), blockInterface.WrappedPhase0SignedBeaconBlock(b)))
