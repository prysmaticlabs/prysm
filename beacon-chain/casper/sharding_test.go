...
func BenchmarkMaxValidators(b *testing.B) {
	...
	for i := 0; i < params.GetBeaconConfig().ModuloBias; i++ {
...
func TestGetCommitteesPerSlotSmallValidatorSet(t *testing.T) {
	numValidators := int(params.GetBeaconConfig().CycleLength * params.GetBeaconConfig().MinCommiteeSize / 4)
...
func TestGetCommitteesPerSlotRegularValidatorSet(t *testing.T) {
	numValidators := int(params.GetBeaconConfig().CycleLength * params.GetBeaconConfig().MinCommiteeSize)
...
func TestGetCommitteesPerSlotLargeValidatorSet(t *testing.T) {
	numValidators := int(params.GetBeaconConfig().CycleLength*params.GetBeaconConfig().MinCommiteeSize) * 8
...
func TestValidatorsBySlotShardRegularValidatorSet(t *testing.T) {
	...
	for i := 0; i < len(shardAndCommitteeArray); i++ {
		...
		if len(shardAndCommittees) != 1 {
			t.Fatalf("Expected %d committee per slot: got %d", params.GetBeaconConfig().MinCommiteeSize, 1)
		}

		committeeSize := len(shardAndCommittees[0].Committee)
		if committeeSize != int(params.GetBeaconConfig().MinCommiteeSize) {
			t.Fatalf("Expected committee size %d: got %d", params.GetBeaconConfig().MinCommiteeSize, committeeSize)
		}
	}
}
...
func TestValidatorsBySlotShardLargeValidatorSet(t *testing.T) {
	validatorIndices := []uint32{}
	numValidators := int(params.GetBeaconConfig().CycleLength*params.GetBeaconConfig().MinCommiteeSize) * 2
	for i := 0; i < numValidators; i++ {
		validatorIndices = append(validatorIndices, uint32(i))
	}
	...
	for i := 0; i < len(shardAndCommitteeArray); i++ {
		...
		if len(shardAndCommittees) != 2 {
			t.Fatalf("Expected %d committee per slot: got %d", params.GetBeaconConfig().MinCommiteeSize, 2)
		}
		...
		for j := 0; j < len(shardAndCommittees); j++ {
			...
			if len(shardCommittee.Committee) != int(params.GetBeaconConfig().MinCommiteeSize) {
				t.Fatalf("Expected committee size %d: got %d", params.GetBeaconConfig().MinCommiteeSize, len(shardCommittee.Committee))
			}
		}
...
func TestValidatorsBySlotShardSmallValidatorSet(t *testing.T) {
	...
	for i := 0; i < len(shardAndCommitteeArray); i++ {
		...
		for j := 0; j < len(shardAndCommittees); j++ {
			shardCommittee := shardAndCommittees[j]
			if len(shardCommittee.Committee) != int(params.GetBeaconConfig().MinCommiteeSize/2) {
				t.Fatalf("Expected committee size %d: got %d", params.GetBeaconConfig().MinCommiteeSize/2, len(shardCommittee.Committee))
			}
		}
	}
}
