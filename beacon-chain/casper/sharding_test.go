...
func BenchmarkMaxValidators(b *testing.B) {
	...
	for i := uint64(0); i < params.GetBeaconConfig().ModuloBias; i++ {
...
func TestInitialShardAndCommiteeForSlots(t *testing.T) {
	...
	if uint64(len(shardAndCommitteeArray)) != 3*params.GetBeaconConfig().CycleLength {
		t.Errorf("shard committee slots are not as expected: %d instead of %d", len(shardAndCommitteeArray), 3*params.GetBeaconConfig().CycleLength)
	}
...
func TestShuffleActiveValidators(t *testing.T) {
	...
	if len(indices) != int(params.GetBeaconConfig().CycleLength) {
		t.Errorf("incorret length for validator indices. Want: %d. Got: %v", params.GetBeaconConfig().CycleLength, len(indices))
	}
}
...
func TestSmallSampleValidators(t *testing.T) {
	...
	if len(indices) != int(params.GetBeaconConfig().CycleLength) {
		t.Errorf("incorret length for validator indices. Want: %d. Got: %d", params.GetBeaconConfig().CycleLength, len(indices))
	}
}

func TestGetCommitteesPerSlotSmallValidatorSet(t *testing.T) {
	numValidators := params.GetBeaconConfig().CycleLength * params.GetBeaconConfig().MinCommitteeSize / 4
...
func TestGetCommitteesPerSlotRegularValidatorSet(t *testing.T) {
	numValidators := params.GetBeaconConfig().CycleLength * params.GetBeaconConfig().MinCommitteeSize
...
func TestGetCommitteesPerSlotLargeValidatorSet(t *testing.T) {
	numValidators := params.GetBeaconConfig().CycleLength * params.GetBeaconConfig().MinCommitteeSize * 8
...
func TestValidatorsBySlotShardRegularValidatorSet(t *testing.T) {
	...
	numValidators := int(params.GetBeaconConfig().CycleLength * params.GetBeaconConfig().MinCommitteeSize)
	...
	if len(shardAndCommitteeArray) != int(params.GetBeaconConfig().CycleLength) {
		t.Fatalf("Expected length %d: got %d", params.GetBeaconConfig().CycleLength, len(shardAndCommitteeArray))
	}
	...
	for i := 0; i < len(shardAndCommitteeArray); i++ {
		...
		if len(shardAndCommittees) != 1 {
			t.Fatalf("Expected %d committee per slot: got %d", params.GetBeaconConfig().MinCommitteeSize, 1)
		}

		committeeSize := len(shardAndCommittees[0].Committee)
		if committeeSize != int(params.GetBeaconConfig().MinCommitteeSize) {
			t.Fatalf("Expected committee size %d: got %d", params.GetBeaconConfig().MinCommitteeSize, committeeSize)
		}
	}
}
...
func TestValidatorsBySlotShardLargeValidatorSet(t *testing.T) {
	validatorIndices := []uint32{}
	numValidators := int(params.GetBeaconConfig().CycleLength*params.GetBeaconConfig().MinCommitteeSize) * 2
	for i := 0; i < numValidators; i++ {
		validatorIndices = append(validatorIndices, uint32(i))
	}
	...
	for i := 0; i < len(shardAndCommitteeArray); i++ {
		...
		if len(shardAndCommittees) != 2 {
			t.Fatalf("Expected %d committee per slot: got %d", params.GetBeaconConfig().MinCommitteeSize, 2)
		}
		...
		for j := 0; j < len(shardAndCommittees); j++ {
			...
			if len(shardCommittee.Committee) != int(params.GetBeaconConfig().MinCommitteeSize) {
				t.Fatalf("Expected committee size %d: got %d", params.GetBeaconConfig().MinCommitteeSize, len(shardCommittee.Committee))
			}
		}
...
func TestValidatorsBySlotShardSmallValidatorSet(t *testing.T) {
	...
	for i := 0; i < len(shardAndCommitteeArray); i++ {
		...
		for j := 0; j < len(shardAndCommittees); j++ {
			shardCommittee := shardAndCommittees[j]
			if len(shardCommittee.Committee) != int(params.GetBeaconConfig().MinCommitteeSize/2) {
				t.Fatalf("Expected committee size %d: got %d", params.GetBeaconConfig().MinCommitteeSize/2, len(shardCommittee.Committee))
			}
		}
	}
}
