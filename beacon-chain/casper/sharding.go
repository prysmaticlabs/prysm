...
func splitBySlotShard(shuffledValidators []uint32, crosslinkStartShard uint64) []*pb.ShardAndCommitteeArray {
	...
	validatorsBySlot := utils.SplitIndices(shuffledValidators, int(params.GetBeaconConfig().CycleLength))
	for i, validatorsForSlot := range validatorsBySlot {
		...
		for j, validatorsForShard := range validatorsByShard {
			shardID := (shardStart + j) % params.GetBeaconConfig().ShardCount
...
func getCommitteesPerSlot(numActiveValidators int) int {
	cycleLength := int(params.GetBeaconConfig().CycleLength)
	boundOnValidators := numActiveValidators/cycleLength/int(params.GetBeaconConfig().MinCommiteeSize*2) + 1
